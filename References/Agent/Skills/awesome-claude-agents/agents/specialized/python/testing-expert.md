...
class TestProductEndpoints:
    ...
    async def test_product_search_endpoint(self, async_client: AsyncClient, db_session):
        ...
        products = [
            ...
            ProductFactory.create(
                name="iPad Air",                description="Apple tablet for productivity",
                tags=["apple", "tablet", "ios"]
            ),
            ProductFactory.create(
                name="Dell Laptop",                description="Windows laptop for business",
...
class ProductFactory(factory.Factory):
    ...
    dimensions = factory.LazyFunction(
        lambda: {
            ...
            "width": random.uniform(1.0, 50.0),            "height": random.uniform(1.0, 50.0)
        }
    )
    tags = factory.LazyFunction(
        lambda: random.sample(['electronics', 'gadgets', 'accessories', 'premium', 'sale'],                            random.randint(1, 3))
    )
    created_at = Faker('date_time_this_year')
