...
1. **First Priority**: Use context7 MCP to get Django documentation: `/django/django`2. **Fallback**: Use WebFetch to get documentation from docs.djangoproject.com
...
class ProductAdmin(admin.ModelAdmin):
    list_display = [
        'name', 'category', 'price_display',        'stock_display', 'is_published', 'is_featured'
    ]
...
def send_import_notification(import_id: int):
    ...
    context = {
        'import': import_obj,
        'success_rate': (import_obj.processed_rows /                        (import_obj.processed_rows + import_obj.error_rows) * 100)
    }
    html_message = render_to_string(
        'emails/import_complete.html',        context
    )
    send_mail(
