...
from django.db.models import (
    F, Q, Count, Sum, Avg, Max, Min,    Prefetch, OuterRef, Subquery, Exists,
...
class AnalyticsQueries:
        ...
        return Product.objects.annotate(
            ...
            growth_pct=Case(
                ...
                default=ExpressionWrapper(
                    (F('total_revenue') - F('prev_month_revenue')) * 100.0 /                    F('prev_month_revenue'),
                    output_field=DecimalField()
                )
            )
        ).filter(
    ...
    def customer_lifetime_value():
        ...
        return User.objects.annotate(
            ...
            segment=Case(
                When(
                    Q(recency__lte=timedelta(days=30)) &                    Q(order_count__gte=5) &                    Q(total_spent__gte=1000),
                    then=Value('VIP')
                ),
                When(
                    Q(recency__lte=timedelta(days=90)) &                    Q(order_count__gte=2),
                    then=Value('Active')
                ),
                When(
...
def create_order_partitions():
    ...
    with connection.cursor() as cursor:
        ...
        for month in range(1, 13):
            cursor.execute(f"""
                CREATE TABLE IF NOT EXISTS orders_2024_{month:02d}                PARTITION OF orders
                FOR VALUES FROM ('2024-{month:02d}-01')                TO ('2024-{(month%12)+1:02d}-01');
            """)
            # Create indexes on partition
            cursor.execute(f"""
                CREATE INDEX idx_orders_2024_{month:02d}_user                ON orders_2024_{month:02d}(user_id);
            """)
```
    ...
    def get_sales_heatmap():
        ...
        with connection.cursor() as cursor:
            cursor.execute("""
                ...
                SELECT                    d.names[h.day_of_week + 1] as day_name,
                    h.hour_of_day,
                    h.order_count,
                    h.revenue,
        ...
        with connection.cursor() as cursor:
            cursor.execute("""
                ...
                WHERE p.id = r.product_id
                    AND (p.review_count != r.count                         OR p.avg_rating != r.avg_rating)
            """)
