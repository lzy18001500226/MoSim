"""Shared helpers for Uniswap V3 skills."""
...
async def get_token_symbol(w3: Any, token_address: str, chain_id: int) -> str:
    """Get ERC20 token symbol. Returns wrapped native symbol for native tokens."""
    wrapped = WRAPPED_NATIVE_ADDRESSES.get(chain_id, "").lower()
    if token_address.lower() == wrapped:
        return NATIVE_WRAPPED_SYMBOL.get(chain_id, "WETH")
    try:
        symbol_abi = [
            {
                "inputs": [],
