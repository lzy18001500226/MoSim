...
<CodeGroup>
```bash yarn
yarn add mcp-use @langchain/openai
```
```bash npm
...
const agent = new MCPAgent({
  llm: new ChatOpenAI({ model: "gpt-5.5" }), // use your preferred LLM provider
  client,
  maxSteps: 30
})
...
const result = await agent.run({
    prompt: 'Find the best restaurant in San Francisco USING GOOGLE SEARCH'
})
console.log(`\nResult: ${result}`)
...
It improves the official Model Context Protocol SDK with support for Edge Runtime, MCP Apps, ChatGPT Apps SDK, and MCP-UI. In addition, it supports all official MCP features and achieves a conformance score of 100/100 based on the official MCP Conformance Tests.
...
- [MCP Apps](/typescript/server/mcp-apps): Build interactive widgets with the standard MCP Apps protocol (works with ChatGPT and MCP clients).
- [ChatGPT Apps SDK](/typescript/server/mcp-apps): ChatGPT-specific widget protocol details.
...
<CardGroup cols={3}>
  ...
  <Card title="MCP Apps" icon="app-window-mac" href="/typescript/server/mcp-apps">
    Build interactive widgets for ChatGPT and MCP Apps clients.
  </Card>
