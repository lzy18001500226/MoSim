...
class Angle_TEST < Test::Unit::TestCase
  def test_construction
    ...
    assert(angle1 == Gz::Math::Angle.Pi,
           "180.0 degrees should equal PI")

    assert(angle1 != Gz::Math::Angle.Pi + Gz::Math::Angle.new(0.1),
           "180.0 degrees should not equal PI + 0.1")

    assert(angle1 == Gz::Math::Angle.Pi +
           Gz::Math::Angle.new(0.0001),
    ...
    assert(Gz::Math::Angle.new(0) == Gz::Math::Angle.new(0),
           "Zero angle should equal zero angle")

    assert(Gz::Math::Angle.new(0) == Gz::Math::Angle.new(0.001),
           "Zero should equal 0.001")

    angle1 = Gz::Math::Angle.new(0.1) - Gz::Math::Angle.new(0.3)
    assert(angle1 == Gz::Math::Angle.new(-0.2),
           "Angle1 should equal -0.2")

    angle = Gz::Math::Angle.new(0.5)
    assert(0.5 == angle.Radian, "Angle should equal 0.5")

    angle.SetRadian(Math::PI)
    assert(Gz::Math::Angle.Pi.Degree == angle.Degree,
           "Pi in degrees should equal Angle.SetRadian(Pi).Degree")

    angleNorm = angle.Normalized()

    angle.Normalize()
    assert(Gz::Math::Angle.new(Math::PI).Degree == angle.Degree,
           "Normalized angle should equal PI")
    assert(angleNorm == angle, "Normalized angles should be equal")

    angle = Gz::Math::Angle.new(0.1) + Gz::Math::Angle.new(0.2)
    ...
    assert(angle == Gz::Math::Angle.new(3), "Angle should equal Angle(3)")
    assert(angle != Gz::Math::Angle.new(2),
           "Angle should not equal Angle(2)")
    assert(angle < Gz::Math::Angle.new(4), "Angle should be < Angle(4)")
    assert(angle > Gz::Math::Angle.new(2), "Angle should be > Angle(2)")
    assert(angle >= Gz::Math::Angle.new(3), "Angle should be >= Angle(3)")
    assert(angle <= Gz::Math::Angle.new(3), "Angle should be <= 3")
    ...
    assert(Gz::Math::Angle.new(1.2000000001) <=
           Gz::Math::Angle.new(1.2),
