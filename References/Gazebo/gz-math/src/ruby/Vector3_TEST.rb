...
class Vector3_TEST < Test::Unit::TestCase
  def test_construction
    ...
    assert(v.Rounded() == Gz::Math::Vector3d.new(1, 2, 4),
           "Vector3d::Rounded() should equal [1, 2, 4]")

    # ::Round
    v.Round()
    assert(v.Round() == Gz::Math::Vector3d.new(1, 2, 4),
           "Vector3d::Round should equal [1,2,4]")

    # ::DotProd
    assert(v.Dot(Gz::Math::Vector3d.new(1, 2, 3)) == 17.0,
           "Vector3d::Dot([1,2,3]) should equal 17.0")

    # ::DistToLine
    v.Set(0, 0, 0)
    assert(1.0 == v.DistToLine(Gz::Math::Vector3d.new(1, -1, 0),
                               Gz::Math::Vector3d.new(1, 1, 0)),
           "Vector3d::DistToLine([1, -1, 0], [1, 1, 0]) should equal 1")

    # ::operator/ vector3
    v.Set(4, 4, 4)
    v = v / Gz::Math::Vector3d.new(1, 2, 4)
    assert(v == Gz::Math::Vector3d.new(4, 2, 1),
           "v / Vector3d(1, 2, 4) should equal Vector3d(4, 2, 1)")

    # ::operator / double
    v = v / 2
    assert(v == Gz::Math::Vector3d.new(2, 1, 0.5),
           "v / 2 should equal Vector3d(2, 1, .5)")

    # ::operator * vector3
    v = v * Gz::Math::Vector3d.new(2, 3, 4)
    assert(v == Gz::Math::Vector3d.new(4, 3, 2),
           "v * Vector3d(2, 3, 4) should equal Vector3d(4, 3, 2)")

    v.Set(1.23, 2.35, 3.654321)
    v.Round(1)
    assert(v == Gz::Math::Vector3d.new(1.2, 2.4, 3.7))

    # operator GetAbs
    v.Set(-1, -2, -3)
    assert(v.Abs() == Gz::Math::Vector3d.new(1, 2, 3))

    # operator /=
    v.Set(1, 2, 4)
    v /= Gz::Math::Vector3d.new(1, 4, 4)
    assert(v == Gz::Math::Vector3d.new(1, 0.5, 1))

    # operator *=
    v.Set(1, 2, 4)
    v *= Gz::Math::Vector3d.new(2, 0.5, 0.1)
    assert(v.Equal(Gz::Math::Vector3d.new(2, 1, 0.4)))

    # Test the static defines.
    assert(Gz::Math::Vector3d.Zero ==
           Gz::Math::Vector3d.new(0, 0, 0),
           "Vector3d::Zero should equal [0, 0, 0]")

    assert(Gz::Math::Vector3d.One ==
           Gz::Math::Vector3d.new(1, 1, 1),
           "Vector3d::One should equal [1, 1, 1]")

    assert(Gz::Math::Vector3d.UnitX ==
           Gz::Math::Vector3d.new(1, 0, 0),
           "Vector3d::UnitX should equal [1, 0, 0]")

    assert(Gz::Math::Vector3d.UnitY ==
           Gz::Math::Vector3d.new(0, 1, 0),
           "Vector3d::UnitY should equal [0, 1, 0]")

    assert(Gz::Math::Vector3d.UnitZ ==
           Gz::Math::Vector3d.new(0, 0, 1),
           "Vector3d::UnitZ should equal [0, 0, 1]")
  end

  def test_distance
    vec1 = Gz::Math::Vector3d.new(0, 0, 0)
    vec2 = Gz::Math::Vector3d.new(1, 2, 3)
  ...
  def test_length
    # Zero vector
    assert(Gz::Math::Vector3d.Zero.Length() == 0.0,
           "Vector3 length of [0, 0, 0] should equal 0")
    assert(Gz::Math::Vector3d.Zero.SquaredLength() == 0.0,
           "Vector3 squared length of [0, 0, 0] should equal 0")

    # UnitXYZ vectorsGz::
    assert(Gz::Math::Vector3d.UnitX.Length() == 1.0,
           "Vector3 length of unitx should equal 1")
    assert(Gz::Math::Vector3d.UnitY.Length() == 1.0,
           "Vector3 length of unity should equal 1")
    assert(Gz::Math::Vector3d.UnitZ.Length() == 1.0,
           "Vector3 length of unitz should equal 1")
    assert(Gz::Math::Vector3d.UnitX.SquaredLength() == 1.0,
           "Vector3 squared length of unitx should equal 1")
    assert(Gz::Math::Vector3d.UnitY.SquaredLength() == 1.0,
           "Vector3 squared length of unity should equal 1")
    assert(Gz::Math::Vector3d.UnitZ.SquaredLength() == 1.0,
           "Vector3 squared length of unitz should equal 1")

    # One vector
    assert((Gz::Math::Vector3d.One.Length() -
  ...
  def test_normalize
    ...
    assert(vec1 == Gz::Math::Vector3d.Zero,
           "Vector3 should equal [0, 0, 0]")

    vec3 = vec2.Normalize()
    assert(vec3 == vec2, "Vector3 vec3 should equal vec2")
    assert(vec2 == Gz::Math::Vector3d.new(0.267261, 0.534522, 0.801784),
           "Vector3 vec2 should equal [0.267261, 0.534522, 0.801784]")

    vecConst = Gz::Math::Vector3d.new(1, 2, 3)
  ...
  def test_ge_normal
    vec1 = Gz::Math::Vector3d.new(0, 0, 0)
    vec2 = Gz::Math::Vector3d.new(0, 1, 0)
    vec3 = Gz::Math::Vector3d.new(1, 1, 0)

    norm = Gz::Math::Vector3d::Normal(vec1, vec2, vec3)
    assert(norm == Gz::Math::Vector3d.new(0, 0, -1),
           "Vector3 norm should equal [0, 0, -1]")
  end

  def test_perpendicular
    vec1 = Gz::Math::Vector3d.new(1, 1, 0)
    vec2 = Gz::Math::Vector3d.new(0, 1, 1)
    vec3 = Gz::Math::Vector3d.new(1e-7, 1e-7, 1e-7)
    vec4 = Gz::Math::Vector3d.new(1, 0, 0)

    assert(vec1.Perpendicular() == Gz::Math::Vector3d.new(0, 0, -1),
           "Vector3 vec1.Perpendicular() should equal [0, 0, -1]")
    assert(vec2.Perpendicular() == Gz::Math::Vector3d.new(0, 1, -1),
           "Vector3 vec2.Perpendicular() should equal [0, 1, -1]")
    assert(vec3.Perpendicular() == Gz::Math::Vector3d.new(0, 0, 0),
           "Vector3 vec3.Perpendicular() should equal [0, 0, 0]")
    assert(vec4.Perpendicular() == Gz::Math::Vector3d.new(0, 0, 1),
           "Vector3 vec4.Perpendicular() should equal [0, 0, 1]")
  end

  def test_max
    vec1 = Gz::Math::Vector3d.new(0.1, 0.2, 0.3)
    vec2 = Gz::Math::Vector3d.new(0.2, 0.3, 0.4)
    vec3 = Gz::Math::Vector3d.new(0.1, 0.2, 0.3)

    assert((vec1.Max() - 0.3).abs() < 1e-10,
           "Vector3 vec1.Max should equal 0.3")

    vec1.Max(vec2)
    assert(vec1 == Gz::Math::Vector3d.new(0.2, 0.3, 0.4),
  ...
  def test_min
    vec1 = Gz::Math::Vector3d.new(0.1, 0.2, 0.3)
    vec2 = Gz::Math::Vector3d.new(0.2, 0.3, 0.4)
    vec3 = Gz::Math::Vector3d.new(0.05, 0.1, 0.2)

    assert((vec1.Min() - 0.1).abs < 1e-10,
           "Vector3 vec1.Min should equal 0.1")

    vec1.Min(vec2)
    assert(vec1 == Gz::Math::Vector3d.new(0.1, 0.2, 0.3),
           "Vector3 vec1 should equal [0.1, 0.2, 0.3]")

    vec1.Min(vec3)
    assert(vec1 == Gz::Math::Vector3d.new(0.05, 0.1, 0.2),
           "Vector3 vec1 should equal [0.05, 0.1, 0.2]")
  end

  def test_max_abs
    vec1 = Gz::Math::Vector3d.new(0.1, 0.2, 0.05)

    assert((vec1.MaxAbs() - 0.2).abs() < 1e-10,
           "Vector3 vec1.MaxAbs should equal 0.2")

  end

  def test_min_abs
    vec1 = Gz::Math::Vector3d.new(-0.2, -0.1, -0.4)

    assert((vec1.MinAbs() - 0.1).abs < 1e-10,
           "Vector3 vec1.MinAbs should equal 0.1")

  end

  def test_add
    vec1 = Gz::Math::Vector3d.new(0.1, 0.2, 0.4)
    vec2 = Gz::Math::Vector3d.new(1.1, 2.2, 3.4)

    vec3 = vec1
    vec3 += vec2

    assert(vec1 + vec2 == Gz::Math::Vector3d.new(1.2, 2.4, 3.8),
           "Vector3 vec1 + vec2 should equal [1.2, 2.4, 3.8]")
    assert(vec3 == Gz::Math::Vector3d.new(1.2, 2.4, 3.8),
    ...
    assert(Gz::Math::Vector3d.Zero + vec1 == vec1,
           "Vector3 Zero + vec1 should equal vec1")
    assert(vec1 + Gz::Math::Vector3d.Zero == vec1,
    ...
    vec4 += Gz::Math::Vector3d.Zero
    assert(vec4 == vec1, "Vector3 vec4 should equal vec1")

    # Add non-trivial scalar values left and right
    assert(vec1 + 2.5 == Gz::Math::Vector3d.new(2.6, 2.7, 2.9),
           "Vector3 vec1 + 2.5 should equal [2.6, 2.7, 2.9]")

    vec1 = vec4
    vec4 += 2.5
    assert(vec4 == Gz::Math::Vector3d.new(2.6, 2.7, 2.9),
  ...
  def test_sub
    ...
    assert(vec2 - vec1 === Gz::Math::Vector3d.new(1.0, 2.0, 3.0),
           "Vector3 vec2 - vec1 should equal [1.0, 2.0 3.0]")
    assert(vec3 == Gz::Math::Vector3d.new(1.0, 2.0, 3.0),
    ...
    assert(vec1 - 2.5 == -Gz::Math::Vector3d.new(2.4, 2.3, 2.1),
           "Vecetor3 vec1 - 2.5 should equal [2.4, 2.3, 2.1]")

    vec4 = vec1
    vec4 -= 2.5
    assert(vec4 == -Gz::Math::Vector3d.new(2.4, 2.3, 2.1),
  ...
  def test_divide
    ...
    vec3 = vec1 / 2.0
    assert(vec3 == Gz::Math::Vector3d.new(0.05, 0.1, 0.2),
           "Vector3 vec3 should equal [0.05, 0.1, 0.2]")

    vec3 /= 4.0
    assert(vec3 == Gz::Math::Vector3d.new(0.0125, 0.025, 0.05),
           "Vector3 vec3 should qual [0.0125, 0.025, 0.05]")
  end

  def test_multiply
    v = Gz::Math::Vector3d.new(0.1, 0.2, 0.3)

    vec3 = v * 2.0
    assert(vec3 == Gz::Math::Vector3d.new(0.2, 0.4, 0.6),
           "Vector3 vec3 should equal[0.2, 0.4, 0.6]")

    vec3 *= 4.0
    assert(vec3 == Gz::Math::Vector3d.new(0.8, 1.6, 2.4),
           "Vector3 vec3 should equal [0.8, 1.6, 2.4]")

    # Multiply by zero

    # Scalar left and right
    assert(v * 0 == Gz::Math::Vector3d.Zero,
           "Vector3 v * 0 should equal Zero")

    # Element-wise vector multiplication
    assert(v * Gz::Math::Vector3d.Zero == Gz::Math::Vector3d.Zero,
    ...
    # Element-wise vector multiplication
    assert(v * Gz::Math::Vector3d.One == v,
           "Vector3 v * One should equal v")

    # Multiply by non-trivial scalar value

    scalar = 2.5
    expect = Gz::Math::Vector3d.new(0.25, 0.5, 0.75)
  ...
  def test_not_equal
    vec1 = Gz::Math::Vector3d.new(0.1, 0.2, 0.3)
    vec2 = Gz::Math::Vector3d.new(0.2, 0.2, 0.3)
    vec3 = Gz::Math::Vector3d.new(0.1, 0.2, 0.3)
  ...
  def test_nan
    ...
    nanVec.Correct()
    assert(Gz::Math::Vector3d.Zero == nanVec,
           "Corrected vector should equal zero")

    nanVecF = Gz::Math::Vector3f.NaN
    ...
    nanVecF.Correct()
    assert(Gz::Math::Vector3f.Zero == nanVecF,
           "Corrected vector should equal zero")
  end
end
