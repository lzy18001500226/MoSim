...
void Parameter_t::config_from_ros_handle(const ros::NodeHandle &nh)
{
	...
	nh.param("thrust_model/estimate_enable", thr_map.estimate_enable, true);
	...
	if ( !thr_map.estimate_enable )
	{
		ROS_WARN("Online thrust model estimation disabled; using fixed hover_percentage thrust mapping.");
	}
};
