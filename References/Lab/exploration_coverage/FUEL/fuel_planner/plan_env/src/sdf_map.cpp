...
void SDFMap::initMap(ros::NodeHandle& nh) {
  ...
  nh.param("sdf_map/diagnose_pointcloud_raycast", mp_->diagnose_pointcloud_raycast_, false);
  nh.param(
      "sdf_map/diagnose_pointcloud_raycast_throttle_s",
      mp_->diagnose_pointcloud_raycast_throttle_s_, 5.0);
...
void SDFMap::inputPointCloud(
  ...
  double length;
  int finite_points = 0;
  int nonfinite_points = 0;
  int occupied_endpoints = 0;
  int truncated_free_endpoints = 0;
  int out_of_map_free_endpoints = 0;
  int low_z_skipped = 0;
  int duplicate_rayend_skipped = 0;
  int unique_rayends = 0;
  int raycast_free_cells = 0;
  ...
  if (mp_->diagnose_pointcloud_raycast_) {
    ROS_WARN_THROTTLE(
        mp_->diagnose_pointcloud_raycast_throttle_s_,
        "[FUEL_RAY_DIAG] points=%d finite=%d nonfinite=%d occ_end=%d trunc_free_end=%d "
        "outmap_free_end=%d low_z_skip=%d dup_rayend=%d unique_rayend=%d free_cells=%d "
        "cam=(%.2f %.2f %.2f) update_min=(%.2f %.2f %.2f) update_max=(%.2f %.2f %.2f)",
        point_num, finite_points, nonfinite_points, occupied_endpoints, truncated_free_endpoints,
        out_of_map_free_endpoints, low_z_skipped, duplicate_rayend_skipped, unique_rayends,
        raycast_free_cells, camera_pos.x(), camera_pos.y(), camera_pos.z(), update_min.x(),
