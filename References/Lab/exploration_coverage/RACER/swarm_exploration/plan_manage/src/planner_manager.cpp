...
void FastPlannerManager::planExploreTraj(const vector<Eigen::Vector3d>& tour,
  ...
  if (tour.empty()) {
    ROS_ERROR("MoSim RACER guard: empty path to traj planner");
    return;
  }

  vector<Eigen::Vector3d> filtered_tour;
  filtered_tour.reserve(tour.size());
  constexpr double kMinWaypointSeparation = 1e-3;
  for (const auto& pt : tour) {
    if (!pt.allFinite()) {
      ROS_ERROR_STREAM("MoSim RACER guard: non-finite waypoint in exploration tour: "
                       << pt.transpose());
      return;
    }
    if (filtered_tour.empty() ||
        (pt - filtered_tour.back()).norm() > kMinWaypointSeparation) {
      filtered_tour.push_back(pt);
    }
  }

  if (filtered_tour.size() < 2) {
    ROS_WARN_STREAM("MoSim RACER guard: degenerate exploration tour after filtering, raw_size="
                    << tour.size() << ", filtered_size=" << filtered_tour.size());
    return;
  }

  // PolynomialTraj::waypointsTraj expects at least three waypoints. This mirrors
  // planGlobalTraj's two-point midpoint insertion and prevents Eigen index
  // assertions during short RACER replans.
  if (filtered_tour.size() == 2) {
    const Eigen::Vector3d mid = 0.5 * (filtered_tour[0] + filtered_tour[1]);
    filtered_tour.insert(filtered_tour.begin() + 1, mid);
  }

  // Generate traj through waypoints-based method
  const int pt_num = filtered_tour.size();
  Eigen::MatrixXd pos(pt_num, 3);
  for (int i = 0; i < pt_num; ++i) pos.row(i) = filtered_tour[i];

  Eigen::Vector3d zero(0, 0, 0);
  Eigen::VectorXd times(pt_num - 1);
  if (pp_.max_vel_ <= 1e-3) {
    ROS_ERROR_STREAM("MoSim RACER guard: invalid max velocity for exploration traj: "
                     << pp_.max_vel_);
    return;
  }
  double min_seg_len = std::numeric_limits<double>::infinity();
  double max_seg_len = 0.0;
  double total_len = 0.0;
  for (int i = 0; i < pt_num - 1; ++i) {
    const double seg_len = (pos.row(i + 1) - pos.row(i)).norm();
    min_seg_len = std::min(min_seg_len, seg_len);
    max_seg_len = std::max(max_seg_len, seg_len);
    total_len += seg_len;
    times(i) = seg_len / (pp_.max_vel_ * 0.5);
    if (!std::isfinite(times(i)) || times(i) <= 1e-4) {
      ROS_WARN_STREAM("MoSim RACER guard: invalid exploration traj segment, pt_num="
                      << pt_num << ", raw_size=" << tour.size()
                      << ", filtered_size=" << filtered_tour.size()
                      << ", index=" << i << ", seg_len=" << seg_len
                      << ", time=" << times(i));
      return;
    }
  }
  ROS_INFO_STREAM_THROTTLE(1.0, "MoSim RACER exploration traj input: raw_size="
                                 << tour.size() << ", filtered_size="
                                 << filtered_tour.size() << ", total_len="
                                 << total_len << ", min_seg=" << min_seg_len
                                 << ", max_seg=" << max_seg_len
                                 << ", time_sum=" << times.sum());
  ...
  if (!std::isfinite(duration) || duration <= 1e-4) {
    ROS_WARN_STREAM("MoSim RACER guard: invalid exploration initial trajectory duration="
                    << duration << ", pt_num=" << pt_num);
    return;
  }
  ...
  if (!std::isfinite(dt) || dt <= 1e-4) {
    ROS_WARN_STREAM("MoSim RACER guard: invalid exploration bspline dt="
                    << dt << ", duration=" << duration << ", seg_num=" << seg_num);
    return;
  }
