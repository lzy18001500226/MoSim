...
struct sch_node{
    ...
    sch_node(shared_ptr<sch_node> &node){
        ...
        // m_g_score_ = node->m_g_score_;
        motion_state_ = false;
    }
    Eigen::Vector3i pos_;
    shared_ptr<sch_node> parent_, h_parent_;
    SchState status_, h_status_;
    uint8_t motion_state_; // 0: not motion, 1: motion mid point, 2: motion end
    int m_id_;
    int me_id_;
    double f_score_;
    double g_score_, h_g_score_;//, m_g_score_;
};
...
struct LR_node{
    ...
    bitset<8> flags_;  //0000 0(new)(local)(Xnode)
};
...
public:
    ...
    /**
     * @brief check if a movement is feasible
     * 
     * @param ps start pos
     * @param m (-1 0 1)(-1 0 1)(-1 0 1)
     * @return true 
     * @return false 
     */
    inline bool FeasibleMove(const Eigen::Vector3i &ps, const Eigen::Vector3i &m);
     ...
     * @param occ_list 
     */
    bool UpdateLocalTopo(const Eigen::Matrix4d rob_pose, vector<Eigen::Vector3d> &occ_list);
     ...
     * @return false 
     */
    bool PathCheck(list<Eigen::Vector3d> &path);
    ...
    inline bool IsFeasible(const Eigen::VectorXd &pos);

    inline bool IsFeasible(const Eigen::Vector3d &pos);

    inline bool IsFeasible(const Eigen::Vector3i &pos); //dont check

    inline int DebugFeasibilityReason(const Eigen::Vector3d &pos);

    inline NodeVoxelStats DebugNodeVoxelStats(const Eigen::Vector3d &pos);

    inline bool IsLocalFeasible(const Eigen::Vector3d &pos);

    inline bool FeasibleLine(const Eigen::Vector3d &p1, const Eigen::Vector3d &p2);

    inline void GetBound(const Eigen::Vector3d &p, Eigen::MatrixXd &bound);
     ...
     */
    bool DjkstraLocalDist(const Eigen::Vector3d &pos, list<Eigen::Vector3d> &p_l, list<double> &d_l);
    ...
    inline int Id3toId(const Eigen::Vector3i &id3);             //dont check
    ...
    void Debug(Eigen::Vector3d up, Eigen::Vector3d down);
    ...
    list<pair<pair<int, Eigen::Vector3d>, pair<int, list<Eigen::Vector3d>>>> frontier_path_;//<f_id, v_id, path>, frontier id and corresponding path to the viewpoint
...
inline void LowResMap::GetCastLine(const Eigen::Vector3d &start, const Eigen::Vector3d &end, list<Eigen::Vector3d> &line){
    // cout<<"FeasibleLine0"<<endl;
    // cout<<p1.transpose()<<"   "<<p2.transpose()<<endl;
    // cout<<map_lowbd_.transpose()<<"   "<<map_upbd_.transpose()<<endl;
    // cout<<"FeasibleLine1.0"<<endl;
    line.clear();
    RayCaster rc;
    Eigen::Vector3d inv_res, ray_iter;
    Eigen::Vector3d half_res = 0.5 * node_scale_;
    // cout<<"FeasibleLine1"<<endl;
    for(int dim = 0; dim < 3; dim++) inv_res(dim) = 1.0 / node_scale_(dim);
    rc.setInput((start - origin_).cwiseProduct(inv_res), (end - origin_).cwiseProduct(inv_res));
    while(rc.step(ray_iter)){
        ray_iter = ray_iter.cwiseProduct(node_scale_) + origin_ + half_res;
        line.emplace_back(ray_iter);
    }
    // cout<<"FeasibleLine2"<<endl;
    ray_iter = ray_iter.cwiseProduct(node_scale_) + origin_ + half_res;
    line.emplace_back(ray_iter);
    // if(!IsFeasible(ray_iter)) return false;
    // return true;
}

inline bool LowResMap::FeasibleMove(const Eigen::Vector3i &ps, const Eigen::Vector3i &m){
    for(int dim = 0; dim < 3; dim++) if(abs(m(dim)) > 1) return false; 
    int move_count = 0;
    if(m(0) != 0) move_count++;
    if(m(1) != 0) move_count++;
    if(m(2) != 0) move_count++;
    ...
    if(move_count == 0) return true;
        ...
        Eigen::Vector3i pe = ps + m;
        return IsFeasible(pe);
    }
    else if(move_count == 2){
        Eigen::Vector3i pe = ps + m;
        if(!IsFeasible(pe)) return false;
        for(int dim = 0; dim < 3; dim++) {
            if(m(dim) != 0){
                Eigen::Vector3i p = ps;
                p(dim) += m(dim);
                if(IsFeasible(p)) return true;
            }
        } 
        return false;
    }
        ...
        if(IsFeasible(p)){
            ...
            p = pe;
            p(1) = pe(1) - m(1);
            if(IsFeasible(p)) m2[1] = true;
            p = pe;
            p(2) = pe(2) - m(2);
            if(IsFeasible(p)) m2[2] = true;
        }

        p = ps;
        p(1) = ps(1) + m(1);
        if(IsFeasible(p)){
            if(m2[2]) return true; //y-->x-->z
            p = pe;
            p(0) = pe(0) - m(0);    //y-->z-->x
            if(IsFeasible(p)) return true;
        }
            ...
            p = pe;
            p(0) = pe(0) - m(0);    
            if(IsFeasible(p)) m2[0] = true;
        }

        p = ps;
        p(2) = ps(2) + m(2);
        if(IsFeasible(p)){
            if(m2[0]) return true; //z-->y-->x

            if(m2[1]) return true; //z-->x-->y
...
inline bool LowResMap::IsFeasible(const Eigen::VectorXd &pos){
...
inline bool LowResMap::IsFeasible(const Eigen::Vector3d &pos){
...
inline bool LowResMap::IsFeasible(const Eigen::Vector3i &pos){
...
inline int LowResMap::DebugFeasibilityReason(const Eigen::Vector3d &pos){
    if(!InsideMap(pos)) return 1;
    int blockid = GetBlockId(pos);
    if(blockid == -1) return 1;
    shared_ptr<LR_block> fp_ptr = gridBLK_[blockid];
    if(fp_ptr == NULL) return 2;
    shared_ptr<LR_node> node = GetNode(pos, fp_ptr);
    if(node == NULL) return 3;
    if(node == Outnode_) return 4;
    if(node->flags_[0]) return 5;
    return 0;
}

inline NodeVoxelStats LowResMap::DebugNodeVoxelStats(const Eigen::Vector3d &pos){
    NodeVoxelStats stats;
    if(!InsideMap(pos)) return stats;

    Eigen::Vector3i node_idx;
    PostoId3(pos, node_idx);
    Eigen::Vector3d startpos = node_idx.cast<double>().cwiseProduct(node_scale_) + origin_;
    ...
    for(int x = 0; x < node_size_(0); x++){
        for(int y = 0; y < node_size_(1); y++){
            for(int z = 0; z < node_size_(2); z++){
                ...
                stats.total++;
                if(cstatus == VoxelState::free) stats.free++;
                else if(cstatus == VoxelState::unknown) stats.unknown++;
                else if(cstatus == VoxelState::occupied) stats.occupied++;
                else if(cstatus == VoxelState::out) stats.out++;
            }
        }
    }
    return stats;
...
inline shared_ptr<LR_node> LowResMap::GetNode(const Eigen::Vector3d &pos, const shared_ptr<LR_block> &FG){
    // if(FG->local_grid_.size() <= GetNodeId(pos, FG)){
    //     ROS_ERROR("error GetNode %ld, %d", FG->local_grid_.size(), GetNodeId(pos, FG));
    ...
    //     cout<<"FG->block_size_:"<<FG->block_size_.transpose()<<endl;
    //     cout<<"dpos:"<<dpos.transpose()<<endl;
    //     cout<<"posid:"<<posid.transpose()<<endl;
    // }
...
inline int LowResMap::Id3toId(const Eigen::Vector3i &id3){
    return id3(2)*v_n_(1)+id3(1)*voxel_num_(0)+id3(0);
}
...
inline pair<int,int> LowResMap::SetNode(const Eigen::Vector3d &pos, bool get_h){
    ...
    if(GetBlock3Id(pos, blockid)){
        ...
        // if(blkid > gridBLK_.size()){
        //     ROS_WARN("SetNode error1");
        // }
        ...
        // if(nodeid > gridBLK_[blkid]->local_grid_.size()){
        //     ROS_WARN("SetNode error2");
        // }
        ...
        gridBLK_[blkid]->local_grid_[nodeid]->flags_.reset(0);
...
inline bool LowResMap::FeasibleLine(const Eigen::Vector3d &p1, const Eigen::Vector3d &p2){
    // cout<<"FeasibleLine0"<<endl;
    // cout<<p1.transpose()<<"   "<<p2.transpose()<<endl;
    // cout<<map_lowbd_.transpose()<<"   "<<map_upbd_.transpose()<<endl;
    if(!InsideMap(p1) || !InsideMap(p2)) {
        // cout<<"FeasibleLine0.9"<<endl;
        return false;
    }
    // cout<<"FeasibleLine1.0"<<endl;
    RayCaster rc;
    ...
    // cout<<"FeasibleLine1"<<endl;
    for(int dim = 0; dim < 3; dim++) inv_res(dim) = 1.0 / node_scale_(dim);
    rc.setInput((p1 - origin_).cwiseProduct(inv_res), (p2 - origin_).cwiseProduct(inv_res));
    while(rc.step(ray_iter)){
        ray_iter = ray_iter.cwiseProduct(node_scale_) + origin_ + half_res;
        if(!IsFeasible(ray_iter)) return false;
    }
...
inline void LowResMap::GetBound(const Eigen::Vector3d &p, Eigen::MatrixXd &bound){
    bound.resize(3, 2);
    for(int dim = 0; dim < 3; dim++){
        bound(dim, 1) = floor((p(dim) - origin_(dim)) / node_scale_(dim)) * node_scale_(dim) + origin_(dim) - p(dim);
        bound(dim, 0) = node_scale_(dim) + bound(dim, 1);
    }
}
...
inline int LowResMap::CheckNode(const Eigen::Vector3i pos){
    ...
    for(int x = 0; x < node_size_(0); x++)
        ...
        chk_pos = startpos+Eigen::Vector3d(x,y,z)*resolution_;
        VoxelState cstatus = map_->GetVoxState(chk_pos);
        if(cstatus == VoxelState::occupied || cstatus == VoxelState::out){
            return 1;
        }
