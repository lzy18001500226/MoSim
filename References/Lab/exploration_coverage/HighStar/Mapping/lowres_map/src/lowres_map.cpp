...
void LowResMap::UpdateLocalBBX(const Eigen::Matrix4d rob_pose, const pair<Eigen::Vector3d, Eigen::Vector3d> bbx){
    ...
    PruneBlock();
    // MoSim MID360 pseudo-depth may leave the body cell unknown before the
    // topology expansion path runs; the FSM checks this cell first in AllowPlan.
    // Seed after PruneBlock because Xlist cleanup can delete cells that were
    // first marked as X and then converted back to free in the same update.
    for(int dx = -1; dx <= 1; dx++){
        for(int dy = -1; dy <= 1; dy++){
            for(int dz = -1; dz <= 1; dz++){
                Eigen::Vector3d seed_pos = Robot_pos_ + Eigen::Vector3d(
                    dx * node_scale_(0),
                    dy * node_scale_(1),
                    dz * node_scale_(2));
                if(!InsideMap(seed_pos)) continue;
                if(GetNode(seed_pos) == NULL) SetEXPNode(seed_pos);
                if(GetNode(seed_pos) != Outnode_) SetNode(seed_pos);
            }
        }
    }
    if(InsideMap(Robot_pos_)){
        if(!IsFeasible(Robot_pos_)){
            shared_ptr<LR_node> robot_node = GetNode(Robot_pos_);
            if(robot_node == NULL){
                ROS_WARN_THROTTLE(
                    2.0,
                    "HighStar LowResMap robot cell still infeasible after seed: null p=(%.3f, %.3f, %.3f)",
                    Robot_pos_(0), Robot_pos_(1), Robot_pos_(2));
            }
            else if(robot_node == Outnode_){
                ROS_WARN_THROTTLE(
                    2.0,
                    "HighStar LowResMap robot cell still infeasible after seed: outnode p=(%.3f, %.3f, %.3f)",
                    Robot_pos_(0), Robot_pos_(1), Robot_pos_(2));
            }
            else{
                std::string flags = robot_node->flags_.to_string();
                ROS_WARN_THROTTLE(
                    2.0,
                    "HighStar LowResMap robot cell still infeasible after seed: flags=%s p=(%.3f, %.3f, %.3f)",
                    flags.c_str(), Robot_pos_(0), Robot_pos_(1), Robot_pos_(2));
            }
        }
    }
    // Debug(debug_list);
...
bool LowResMap::UpdateLocalTopo(const Eigen::Matrix4d rob_pose, vector<Eigen::Vector3d> &occ_list){
    mtx_.lock();
    Robot_pose_ = rob_pose;
...
void LowResMap::PruneTopoBlock(){
    ...
    for(auto &i_v : frontier_list_){
        ...
        if(length < 99998.0){
            ...
            frontier_path_.push_back({{i_v.first, debug_pos}, {best_v_id ,path}});
        }
...
bool LowResMap::PathCheck(list<Eigen::Vector3d> &path){
    RayCaster rc;
...
bool LowResMap::FindCorridors(const vector<Eigen::Vector3d> path, 
    ...
    if(path_cast.size() == 1){
        path_cast.emplace_back(GetStdPos(path.back()));
    }
    ...
    while(1){
        ...
        if(!success || cur_length >= seg_length_ || end_idx + 1 >= path_cast.size() || start_idx == 0){
            ...
            p.setZero();
            for(int dim = 0; dim < 3; dim++){
                up_corner(dim) -= Robot_size_(dim) * 0.51;
                down_corner(dim) += Robot_size_(dim) * 0.51;
...
void LowResMap::ShowGridLocal(const ros::TimerEvent& e){
    ...
    if(Showblocklist_.size() > 0 && node_pub_.getNumSubscribers() > 0){
        ...
        for(list<int>::iterator idit = Showblocklist_.begin(); idit != Showblocklist_.end(); idit++){
            ...
            MKArray.markers[i].type = visualization_msgs::Marker::POINTS;        //Xnodes
        ...
        int nodeid;
        std_msgs::ColorRGBA localcolor, gobalcolor, Xcolor, Ecolor;
        Ecolor.a = 1.0;
        Ecolor.g = 1.0;
                    ...
                    for(iterp(1) = 0; iterp(1) < gridBLK_[*idit]->block_size_(1); iterp(1)++){
                        for(iterp(2) = 0; iterp(2) < gridBLK_[*idit]->block_size_(2); iterp(2)++){
                            ...
                            if(gridBLK_[*idit]->local_grid_[nodeid] != NULL){
                                ...
                                else if(gridBLK_[*idit]->local_grid_[nodeid]->flags_[1]){
                                    MKArray.markers[i].colors.push_back(localcolor);
                                    MKArray.markers[i].points.push_back(pt);
                                }
                                else{
                                    MKArray.markers[i].colors.push_back(gobalcolor);
                                    MKArray.markers[i].points.push_back(pt);
                                }
                                i++;
                                if(show_dtg_){
                                    ...
                                    for(auto &tie : gridBLK_[*idit]->local_grid_[nodeid]->ties_){
                                        pt.y += resolution_ * 2;
                                        MKArray.markers[i].colors.push_back(CM_->Id2Color(int(tie.root_id_) / 256, 0.5));
                                        MKArray.markers[i].points.push_back(pt);
                                    }
                                }
                                i--;
...
void LowResMap::Debug(Eigen::Vector3d up, Eigen::Vector3d down){
    visualization_msgs::Marker mk;
    mk.action = visualization_msgs::Marker::ADD;
    mk.pose.orientation.w = 1.0;
    mk.type = visualization_msgs::Marker::CUBE;      //nodes
    mk.scale.x = up(0) - down(0);
    mk.scale.y = up(1) - down(1);
    mk.scale.z = up(2) - down(2);
    mk.pose.position.x = (up(0) + down(0)) / 2;
    mk.pose.position.y = (up(1) + down(1)) / 2;
    mk.pose.position.z = (up(2) + down(2)) / 2;
    mk.header.frame_id = "world";
    mk.header.stamp = ros::Time::now();
    mk.id = 0;
    mk.color.a = 0.7;
