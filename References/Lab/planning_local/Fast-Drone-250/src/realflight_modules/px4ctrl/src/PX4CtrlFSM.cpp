...
void PX4CtrlFSM::process()
{
	...
	case MANUAL_CTRL:
	{
		...
		else if (param.takeoff_land.enable && takeoff_land_data.triggered && takeoff_land_data.takeoff_land_cmd == quadrotor_msgs::TakeoffLand::TAKEOFF) // Try to jump to AUTO_TAKEOFF
		{
			...
			takeoff_land.last_auto_arm_retry_time = now_time;
			takeoff_land.auto_arm_retry_count = 0;
			takeoff_land.auto_arm_retry_exhausted_reported = false;
			bool offboard_request_sent = toggle_offboard_mode(true); // toggle on offboard before arm
			ros::WallTime offboard_wait_start = ros::WallTime::now();
			while (offboard_request_sent && ros::ok() && state_data.current_state.mode != "OFFBOARD" &&
				   (ros::WallTime::now() - offboard_wait_start).toSec() < 2.0)
			{
				ros::WallDuration(0.02).sleep();
				ros::spinOnce();
			}
			if (offboard_request_sent && state_data.current_state.mode != "OFFBOARD")
			{
				ROS_WARN("[px4ctrl] OFFBOARD mode not observed before arm; current PX4 mode=%s",
						 state_data.current_state.mode.c_str());
			}
			if (param.takeoff_land.enable_auto_arm)
			{
				if (!offboard_request_sent)
				{
					ROS_ERROR("[px4ctrl] Skip ARM because OFFBOARD request was rejected by PX4.");
				}
				else
				{
					bool armed_ok = false;
					for (int i = 0; i < 3 && ros::ok() && !armed_ok; ++i)
					{
						armed_ok = toggle_arm_disarm(true);
						++takeoff_land.auto_arm_retry_count;
						if (!armed_ok)
						{
					...
					if (!armed_ok)
					{
						ROS_ERROR("[px4ctrl] ARM failed after bounded retries; AUTO_TAKEOFF will not lift until PX4 accepts arming.");
					}
				}
			}
	...
	case AUTO_HOVER:
	{
		...
		{
			ROS_WARN("[px4ctrl][fsm_diag] AUTO_HOVER exit: rc_hover=%d rc_command=%d odom_received=%d cmd_received=%d rc_age=%.3f odom_age=%.3f cmd_age=%.3f timeout_rc=%.3f timeout_odom=%.3f timeout_cmd=%.3f px4_mode=%s armed=%d",
					 rc_data.is_hover_mode,
					 rc_data.is_command_mode,
					 odom_is_received(now_time),
					 cmd_is_received(now_time),
					 (now_time - rc_data.rcv_stamp).toSec(),
					 (now_time - odom_data.rcv_stamp).toSec(),
					 (now_time - cmd_data.rcv_stamp).toSec(),
					 param.msg_timeout.rc,
					 param.msg_timeout.odom,
	...
	case CMD_CTRL:
	{
		if (!rc_data.is_hover_mode || !odom_is_received(now_time))
		{
			ROS_WARN("[px4ctrl][fsm_diag] CMD_CTRL exit: rc_hover=%d rc_command=%d odom_received=%d cmd_received=%d rc_age=%.3f odom_age=%.3f cmd_age=%.3f timeout_rc=%.3f timeout_odom=%.3f timeout_cmd=%.3f px4_mode=%s armed=%d",
					 rc_data.is_hover_mode,
					 rc_data.is_command_mode,
					 odom_is_received(now_time),
					 cmd_is_received(now_time),
		...
		{
			ROS_INFO("[px4ctrl][fsm_diag] CMD_CTRL hover fallback: rc_hover=%d rc_command=%d odom_received=%d cmd_received=%d rc_age=%.3f odom_age=%.3f cmd_age=%.3f timeout_rc=%.3f timeout_odom=%.3f timeout_cmd=%.3f px4_mode=%s armed=%d",
					 rc_data.is_hover_mode,
					 rc_data.is_command_mode,
					 odom_is_received(now_time),
					 cmd_is_received(now_time),
	...
	case AUTO_TAKEOFF:
	{
		if (param.takeoff_land.enable_auto_arm && !state_data.current_state.armed)
		{
			...
			else if ((now_time - takeoff_land.last_auto_arm_retry_time).toSec() >= AutoTakeoffLand_t::AUTO_ARM_RETRY_INTERVAL)
			{
				if (takeoff_land.auto_arm_retry_count < AutoTakeoffLand_t::AUTO_ARM_RETRY_MAX)
				{
					takeoff_land.last_auto_arm_retry_time = now_time;
					++takeoff_land.auto_arm_retry_count;
					if (toggle_arm_disarm(true))
					{
						ROS_INFO("[px4ctrl] AUTO_TAKEOFF arm retry accepted by PX4 after %d attempts.",
								 takeoff_land.auto_arm_retry_count);
					}
				}
				else if (!takeoff_land.auto_arm_retry_exhausted_reported)
				{
					ROS_ERROR("[px4ctrl] AUTO_TAKEOFF arm retry exhausted; PX4 health gate is still rejecting arming.");
					takeoff_land.auto_arm_retry_exhausted_reported = true;
				}
			}
		}
	...
	case AUTO_LAND:
	{
		if (!rc_data.is_hover_mode || !odom_is_received(now_time))
		{
			ROS_WARN("[px4ctrl][fsm_diag] AUTO_LAND exit: rc_hover=%d rc_command=%d odom_received=%d cmd_received=%d rc_age=%.3f odom_age=%.3f cmd_age=%.3f timeout_rc=%.3f timeout_odom=%.3f timeout_cmd=%.3f px4_mode=%s armed=%d",
					 rc_data.is_hover_mode,
					 rc_data.is_command_mode,
					 odom_is_received(now_time),
					 cmd_is_received(now_time),
	...
	if (param.thr_map.estimate_enable && (state == AUTO_HOVER || state == CMD_CTRL))
	{
	...
	else if (controller.usingMosimWrapperOwnedLandingCore() && state == AUTO_LAND)
	{
		// AUTO_LAND is wrapper-owned safety logic in the MoSim Golden Slice.
		// Keep the upstream descent law here and use the generated core for
		// hover / trajectory control phases that define px4ctrl_core.
		debug_msg = controller.calculateOriginalControl(des, odom_data, imu_data, u);
