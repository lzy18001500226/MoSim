...
void main() 
{
	float age_life = age/life;
	vec4 DiffuseColor = texture2DArray(particleSamplers, vec3(TexCoord, age_life*(texArraySize-1)));

    DiffuseColor.a *= transparency;

	/// not work ?
	//if( age_life < 0.01 ) DiffuseColor.a *= (1.0 - (0.01 - age_life)*100);
    
    DiffuseColor.rgb *= Color * exposure;
	...
	transmittance_light = min(transmittance_light*3 + 0.3, 1.0);
	
	float transmittance = CalcTransmittance(gFourierA, gFourierB, ivec2(gl_FragCoord.xy), depth);

	gFragColor = vec4(transmittance * DiffuseColor.xyz * DiffuseColor.a / (1.0 - DiffuseColor.a) * transmittance_light, 
	              transmittance * DiffuseColor.a / (1.0 - DiffuseColor.a));
}
