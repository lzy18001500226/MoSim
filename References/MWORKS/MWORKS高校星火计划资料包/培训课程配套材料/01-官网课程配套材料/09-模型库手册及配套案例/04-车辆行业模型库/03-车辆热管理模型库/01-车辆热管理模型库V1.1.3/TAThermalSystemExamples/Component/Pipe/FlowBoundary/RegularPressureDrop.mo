model RegularPressureDrop "RegularPressureDrop压降方式"
  parameter Modelica.Units.SI.MassFlowRate m_flow_in = 0.2 "输入流量";
  parameter Modelica.Units.SI.Temperature T_heatin = 313.15 "壁面边界温度";
  ...
  TAThermalSystem.Pipes.LiquidCoolingPipe.CoolingPipe coolingPipeBase2(
    useHeatTransfer = true, redeclare model Friction = TAThermalSystem.Utilities.PressureDrop.RegularPressureDrop, redeclare Integer calType = 2, h_conv_set = 75,
    usePipeCompressibility = true, thick(displayUnit
       = "mm"),
    CF_PressureLoss = 1,
    fromDp = false,
    rr = 1e-05) 

    annotation (Placement(transformation(origin = {-0.6808510638297873, -1.0212765957446805},
  ...
  annotation (Icon(coordinateSystem(extent = {{-100.0, -100.0}, {100.0, 100.0}},
    ...
    Documentation(link = "modelica://TAThermalSystem/Resource/Doc/FlowBoundaryRegularPressureDrop.html"),__MWORKS(ResultViewerManager(resultViewers={
ResultViewer(name="1", executeTrigger=executeTrigger.SimulationStarted, commands={
CreatePlot(id=1, x_display_unit="s", legend_layout=7, left_title="[bar]", fix_time_range_value=0, zoom_x=(0, 10), zoom_y_l=(4.9998, 5.0014)),
...
equation
  ...
  connect(fixedTemperature.port[1], coolingPipeBase2.qa) 
    annotation (Line(origin = {-6.0, 20.0},
      points = {{-6.0, 19.0}, {5.0, 19.0}, {5.0, -12.0}},
      color = {191, 0, 0},
      thickness = 1.0));
