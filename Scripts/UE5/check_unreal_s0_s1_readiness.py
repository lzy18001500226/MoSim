...
def main() -> int:
    ...
    checks = [
        run_step("python syntax: stream/check/probe", [
            ...
            (
                "from pathlib import Path\n"
                "for rel in ['Scripts/UE5/stream_unreal_udp.py','Scripts/UE5/check_unreal_bridge.py',"
                "'Scripts/UE5/probe_unreal_mcp_listener.py']:\n"
                "    compile(Path(rel).read_text(encoding='utf-8'), rel, 'exec')\n"
                "print('[OK] syntax bundle')\n"
            ),
        ]),
        run_step("unreal bridge source contract", [sys.executable, "Scripts/UE5/check_unreal_bridge.py"]),
    ...
    if args.build:
        checks.append(run_step("UE renderer build", ["bash", "Scripts/UE5/build_unreal_renderer.sh"]))
