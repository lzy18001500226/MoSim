...
def battery_heatshrink() -> dict[str, str]:
    ...
    label_core = label_band & (y > 0.28) & (y < 0.76)
    warning_tab = label_band & (y > 0.38) & (y < 0.48)
    base[label_core] = np.array([185, 180, 150]) + noise[label_core, None] * 16
    base[warning_tab] = np.array([185, 34, 24]) + noise[warning_tab, None] * 10
...
def pcb() -> dict[str, str]:
    ...
    for _ in range(120):
        ...
        draw.line((int(x0), int(y0), x1, y1), fill=(150, 125, 55), width=int(rng.integers(1, 4)))
    for _ in range(80):
    ...
    for _ in range(45):
        x0, y0 = rng.integers(0, SIZE - 18, 2)
        w, h = rng.integers(5, 18), rng.integers(4, 14)
        fill = tuple(map(int, rng.choice([(180, 135, 42), (210, 170, 62), (235, 205, 100)], 1)[0]))
        draw.rectangle((int(x0), int(y0), int(x0 + w), int(y0 + h)), fill=fill)
    return {
...
def main() -> None:
    manifest = {
        ...
        "nickel_connector": brushed_metal("sunray150_nickel_connector", (72, 72, 68), 23),
        ...
        "smoked_guard": rubber("sunray150_smoked_translucent_guard", (42, 50, 52), 32),
