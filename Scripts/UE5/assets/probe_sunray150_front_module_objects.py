#!/usr/bin/env python3
"""List visible mesh objects around the front camera/module area."""
...
OUT = PROJECT_ROOT / "Results" / "unreal_scene_mapping" / "sunray150_front_module_objects_20260603.json"
...
def main() -> None:
    ...
    for obj in bpy.context.scene.objects:
        ...
        center = (mn + mx) * 0.5
        size = mx - mn
        if -0.055 <= center.x <= 0.055 and 0.055 <= center.y <= 0.125 and -0.015 <= center.z <= 0.060:
            rows.append(
                {
                    "name": obj.name,
