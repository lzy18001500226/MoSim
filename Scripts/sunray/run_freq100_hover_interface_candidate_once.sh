...
POSITION_HOLD_MODE="${1:-px4xyzpos}"
CONTROL_MODE="${2:-ctrltraj}"

STAMP=$(date +%Y%m%d_%H%M%S)
RESULT_DIR="/mnt/c/Users/HP/Desktop/MoSim/Results/sunray_ros1/sunray_ros1_freq100_hover_interface_${POSITION_HOLD_MODE}_${CONTROL_MODE}_${STAMP}"
mkdir -p "${RESULT_DIR}"
...
export MISSION_NODE_ARGS="--position-hold-mode ${POSITION_HOLD_MODE} --control-mode ${CONTROL_MODE} --enable-hover-bias-calibration --hover-bias-calibration-settle-s 8.0 --hover-bias-calibration-tail-s 4.0 --hover-bias-calibration-verify-s 3.0 --hover-bias-calibration-gain 0.6 --hover-bias-calibration-max-step-m 0.04 --steady-hover-eval-tail-s 8.0"

{
  echo "RESULT_DIR=${RESULT_DIR}"
  echo "POSITION_HOLD_MODE=${POSITION_HOLD_MODE}"
