#!/usr/bin/env python3
"""Fail-closed provenance check for the classic-controller px4ctrl backend."""
...
SCHEMA = "mosim.classic_controller.generated_runtime_provenance.v1"
MODEL = "MoSim_Classic_CFunction_Sysblock"
BACKEND = "classic_controller_attitude_thrust"
BACKEND_DEFINITION = "MOSIM_PX4CTRL_GENERATED_BACKEND_CLASSIC_CONTROLLER_ATTITUDE_THRUST"
GENERATED_DIR_CACHE_VAR = "MOSIM_PX4CTRL_CLASSIC_CONTROLLER_ATTITUDE_THRUST_GENERATED_DIR"
RUNTIME_SYMBOL = f"{MODEL}::Step"
BINARY_SYMBOL = "MosimClassicStepScalar"
...
def build_payload(args: argparse.Namespace) -> dict[str, object]:
    ...
    codegen = load_json(args.codegen_manifest, "MWORKS codegen manifest", errors)
    sil = load_json(args.sil_report, "generated-C SIL report", errors)
    if codegen.get("status") != "passed" or codegen.get("model_name") != MODEL:
        errors.append("MWORKS codegen manifest is missing, failed, or names another model")
    if codegen.get("generated_file_count") != 12:
        errors.append("MWORKS codegen manifest does not declare the 12-file archive")
    if sil.get("status") != "passed" or sil.get("controllers") != list(CONTROLLERS):
        errors.append("generated-C SIL report is missing, failed, or has a controller mismatch")
    if sil.get("total_scalar_comparisons") != 700 or sil.get("max_abs_difference") != 0.0:
        errors.append("generated-C SIL report is not the accepted 700-comparison zero-difference gate")
