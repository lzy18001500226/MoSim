#!/usr/bin/env bash
# Goal 2 gate: FAST-LIO aligned odom -> Sunray external_fusion.
...
RUN_ID="${RUN_ID:-sunray_ros1_fastlio_goal2_$(date +%Y%m%d_%H%M%S)}"
...
EXTERNAL_FUSION_READY_TIMEOUT_S="${EXTERNAL_FUSION_READY_TIMEOUT_S:-30}"
GOAL2_RECORD_DURATION_S="${GOAL2_RECORD_DURATION_S:-25}"
...
cleanup() {
  ...
  pkill -f "mosim_fastlio_external_fusion_goal2_recorder" >/dev/null 2>&1 || true
  pkill -f "external_fusion.launch" >/dev/null 2>&1 || true
...
source_env
{
  echo "GOAL2_FASTLIO_EXTERNAL_FUSION_GATE"
  ...
  echo "external_source=0"
  echo "position_topic=/mosim/fastlio/odom_aligned"
  echo "use_vision_pose=true"
...
rospy.init_node("mosim_goal2_wait_mavros_connected", anonymous=True)
...
PY
then
  echo "MAVROS did not connect for Goal 2 external-fusion gate" >&2
  exit 4
fi
...
roslaunch sunray_uav_control external_fusion.launch \
  uav_id:=1 uav_name:=uav external_source:=0 \
  position_topic:=/mosim/fastlio/odom_aligned \
  use_vision_pose:=true enable_rviz:=false \
  > "${RESULT_DIR}/external_fusion.log" 2>&1 &
PIDS+=("$!")

wait_topic /uav1/mavros/vision_pose/pose "${EXTERNAL_FUSION_READY_TIMEOUT_S}" "${RESULT_DIR}/mavros_vision_pose_first.txt" || {
  echo "No /uav1/mavros/vision_pose/pose sample" >&2
  exit 11
}
wait_topic /uav1/sunray/px4_state "${EXTERNAL_FUSION_READY_TIMEOUT_S}" "${RESULT_DIR}/sunray_px4_state_first.txt" || {
  echo "No /uav1/sunray/px4_state sample" >&2
  exit 12
}

set +e
python3 "${PROJECT_ROOT}/Scripts/sunray/record_fastlio_external_fusion_goal2.py" \
  --duration-s "${GOAL2_RECORD_DURATION_S}" \
  --out "${RESULT_DIR}/GOAL2_FASTLIO_EXTERNAL_FUSION_GATE.json" \
  > "${RESULT_DIR}/goal2_recorder_stdout.json" 2>&1
RECORDER_EXIT=$?
set -e
...
{
  "schema": "mosim.sunray_ros1.fastlio_goal2_external_fusion_manifest.v1",
  "result_dir": "${RESULT_DIR}",
  "claim_boundary": "FAST-LIO aligned odom to Sunray external_fusion only; no arming, no setpoint publication, no px4ctrl mission.",
  "external_source": 0,
  "external_source_name": "ODOM",
  ...
  "local_odom_topic": "/uav1/mavros/local_position/odom",
