#!/usr/bin/env python3
"""Generate the G9-A official PID scalar CFunction Sysblock model."""
...
def strip_c_for_modelica_include(header: str, source: str) -> str:
    ...
    for line in source.splitlines():
        if line.strip() == '#include "px4ctrl_g9a_core_c.h"':
            continue
...
def inport_decl(name: str, index: int) -> str:
    ...
    annotation (Placement(transformation(origin={{-240,{y}}},extent={{{{-8,-8}},{{8,8}}}})),__MWORKS(BlockSystem(Type(inherit=InheritType.auto,ref="double"),Dimension(dimensionType=DimensionType.auto)=1,SampleTime(group="D1")=0.01)));'''


def outport_decl(name: str, index: int) -> str:
    y = 110 - index * 10
    return f'''  SysplorerEmbeddedCoder.Port.Outport {name}_out
    annotation (Placement(transformation(origin={{240,{y}}},extent={{{{-8,-8}},{{8,8}}}})),__MWORKS(BlockSystem(Type(ref="double"),Dimension=1,SampleTime(group="D1")=0.01)));'''
...
def connect_line(src: str, dst: str, index: int) -> str:
    y = 150 - index * 7
    return f'''  connect({src}, {dst}) annotation(Line(origin={{0,0}},points={{{{-190,{y}}},{{-40,{y}}}}},color={{0,0,0}}));'''


def output_connect_line(src: str, dst: str, index: int) -> str:
    y = 110 - index * 7
    return f'''  connect({src}, {dst}) annotation(Line(origin={{0,0}},points={{{{40,{y}}},{{190,{y}}}}},color={{0,0,0}}));'''
...
def build_model(model_name: str, out_dir: Path, include_code: str) -> str:
    ...
    external_call = (
        "MosimPx4ctrlG9AOfficialPidCStepScalar("
...
def main() -> int:
    ...
    parser.add_argument("--model-name", default="G9A_OfficialPID_CFunction_Sysblock")
    ...
    result_dir = (
        ...
        if args.result_dir
        else root / "Results" / "g9" / "official_pid_attitude_thrust_v1" / "g9a_mworks_codegen_work"
    )
    model_dir = (
        Path(args.model_dir).resolve()
        if args.model_dir
        else result_dir / "g9a_official_pid_cfunction_model"
    )
    codegen_dir = (
        Path(args.codegen_dir).resolve()
        if args.codegen_dir
        else result_dir / "g9a_official_pid_cfunction_codegen"
    )
    model_dir.mkdir(parents=True, exist_ok=True)
    codegen_dir.mkdir(parents=True, exist_ok=True)

    header = (root / "Scripts" / "sunray" / "px4ctrl_golden_slice" / "px4ctrl_g9a_core_c.h").read_text(encoding="utf-8")
    source = (root / "Scripts" / "sunray" / "px4ctrl_golden_slice" / "px4ctrl_g9a_core_c.c").read_text(encoding="utf-8")
    ...
    manifest = {
        "schema": "mosim.g9a_official_pid_cfunction_model_build.v1",
        ...
        "interface": {
            ...
            "outputs": OUTPUTS,
            "sample_time_s": 0.01,
            "external_c_function": "MosimPx4ctrlG9AOfficialPidCStepScalar",
        },
