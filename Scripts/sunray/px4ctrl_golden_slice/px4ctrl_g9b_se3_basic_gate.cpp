...
void run_hover_case(GateStats &stats)
{
    ...
    const ControllerOutput out = mosim_px4ctrl::calculate_se3_basic_core(params, state, base_input());
    ...
    {
        fail(stats, "hover_identity", "SE3 hover must produce level attitude and hover normalized thrust");
    }
}
...
void run_horizontal_error_case(GateStats &stats)
{
    ...
    in.reference_position = Vec3{0.0, 0.0, 1.0};
    const ControllerOutput out = mosim_px4ctrl::calculate_se3_basic_core(params, state, in);
    ...
    {
        fail(stats, "horizontal_error_tilts_force", "positive X position error must tilt thrust direction toward +X and keep thrust positive");
    }
}

void run_yaw_and_degenerate_cases(GateStats &stats)
{
    CoreParams params;
    params.hover_percentage = 0.37;
    ...
    const ControllerOutput yaw_out = mosim_px4ctrl::calculate_se3_basic_core(params, state, in);
    stats.yaw_error_rad = std::fabs(mosim_px4ctrl::yaw_from_quat(yaw_out.desired_attitude) - 1.0);
    if (stats.yaw_error_rad > 1.0e-9)
    {
        fail(stats, "yaw_preserved", "level SE3 attitude did not preserve reference yaw");
    }

    in.reference_acceleration = Vec3{20.0, 0.0, -params.gravity};
    in.reference_yaw = 0.0;
    const ControllerOutput degenerate = mosim_px4ctrl::calculate_se3_basic_core(params, state, in);
    if (!finite_quat(degenerate.desired_attitude) || degenerate.normalized_thrust <= 0.0)
    {
        fail(stats, "degenerate_yaw_cross_fallback", "SE3 degenerate b3/b1d case did not produce finite positive output");
    }
}
...
void run_limit_and_disabled_cases(GateStats &stats)
{
    ...
    const ControllerOutput limited = mosim_px4ctrl::calculate_se3_basic_core(params, state, in);
    ...
    {
        fail(stats, "limits_enforced", "SE3 thrust or tilt limits were not enforced cleanly");
    }

    in.enable = false;
    const ControllerOutput disabled = mosim_px4ctrl::calculate_se3_basic_core(params, state, in);
...
int main()
{
    ...
    run_yaw_and_degenerate_cases(stats);
    run_limit_and_disabled_cases(stats);

    std::cout << std::setprecision(17);
    std::cout << "{\n";
    std::cout << "  \"schema\": \"mosim.g9b_se3_basic_static_gate.v1\",\n";
    std::cout << "  \"status\": \"" << (stats.failures == 0 ? "passed" : "failed") << "\",\n";
    std::cout << "  \"claim_boundary\": \"Static se3_basic ATTITUDE_THRUST backend gate only. No ROS, Gazebo, PX4, MAVROS, RViz, UE, or MWORKS runtime is executed.\",\n";
