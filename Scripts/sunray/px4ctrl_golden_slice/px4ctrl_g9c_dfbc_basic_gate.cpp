...
struct GateStats
{
    ...
    double horizontal_b3_x{0.0};
    double yaw_error_rad{0.0};
    double acceleration_feedforward_b3_y{0.0};
...
void run_hover_case(GateStats &stats)
{
    ...
    const ControllerOutput out = mosim_px4ctrl::calculate_dfbc_basic_core(params, state, base_input());
    ...
    {
        fail(stats, "hover_identity", "DFBC Basic hover must produce level attitude and hover normalized thrust");
    }
    if (out.status_text != "dfbc_basic_attitude_thrust")
    {
        fail(stats, "status_text", "DFBC Basic status text did not identify the backend");
    }
}

void run_horizontal_error_case(GateStats &stats)
{
    CoreParams params;
    params.kp[0] = 2.0;
    params.kp[1] = 2.0;
    ...
    auto in = base_input();
    in.position = Vec3{-0.4, 0.0, 1.0};
    const ControllerOutput out = mosim_px4ctrl::calculate_dfbc_basic_core(params, state, in);
    const Vec3 b3 = b3_from_quat(out.desired_attitude);
    stats.horizontal_b3_x = b3.x;
    if (stats.horizontal_b3_x <= 0.01 || !finite_quat(out.desired_attitude) || out.normalized_thrust <= params.hover_percentage)
    {
        fail(stats, "horizontal_error_tilts_force", "positive X flat-output error must tilt thrust direction toward +X and keep thrust positive");
    }
}
...
void run_yaw_and_feedforward_cases(GateStats &stats)
{
    ...
    auto in = base_input();
    in.reference_yaw = 1.0;
    const ControllerOutput yaw_out = mosim_px4ctrl::calculate_dfbc_basic_core(params, state, in);
    stats.yaw_error_rad = std::fabs(mosim_px4ctrl::yaw_from_quat(yaw_out.desired_attitude) - 1.0);
    if (stats.yaw_error_rad > 1.0e-9)
    {
        fail(stats, "yaw_preserved", "level DFBC Basic attitude did not preserve reference yaw");
    }

    in = base_input();
    in.reference_acceleration = Vec3{0.0, 1.5, 0.0};
    const ControllerOutput accel_out = mosim_px4ctrl::calculate_dfbc_basic_core(params, state, in);
    const Vec3 b3 = b3_from_quat(accel_out.desired_attitude);
    stats.acceleration_feedforward_b3_y = b3.y;
    if (stats.acceleration_feedforward_b3_y <= 0.05 || accel_out.normalized_thrust <= params.hover_percentage)
    {
        fail(stats, "acceleration_feedforward", "positive Y flat-output acceleration must tilt thrust direction toward +Y and increase thrust");
    }
}
...
void run_limit_and_disabled_cases(GateStats &stats)
{
    ...
    const ControllerOutput limited = mosim_px4ctrl::calculate_dfbc_basic_core(params, state, in);
    ...
    {
        fail(stats, "limits_enforced", "DFBC Basic thrust or tilt limits were not enforced cleanly");
    }

    in.enable = false;
    const ControllerOutput disabled = mosim_px4ctrl::calculate_dfbc_basic_core(params, state, in);
...
int main()
{
    ...
    run_horizontal_error_case(stats);
    run_yaw_and_feedforward_cases(stats);
    run_limit_and_disabled_cases(stats);

    std::cout << std::setprecision(17);
    std::cout << "{\n";
    std::cout << "  \"schema\": \"mosim.g9c_dfbc_basic_static_gate.v1\",\n";
    std::cout << "  \"status\": \"" << (stats.failures == 0 ? "passed" : "failed") << "\",\n";
    std::cout << "  \"claim_boundary\": \"Static dfbc_basic ATTITUDE_THRUST backend gate only. No jerk/snap full DFBC, ROS, Gazebo, PX4, MAVROS, RViz, UE, or MWORKS runtime is executed.\",\n";
    ...
    std::cout << "  \"horizontal_b3_x\": " << stats.horizontal_b3_x << ",\n";
    std::cout << "  \"yaw_error_rad\": " << stats.yaw_error_rad << ",\n";
    std::cout << "  \"acceleration_feedforward_b3_y\": " << stats.acceleration_feedforward_b3_y << ",\n";
