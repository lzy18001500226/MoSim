#!/usr/bin/env python3
"""Compile and run the G9-D SMC boundary-layer static controller gate."""
...
def main() -> int:
    ...
    if args.result_dir:
        ...
        result_dir = project_root / "Results" / "g9" / "smc_boundary_layer_attitude_thrust_v1" / f"g9d_static_gate_{time.strftime('%Y%m%d_%H%M%S')}"
    ...
    source_files = [
        ...
        script_dir / "px4ctrl_g9d_smc_boundary_layer_gate.cpp",
        Path(__file__).resolve(),
        project_root / "Docs" / "Design" / "架构" / "01_控制器平台" / "controllers" / "SMC.md",
    ...
    exe = build_dir / "px4ctrl_g9d_smc_boundary_layer_gate"
    compile_cmd = [
        "g++",
        "-std=c++11",
        "-O2",
        ...
        windows_path_to_wsl(script_dir / "px4ctrl_g9d_smc_boundary_layer_gate.cpp"),
        "-I",
    ...
    manifest: dict[str, object] = {
        "schema": "mosim.g9d_smc_boundary_layer_static_gate_manifest.v1",
        "status": "unknown",
        "goal": "G9-D",
        "controller_profile": "smc_boundary_layer_attitude_thrust_v1",
        "project_root": str(project_root),
        "result_dir": str(result_dir),
        "compile_command": compile_cmd,
        "claim_boundary": "Static C++ smc_boundary_layer ATTITUDE_THRUST backend gate only. No runtime, terminal/super-twisting SMC, or MWORKS GUI is started.",
    }
