#!/usr/bin/env python3
"""Compile and run the G9-F NMPC outer-loop static controller gate."""
...
def main() -> int:
    ...
    if args.result_dir:
        ...
        result_dir = project_root / "Results" / "g9" / "nmpc_outer_attitude_thrust_v1" / f"g9f_static_gate_{time.strftime('%Y%m%d_%H%M%S')}"
    ...
    source_files = [
        ...
        script_dir / "px4ctrl_g9f_nmpc_outer_gate.cpp",
        Path(__file__).resolve(),
        project_root / "Docs" / "Design" / "架构" / "01_控制器平台" / "controllers" / "NMPC.md",
    ...
    exe = build_dir / "px4ctrl_g9f_nmpc_outer_gate"
    compile_cmd = [
        "g++",
        "-std=c++11",
        "-O2",
        ...
        windows_path_to_wsl(script_dir / "px4ctrl_g9f_nmpc_outer_gate.cpp"),
        "-I",
    ...
    manifest: dict[str, object] = {
        "schema": "mosim.g9f_nmpc_outer_static_gate_manifest.v1",
        "status": "unknown",
        "goal": "G9-F",
        "controller_profile": "nmpc_outer_attitude_thrust_v1",
        "runtime_core_profile": "nmpc_outer",
        "project_root": str(project_root),
        "result_dir": str(result_dir),
        "compile_command": compile_cmd,
        "claim_boundary": "Static short-horizon constrained outer-loop ATTITUDE_THRUST backend gate only. No full nonlinear online solver, runtime, or MWORKS GUI is started.",
    }
