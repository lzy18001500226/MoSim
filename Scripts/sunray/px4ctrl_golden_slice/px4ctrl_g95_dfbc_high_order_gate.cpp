#include "px4ctrl_core.h"

#include <algorithm>
#include <cstdlib>
#include <iomanip>
#include <iostream>
#include <string>

using mosim_px4ctrl::ControllerInput;
using mosim_px4ctrl::ControllerOutput;
using mosim_px4ctrl::CoreParams;
using mosim_px4ctrl::CoreState;
using mosim_px4ctrl::Quat;
using mosim_px4ctrl::Vec3;

namespace
{

constexpr double kTol = 1.0e-9;

struct GateStats
{
    int failures{0};
    double hover_thrust_error{0.0};
    double jerk_body_rate_x{0.0};
    double snap_body_accel_x{0.0};
    double body_rate_limit_y{0.0};
    double yaw_rate_body_rate_z{0.0};
};

double quat_min_norm(const Quat &a, const Quat &b)
{
    const double same = std::sqrt(
        (a.w - b.w) * (a.w - b.w) +
        (a.x - b.x) * (a.x - b.x) +
        (a.y - b.y) * (a.y - b.y) +
        (a.z - b.z) * (a.z - b.z));
    const double neg = std::sqrt(
        (a.w + b.w) * (a.w + b.w) +
        (a.x + b.x) * (a.x + b.x) +
        (a.y + b.y) * (a.y + b.y) +
        (a.z + b.z) * (a.z + b.z));
    return std::min(same, neg);
}

void fail(GateStats &stats, const std::string &name, const std::string &message)
{
    ++stats.failures;
    std::cerr << "FAILED_CASE " << name << " " << message << "\n";
}

ControllerInput base_input()
{
    ControllerInput in;
    in.dt = 0.01;
    in.position = Vec3{0.0, 0.0, 1.0};
    in.velocity = Vec3{0.0, 0.0, 0.0};
    in.attitude = Quat{};
    in.imu_attitude = Quat{};
    in.reference_position = Vec3{0.0, 0.0, 1.0};
    in.reference_velocity = Vec3{0.0, 0.0, 0.0};
    in.reference_acceleration = Vec3{0.0, 0.0, 0.0};
    in.reference_jerk = Vec3{0.0, 0.0, 0.0};
    in.reference_snap = Vec3{0.0, 0.0, 0.0};
    in.reference_yaw = 0.0;
    in.reference_yaw_rate = 0.0;
    in.reference_yaw_acceleration = 0.0;
    return in;
}

void run_hover_case(GateStats &stats)
{
    CoreParams params;
    params.hover_percentage = 0.294;
    params.tilt_limit_rad = 0.8;
    CoreState state;
    mosim_px4ctrl::reset_thrust_mapping(params, state);

    const ControllerOutput out = mosim_px4ctrl::calculate_dfbc_high_order_core(params, state, base_input());
    stats.hover_thrust_error = std::fabs(out.normalized_thrust - params.hover_percentage);
    if (stats.hover_thrust_error > 1.0e-12 ||
        quat_min_norm(out.desired_attitude, Quat{}) > 1.0e-12 ||
        mosim_px4ctrl::norm(out.desired_body_rate) > 1.0e-12 ||
        mosim_px4ctrl::norm(out.desired_body_acceleration) > 1.0e-12)
    {
        fail(stats, "hover_identity", "high-order DFBC hover must keep level attitude, hover thrust, and zero high-order feedforward");
    }
}

void run_high_order_cases(GateStats &stats)
{
    CoreParams params;
    params.hover_percentage = 0.294;
    params.high_order_body_rate_limit[0] = 2.0;
    params.high_order_body_rate_limit[1] = 2.0;
    params.high_order_body_rate_limit[2] = 1.0;
    params.high_order_body_accel_limit[0] = 20.0;
    params.high_order_body_accel_limit[1] = 20.0;
    params.high_order_body_accel_limit[2] = 8.0;
    CoreState state;
    mosim_px4ctrl::reset_thrust_mapping(params, state);

    auto in = base_input();
    in.reference_jerk = Vec3{1.0, 0.0, 0.0};
    in.reference_snap = Vec3{2.0, 0.0, 0.0};
    in.reference_yaw_rate = 0.3;
    in.reference_yaw_acceleration = 0.4;
    const ControllerOutput out = mosim_px4ctrl::calculate_dfbc_high_order_core(params, state, in);
    stats.jerk_body_rate_x = out.desired_body_rate.x;
    stats.snap_body_accel_x = out.desired_body_acceleration.x;
    stats.yaw_rate_body_rate_z = out.desired_body_rate.z;
    if (stats.jerk_body_rate_x >= -0.01 ||
        stats.snap_body_accel_x >= -0.01 ||
        std::fabs(stats.yaw_rate_body_rate_z - in.reference_yaw_rate) > 1.0e-6)
    {
        fail(stats, "high_order_feedforward", "jerk/snap/yaw-rate must generate nonzero body-rate/body-acceleration feedforward");
    }

    in.reference_jerk = Vec3{0.0, -100.0, 0.0};
    const ControllerOutput limited = mosim_px4ctrl::calculate_dfbc_high_order_core(params, state, in);
    stats.body_rate_limit_y = limited.desired_body_rate.y;
    if (std::fabs(stats.body_rate_limit_y - params.high_order_body_rate_limit[1]) > kTol)
    {
        fail(stats, "body_rate_limit", "high-order body-rate feedforward must be bounded");
    }
}

} // namespace

int main()
{
    GateStats stats;

    run_hover_case(stats);
    run_high_order_cases(stats);

    std::cout << std::setprecision(17);
    std::cout << "{\n";
    std::cout << "  \"schema\": \"mosim.g95_dfbc_high_order_static_gate.v1\",\n";
    std::cout << "  \"status\": \"" << (stats.failures == 0 ? "passed" : "failed") << "\",\n";
    std::cout << "  \"claim_boundary\": \"Static high-order DFBC core gate only. It proves p/v/a/jerk/snap/yaw-rate inputs can produce attitude/thrust plus body-rate/body-acceleration feedforward. It does not switch PX4/MAVROS to body-rate control, run ROS/Gazebo, or prove MWORKS/PX4-native deployment.\",\n";
    std::cout << "  \"failure_count\": " << stats.failures << ",\n";
    std::cout << "  \"hover_thrust_error\": " << stats.hover_thrust_error << ",\n";
    std::cout << "  \"jerk_body_rate_x\": " << stats.jerk_body_rate_x << ",\n";
    std::cout << "  \"snap_body_accel_x\": " << stats.snap_body_accel_x << ",\n";
    std::cout << "  \"yaw_rate_body_rate_z\": " << stats.yaw_rate_body_rate_z << ",\n";
    std::cout << "  \"body_rate_limit_y\": " << stats.body_rate_limit_y << "\n";
    std::cout << "}\n";

    return stats.failures == 0 ? EXIT_SUCCESS : EXIT_FAILURE;
}
