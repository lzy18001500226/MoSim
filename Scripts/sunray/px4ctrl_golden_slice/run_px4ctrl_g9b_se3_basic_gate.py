#!/usr/bin/env python3
"""Compile and run the G9-B SE3 Basic static controller gate."""
...
def main() -> int:
    ...
    if args.result_dir:
        ...
        result_dir = project_root / "Results" / "g9" / "se3_basic_attitude_thrust_v1" / f"g9b_static_gate_{time.strftime('%Y%m%d_%H%M%S')}"
    ...
    source_files = [
        ...
        script_dir / "px4ctrl_g9b_se3_basic_gate.cpp",
        Path(__file__).resolve(),
        project_root / "References" / "Lab" / "ego-planner" / "src" / "uav_simulator" / "so3_control" / "src" / "SO3Control.cpp",
        project_root / "References" / "Lab" / "ego-planner" / "src" / "uav_simulator" / "so3_control" / "include" / "so3_control" / "SO3Control.h",
        project_root / "References" / "Control" / "Quadrotor_SE3_Control" / "se3_controller.py",
    ...
    exe = build_dir / "px4ctrl_g9b_se3_basic_gate"
    compile_cmd = [
        "g++",
        "-std=c++11",
        "-O2",
        ...
        windows_path_to_wsl(script_dir / "px4ctrl_g9b_se3_basic_gate.cpp"),
        "-I",
    ...
    manifest: dict[str, object] = {
        "schema": "mosim.g9b_se3_basic_static_gate_manifest.v1",
        "status": "unknown",
        "goal": "G9-B",
        ...
        "claim_boundary": "Static C++ se3_basic ATTITUDE_THRUST backend gate only. No runtime or MWORKS GUI is started.",
    }
