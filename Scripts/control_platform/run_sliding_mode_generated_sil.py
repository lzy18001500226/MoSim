#!/usr/bin/env python3
"""Compare official MWORKS-generated P3 C with the project sliding-mode core."""
...
OUTPUT_FIELDS = [
    ...
    "normalized_thrust", "collective_thrust_n", "saturated", "status_code",
]
...
def main() -> int:
    ...
    parser.add_argument("--generated-dir", type=Path, default=ROOT / f"Results/control_platform/p3_sliding_mode_mworks_20260716/generated_c/{MODEL}")
    parser.add_argument("--result-dir", type=Path, default=ROOT / "Results/control_platform/p3_sliding_mode_mworks_20260716/sil")
    ...
    source_compile = run_wsl([
        ...
        wsl_path(SOURCE_DIR / "sliding_mode_attitude_thrust_core.c"), wsl_path(source_path),
    ...
    if source_compile.returncode or generated_compile.returncode:
        report = {"schema": "mosim.control_platform.p3_generated_sil.v1", "status": "blocked", "stage": "compile", "source_return_code": source_compile.returncode, "generated_return_code": generated_compile.returncode}
    else:
        ...
        for controller_id in range(1, 7):
            ...
            for index, (left, right) in enumerate(zip(expected, actual, strict=True)):
                ...
                if difference > tolerance:
                    failures.append({"controller_id": controller_id, "column": index, "expected": left, "actual": right, "difference": difference})
        report = {
            "schema": "mosim.control_platform.p3_generated_sil.v1",
            ...
            "official_codegen": "MWORKS GenerateModelCode", "failure_count": len(failures),
            "failures": failures[:30],
            "claim_ceiling": "Official MWORKS-generated C is numerically equivalent to the project core for six deterministic sliding-mode cases; graphical Sysblock equivalence and Gazebo runtime remain separate gates.",
        }
    source_exe.unlink(missing_ok=True); generated_exe.unlink(missing_ok=True)
    (result_dir / "P3_GENERATED_SIL_EQUIVALENCE.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
