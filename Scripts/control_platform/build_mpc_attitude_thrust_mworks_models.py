#!/usr/bin/env python3
"""Build the P4 MPC ATTITUDE_THRUST CFunction bridge and MIL fixtures."""
...
OUTPUTS = [
    ...
    "unconstrained_acceleration_x", "unconstrained_acceleration_y", "unconstrained_acceleration_z",
    "auxiliary_x", "auxiliary_y", "auxiliary_z",
...
FEATURE_OUTPUT = {
    "linear_mpc": "solver_cost",
    "robust_mpc": "auxiliary_x",
    "adaptive_mpc": "auxiliary_x",
    "tube_mpc": "auxiliary_x",
    "explicit_gain_scheduled_mpc": "auxiliary_x",
    "ilqr": "solver_iterations",
    "mppi": "solver_iterations",
}
BASE_INPUTS = {
    "dt": 0.01,
...
def embedded_c() -> str:
    ...
    header = (core_dir / "mpc_attitude_thrust_core.h").read_text(encoding="utf-8")
    source = (core_dir / "mpc_attitude_thrust_core.c").read_text(encoding="utf-8")
    blob = builder.strip_c_for_modelica_include(header, source)
    blob = "\n".join(
        line for line in blob.splitlines()
        if line.strip() != '#include "mpc_attitude_thrust_core.h"'
    )
...
def main() -> int:
    ...
    parser.add_argument("--result-dir", default="Results/control_platform/p4_mpc_mworks_20260716")
    ...
    bridge_name = "MoSim_P4_Mpc_CFunction_Sysblock"
    bridge = generic_builder.build_model(bridge_name, codegen_dir, embedded_c(), real_as_float=False)
    bridge = bridge.replace("MosimPx4ctrlG9FamilyCStepScalar", "MosimMpcStepScalar")
    ...
    for controller_id, algorithm_name in VARIANTS.items():
        fixture_name = f"MoSim_P4_{algorithm_name.upper()}_MIL"
    ...
    manifest = {
        "schema": "mosim.p4_mpc_mworks_build.v1",
        ...
        "deferred": ["learning_mpc", "distributed_mpc"],
        "claim_boundary": "Fixed-size CFunction bridge and seven MIL fixtures only. Live CheckModel/MIL, graphical equivalence, official codegen/SIL, timing, and Gazebo runtime are separate gates.",
    }
