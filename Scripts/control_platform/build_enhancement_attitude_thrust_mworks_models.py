#!/usr/bin/env python3
"""Build P5 fixed-size enhancement ATTITUDE_THRUST MWORKS fixtures."""
...
INPUTS = [
    ...
    "measured_acceleration_x", "measured_acceleration_y", "measured_acceleration_z",
    ...
    "reference_yaw", "trajectory_phase_bin", "repeat_complete",
    "mass_kg", "gravity_mps2", "hover_percentage", "max_tilt_rad",
    "min_collective_thrust_n", "max_collective_thrust_n", "enable", "reset",
]
OUTPUTS = [
...
BASE_INPUTS = {
    ...
    "max_collective_thrust_n": 16.0, "enable": 1.0, "reset": 0.0,
}
...
def embedded_c() -> str:
    ...
    header = (core_dir / "enhancement_attitude_thrust_core.h").read_text(encoding="utf-8")
    source = (core_dir / "enhancement_attitude_thrust_core.c").read_text(encoding="utf-8")
    blob = builder.strip_c_for_modelica_include(header, source)
    blob = "\n".join(
        line for line in blob.splitlines()
        if line.strip() != '#include "enhancement_attitude_thrust_core.h"'
    )
...
def main() -> int:
    ...
    parser.add_argument("--result-dir", default="Results/control_platform/p5_enhancement_mworks_20260717")
    ...
    bridge_name = "MoSim_P5_Enhancement_CFunction_Sysblock"
    bridge = generic_builder.build_model(bridge_name, codegen_dir, embedded_c(), real_as_float=False)
    bridge = bridge.replace("MosimPx4ctrlG9FamilyCStepScalar", "MosimEnhancementStepScalar")
    ...
    for controller_id, algorithm_name in VARIANTS.items():
        fixture_name = f"MoSim_P5_{algorithm_name.upper()}_MIL"
    ...
    manifest = {
        "schema": "mosim.p5_enhancement_mworks_build.v1",
        ...
        "learning_modules": {
            "fuzzy_anfis_compensation": "deferred_missing_frozen_artifact",
            "rbf_nn_disturbance_compensation": "deferred_missing_frozen_artifact",
            "rl_gain_scheduling_residual_policy": "deferred_missing_frozen_artifact_and_safety_benchmark",
        },
