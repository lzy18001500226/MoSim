...
def main() -> int:
    ...
    require("NominalControlRateHz = 70.0f" in playback_header, "UE playback must preserve the current 70 Hz nominal input contract")
    require("MinimumDisplayRateHz = 30.0f" in playback_header, "UE playback must preserve the current 30 fps display floor")
    ...
    require("SetFollowTargets" in review_camera_header, "Review camera must expose multi-UAV follow targets")
    require("MworksReviewCycleTarget" in review_camera_source, "Review camera must bind the Q-cycle action")
    require("formation_overview" in review_camera_source, "Review camera must expose the formation-centroid overview mode")
    require("LocationSum / static_cast<float>(ValidCount)" in review_camera_source, "Formation overview must follow the geometric centroid")
    require("ReviewPawn->SetFollowTargets(FollowActors)" in game_mode_source, "GameMode must bind all spawned playback actors to the review camera")
    ...
    require("const FQuadrotorMworksFrame Frame = Receiver->GetLatestFrame()" in bridge_source and "Playback->ApplyFrame(Frame, DeltaSeconds)" in bridge_source, "Playback actor must apply the latest UDP frame before visual-gate diagnostics")
