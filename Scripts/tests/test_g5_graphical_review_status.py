...
def test_g5_status_tracks_current_packets_and_excludes_history_without_promoting_simulation() -> None:
    ...
    assert status["summary"]["top_level_scheme_count"] == 49
    ...
    assert status["families"]["pid_family"]["candidate_count"] == 5
    assert 0 <= status["families"]["pid_family"]["reviewed_count"] <= 5
