...
def test_qgc_command_surface_does_not_autostart_runtime() -> None:
    operator_bridge = (QGC_CUSTOM / "src" / "MoSimOperatorBridge.cc").read_text(encoding="utf-8")
    operator_qml = (QGC_CUSTOM / "src" / "FlyViewCustomLayer.qml").read_text(encoding="utf-8")
    legacy_bridge = (QGC_CUSTOM / "src" / "MoSimOrchestratorBridge.cc").read_text(encoding="utf-8")
    ...
    assert "QClipboard" in operator_bridge
    assert "clipboard->setText(command)" in operator_bridge
    ...
    assert "QGC 不执行这些命令" in operator_qml
...
def test_operator_cmd_entrypoints_are_explicit() -> None:
    flight_cmd = Path("Scripts/cmd/01_\u8fd0\u884cC99\u5355\u673a\u8d77\u98de\u60ac\u505c\u964d\u843d.cmd").read_text(encoding="utf-8")
    ground_cmd = Path("Scripts/cmd/\u542f\u52a8MoSim\u5730\u9762\u7ad9.cmd").read_text(encoding="utf-8")
    stop_cmd = Path("Scripts/cmd/\u505c\u6b62\u6240\u6709\u4eff\u771f.cmd").read_text(encoding="utf-8")
    compatibility = Path("Scripts/cmd/Archive/legacy_unverified/Start_MoSim_QGC.cmd").read_text(encoding="utf-8")

    assert "run_px4ctrl_fastlio_hover_gate.sh" in flight_cmd
...
def test_operator_workflow_distinguishes_manual_and_automatic_flight_authority() -> None:
    ...
    assert "\u5f53\u524d\u5165\u53e3\uff1a`Scripts/cmd/\u542f\u52a8MoSim\u5730\u9762\u7ad9.cmd`" in workflow
