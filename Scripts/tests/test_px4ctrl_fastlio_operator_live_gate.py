...
def test_operator_live_gate_preserves_the_frozen_fastlio_baseline_and_starts_read_only_displays() -> None:
    ...
    assert '"${MISSION}" == "figure8"' in gate
    assert 'run_px4ctrl_basic_gate.sh" "${MISSION}"' in gate
    ...
    assert "/mosim/fastlio/occupancy_object_review" in gate
    assert "/Laser_map" in gate
    assert "MID360" not in gate
    assert "export SUNRAY_GPS_SENSOR_MODE=removed" in gate
