...
def parse_variable_info(f):
    ...
    # The Variable Index Table has structure:
    # Row 0: variable type (1=continuous, 2=discrete, etc.)
    # Row 1: column index in corresponding data table
    # Row 2: byte offset in name table
    # (Row 3 if exists: length or other metadata)
    ...
    for i in range(num_vars):
        ...
        name_start = int(index_table[2][i])

        # Find name end (next variable start or end of string)
        if i + 1 < num_vars:
            name_end = int(index_table[2][i + 1])
        else:
            name_end = len(name_str)

        # Extract name
        name = name_str[name_start:name_end].strip('\x00').strip()
