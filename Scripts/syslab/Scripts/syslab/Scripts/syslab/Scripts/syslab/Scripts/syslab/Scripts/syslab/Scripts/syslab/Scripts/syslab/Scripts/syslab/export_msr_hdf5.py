...
def parse_variable_info(f):
    ...
    # Split by null bytes to get individual variable names
    var_names = [name.decode('utf-8', errors='ignore')
                 for name in name_bytes.split(b'\x00')
                 if name]
    ...
    for i in range(index_table.shape[1]):
        ...
        # var_num is 1-indexed, convert to 0-indexed
        name_idx = var_num - 1
