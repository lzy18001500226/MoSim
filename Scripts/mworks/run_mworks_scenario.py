...
def to_windows_path(path: Path) -> str:
    """Return a Windows path for Sysplorer even when invoked from WSL."""
...
def windows_path(repo_path: str, *, default: Path | None = None) -> str:
    if not repo_path:
        if default is None:
            raise ValueError("Missing required path")
        return to_windows_path(default)
    if ":" in repo_path[:4]:
        return repo_path
...
def run_postprocess(config: dict[str, Any]) -> None:
    ...
    if config.get("generate_replay_html", False):
        ...
        subprocess.run(
            [sys.executable, "Scripts/results/generate_replay_html.py", str(replay_file), str(replay_html)],
            cwd=ROOT,
            check=True,
        )
