#!/usr/bin/env python3
"""Run the three-UAV OpenBlocks PX4CTRL ECBF-safety branch on one existing session.
...
def refresh_and_check(record: dict[str, Any], port: int, refresh_root: bool, open_diagram: bool) -> None:
    # The bundled 2026a API accepts the existing-session port as a keyword.
    # Passing host and port positionally can leave the attach handshake pending.
    record["connect_sysplorer_return"] = repr(ModelingPy.ConnectSysplorer(port=port))
...
def smoke_metrics(series: dict[str, list[float]], stop_time_s: float) -> dict[str, Any]:
    ...
    return {
        "schema": "mosim.mworks.three_uav_openblocks_px4ctrl_ecbf_safety.smoke.v1",
        "generated_at": now_iso(),
        "source": "MWORKS_MCP",
        "model_name": MODEL_NAME,
        "controller_identity": "PX4CTRL equation bridge with an external pairwise ECBF reference governor",
        ...
        **tracking_max,
        ...
        "claim_boundary": (
            "A bounded three-UAV MWORKS PX4CTRL safety-branch smoke test. The pairwise ECBF reference governor is "
            "not an online global replanner or a plant-coupled wall-collision assertion."
        ),
    }
...
def run_check(args: argparse.Namespace) -> int:
    ...
    record: dict[str, Any] = {
        "schema": "mosim.mworks.three_uav_openblocks_px4ctrl_ecbf_safety.check.v1",
        "created_at": now_iso(),
        "model_name": MODEL_NAME,
        ...
        "controller_identity": "PX4CTRL equation bridge with an external pairwise ECBF reference governor",
        "claim_boundary": "Native safety-branch model integrity and diagram preparation only; no simulation, global collision, or runtime acceptance claim.",
    }
    try:
        # The graphical surface is opened only by the dedicated review phase.
        # Keeping CheckModel headless lets this process write its native record
        # before any GUI-focus handoff occurs.
        refresh_and_check(record, args.port, refresh_root=True, open_diagram=False)
...
def run_phase(args: argparse.Namespace, phase: str, stop_time_s: float, interval_s: float) -> int:
    ...
    record: dict[str, Any] = {
        "schema": "mosim.mworks.three_uav_openblocks_px4ctrl_ecbf_safety.run.v1",
        "created_at": now_iso(),
        "phase": phase,
        "source": "MWORKS_MCP",
        ...
        "controller_identity": "PX4CTRL equation bridge with an external pairwise ECBF reference governor",
        "claim_boundary": (
            "Three current MWORKS whole-aircraft PX4CTRL loops track the frozen OpenBlocks references through an "
            "external pairwise ECBF reference governor. The governor is assessed for finite output, intervention, and "
            "inter-UAV separation only; it is not online replanning or plant-coupled static-obstacle collision acceptance."
        ),
    }
    try:
        ...
        if phase == "smoke":
            ...
            from audit_three_uav_openblocks_px4ctrl_ecbf_safety_result import audit, write_outputs  # noqa: PLC0415
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "Results" / "planning" / "three_uav_openblocks_px4ctrl_ecbf_safety_20260731",
    )
