#!/usr/bin/env python3
"""Run the three-UAV OpenBlocks PX4CTRL formation model on one existing session.
...
MODEL_NAME = "MoSimQuadrotorModel.Guidance.Planning.ThreeUavOpenBlocksReconfigurableFormationPx4Ctrl"
...
def smoke_metrics(series: dict[str, list[float]], stop_time_s: float) -> dict[str, Any]:
    ...
    return {
        "schema": "mosim.mworks.three_uav_openblocks_px4ctrl.smoke.v1",
        "generated_at": now_iso(),
        "source": "MWORKS_MCP",
        "model_name": MODEL_NAME,
        "controller_identity": "PX4CTRL equation bridge through Px4CtrlAttitudeThrustAdapter",
        ...
        **tracking_max,
        ...
        "claim_boundary": (
            "A bounded three-UAV MWORKS PX4CTRL dynamics smoke test. The global wall layer is rendered and is part of the "
...
def run_check(args: argparse.Namespace) -> int:
    ...
    record: dict[str, Any] = {
        "schema": "mosim.mworks.three_uav_openblocks_px4ctrl.check.v1",
        "created_at": now_iso(),
        "model_name": MODEL_NAME,
        ...
        "controller_identity": "PX4CTRL equation bridge through Px4CtrlAttitudeThrustAdapter",
        "claim_boundary": "Native PX4CTRL model integrity and diagram preparation only; no simulation or collision-avoidance acceptance claim.",
    }
    try:
...
def run_phase(args: argparse.Namespace, phase: str, stop_time_s: float, interval_s: float) -> int:
    output_dir = args.output_dir.resolve()
    stem = "mworks_px4ctrl_smoke_15s" if phase == "smoke" else "mworks_px4ctrl_full_304p84s"
    ...
    record: dict[str, Any] = {
        "schema": "mosim.mworks.three_uav_openblocks_px4ctrl.run.v1",
        "created_at": now_iso(),
        "phase": phase,
        "source": "MWORKS_MCP",
        ...
        "controller_identity": "PX4CTRL equation bridge through Px4CtrlAttitudeThrustAdapter",
        "claim_boundary": (
            "Three current MWORKS whole-aircraft PX4CTRL loops track the frozen OpenBlocks references. "
    ...
    try:
        ...
        if phase == "smoke":
            ...
            from audit_three_uav_openblocks_px4ctrl_result import audit, write_outputs  # noqa: PLC0415
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "Results" / "planning" / "three_uav_openblocks_px4ctrl_completion_20260731",
    )
