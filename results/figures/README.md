# results/figures 图表审核入口

本目录只保存由真实 raw CSV 和 metrics 生成的报告候选图。人工审核以 `人工审核清单.csv` 为入口，不直接在根目录里翻散乱文件。
...
| `smoke/example1_mcp/` | 0-1 s MCP 链路烟雾验证，只证明流程通，不作为最终展示素材 | low |
...
1. 优先审核 `official/example3_figure8/`。这才是 8 字形轨迹相关图。
