...
model AWFF_PID_Sysblock_Demo_SIL_Constant "MWORKS.Sysblock AWFF PID SIL constant-input reference"
  extends ModelWorkspace;
  import SysplorerEmbeddedCoder.Types.*;
  import BaseWorkspace.*;
  annotation(__MWORKS(version="26.3.0",modelType=Control,PortArrangement(Right(thrust_cmd)),BlockSystem(blockKind=BlockKind.userModel,SampleTime(auto=true,group="")=0.01,OutputInterval=0.01),SysblockVersion="1.0"),Icon(coordinateSystem(preserveAspectRatio=false)),experiment(DoublePrecision=false,Algorithm=Euler,IntegratorStep=0.01,Interval=0.01,StartTime=0,StopTime=0.03,StoreEventValue=0),Diagram(coordinateSystem(extent={{-240,-100},{120,100}},grid={2,2})));

  SysplorerEmbeddedCoder.Sources.Constant z_error_source(k=0.1)
    annotation (Placement(transformation(origin={-222,34},extent={{-10,-10},{10,10}})),__MWORKS(BlockSystem(SampleTime(auto=true,group="D1")=0.01,Instance(y(Type(inherit=InheritType.constantValue,ref="double"),Dimension=1),k(Type(ref="double"),Dimension=1)))));
  ...
  equation
  connect(z_error_source.y, ki_integrator.u1)
  ...
  connect(ff_gain.u, z_error_source.y)
...
end AWFF_PID_Sysblock_Demo_SIL_Constant;