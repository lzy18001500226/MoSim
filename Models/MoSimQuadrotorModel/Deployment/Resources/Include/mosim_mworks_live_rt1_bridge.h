...
static int mosim_rt1_compute_and_send_official_pid(
    const MosimRt1StateReference *state,
    ...
    valid =
        (state->header.flags & MOSIM_RT1_FLAG_STATE_VALID) != 0u &&
        (state->header.flags & MOSIM_RT1_FLAG_REFERENCE_VALID) != 0u;
    if (!valid) {
        *output_valid = 0;
        return 0;
    }

    desired_ax = state->values[19] +
        kv_x * (state->values[16] - state->values[3]) +
        kp_x * (state->values[13] - state->values[0]);
    desired_ay = state->values[20] +
        kv_y * (state->values[17] - state->values[4]) +
        kp_y * (state->values[14] - state->values[1]);
    desired_az = state->values[21] +
        kv_z * (state->values[18] - state->values[5]) +
        kp_z * (state->values[15] - state->values[2]) + gravity;
    yaw = state->values[22];
    roll = (desired_ax * sin(yaw) - desired_ay * cos(yaw)) / gravity;
    pitch = (desired_ax * cos(yaw) + desired_ay * sin(yaw)) / gravity;
    qw = cos(yaw / 2.0) * cos(pitch / 2.0) * cos(roll / 2.0) +
         sin(yaw / 2.0) * sin(pitch / 2.0) * sin(roll / 2.0);
    qx = cos(yaw / 2.0) * cos(pitch / 2.0) * sin(roll / 2.0) -
         sin(yaw / 2.0) * sin(pitch / 2.0) * cos(roll / 2.0);
    qy = cos(yaw / 2.0) * sin(pitch / 2.0) * cos(roll / 2.0) +
         sin(yaw / 2.0) * cos(pitch / 2.0) * sin(roll / 2.0);
    qz = sin(yaw / 2.0) * cos(pitch / 2.0) * cos(roll / 2.0) -
         cos(yaw / 2.0) * sin(pitch / 2.0) * sin(roll / 2.0);
    ...
    full_thrust = mass * gravity / hover_percentage;
    collective = mass * desired_az;
    if (collective < 0.0) collective = 0.0;
    if (collective > full_thrust) collective = full_thrust;
    valid = MOSIM_RT1_ISFINITE(qx) && MOSIM_RT1_ISFINITE(qy) &&
        MOSIM_RT1_ISFINITE(qz) && MOSIM_RT1_ISFINITE(qw) &&
        MOSIM_RT1_ISFINITE(collective) && collective > 0.0;

    memset(&command, 0, sizeof(command));
    adapter_stamp = state->header.produced_or_receive_monotonic_ns;
    command.header.magic = MOSIM_RT1_COMMAND_MAGIC;
    command.header.version = MOSIM_RT1_VERSION;
    command.header.flags = valid ? MOSIM_RT1_FLAG_OUTPUT_VALID : 0u;
    command.header.sequence = mosim_rt1_command_sequence++;
    command.header.related_sequence = state->header.sequence;
    command.header.source_stamp_ns = adapter_stamp;
    command.header.produced_or_receive_monotonic_ns = mosim_rt1_monotonic_ns();
    command.header.valid_until_ns = adapter_stamp + 50000000ULL;
    memcpy(command.header.run_id, state->header.run_id, MOSIM_RT1_RUN_ID_SIZE);
    command.qx = qx;
    command.qy = qy;
    command.qz = qz;
    command.qw = qw;
    command.collective_thrust_n = collective;
    command.saturation_mask = mass * desired_az != collective ? 1u : 0u;
    command.controller_status = 1u;
    ...
    *output_valid = valid;
    ...
    return sendto(
        ...
        peer_size) == (int)sizeof(command) ? 1 : -1;
}

/* Drain bounded backlog, then control from the freshest state only. */
static int mosim_mworks_live_rt1_exchange_official_pid(
    double mass,
    double gravity,
    double hover_percentage,
    double kp_x,
    ...
    double kv_z,
    int *sent_frames,
    int *last_state_sequence,
    double *last_collective_thrust_n,
    ...
    int index;
    int have_latest = 0;
    MosimRt1StateReference state;
    MosimRt1StateReference latest_state;
    struct sockaddr_in peer;
    ...
    *socket_local_port = mosim_rt1_socket_local_port;
    for (index = 0; index < MOSIM_RT1_MAX_DRAIN; ++index) {
        ...
        if (received != (int)sizeof(state) ||
            state.header.magic != MOSIM_RT1_STATE_MAGIC ||
            state.header.version != MOSIM_RT1_VERSION) {
            continue;
        }
        ++processed;
        mosim_rt1_latest = state;
        mosim_rt1_peer = peer;
        mosim_rt1_peer_size = peer_size;
        latest_state = state;
        latest_peer = peer;
        ...
        have_latest = 1;
        *last_state_sequence = (int)state.header.sequence;
    }
    if (have_latest && mosim_rt1_compute_and_send_official_pid(
            &latest_state, &latest_peer, latest_peer_size,
            mass, gravity, hover_percentage,
            kp_x, kp_y, kp_z, kv_x, kv_y, kv_z,
            last_collective_thrust_n, last_output_valid) > 0) {
        ++(*sent_frames);
    }
    return processed;
}
