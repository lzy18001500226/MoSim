...
model PhysicalWrenchAdapter
  ...
  parameter Boolean enable_angular_damping = profile.enable_angular_damping_default
    "Optional experimental body-rate damping; disabled for the nominal baseline";
  parameter Real angular_damping_coefficient[3](each unit = "N.m.s/rad") =
    profile.angular_damping_coefficient_seed
    "Linear damping coefficients resolved in body axes";
  ...
  MoSimQuadrotorModel.Vehicle.Sunray150Modules.WrapperSurface wrapper(
    profile = profile,
  ...
  Real angular_damping_torque_body[3](each unit = "N.m");
  Real applied_total_yaw_torque_body(unit = "N.m");
  Real motor_order_gate_error;
  Real yaw_direction_gate_error;
equation
  applied_force_body = {0, 0, wrapper.total_thrust};
  angular_damping_torque_body = if enable_angular_damping then {
    -angular_damping_coefficient[1] * body.w_a[1],
    -angular_damping_coefficient[2] * body.w_a[2],
    -angular_damping_coefficient[3] * body.w_a[3]} else {0, 0, 0};
  applied_torque_body = wrapper.total_moment_body + angular_damping_torque_body;
  ...
  applied_yaw_torque_body = wrapper.total_moment_body[3];
  applied_total_yaw_torque_body = applied_torque_body[3];
  ...
  annotation(
    Icon(coordinateSystem(extent = {{-100, -100}, {100, 100}}), graphics = {
      ...
      Text(origin = {0, 12}, extent = {{-88, 30}, {88, -30}},
        textString = "Plant", textColor = {100, 55, 0},
        textStyle = {TextStyle.Bold}),
      Text(origin = {0, -28}, extent = {{-88, 22}, {88, -22}},
        textString = "Dynamics", textColor = {130, 80, 20})}),
