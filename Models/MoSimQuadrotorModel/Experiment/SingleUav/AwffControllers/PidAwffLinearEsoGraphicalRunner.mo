within MoSimQuadrotorModel.Experiment.SingleUav.AwffControllers;
model PidAwffLinearEsoGraphicalRunner
  "pid_awff_linear_eso graphical runner with AWFF_PidLinearEsoControllerEquation_Sysblock"
...
equation
  // ── Reference/Plant → Controller core (direct Sysblock ports) ──
  connect(reference.position_command[1], controller_core.x_error) 
    annotation(Line(points={{-300,240}}, color={0,0,127}));
  connect(reference.position_command[2], controller_core.y_error) 
    annotation(Line(points={{-300,226}}, color={0,0,127}));
  connect(reference.position_command[3], controller_core.z_error) 
    annotation(Line(points={{-300,212}}, color={0,0,127}));
  connect(reference.velocity_command[3], controller_core.z_ref_rate) 
    annotation(Line(points={{-300,198}}, color={0,0,127}));
  connect(plant.attitude[1], controller_core.roll_mea) 
    annotation(Line(points={{777,191},{820,191},{820,-100},{-220,-100},{-220,184},{-145,184}}, color={0,100,150}));
  connect(plant.attitude[2], controller_core.pitch_mea) 
    annotation(Line(points={{777,191},{820,191},{820,-100},{-220,-100},{-220,170},{-145,170}}, color={0,100,150}));
  connect(plant.attitude[3], controller_core.yaw_mea) 
    annotation(Line(points={{777,191},{820,191},{820,-100},{-220,-100},{-220,156},{-145,156}}, color={0,100,150}));
  connect(reference.velocity_command[1], controller_core.yaw_ref) 
    annotation(Line(points={{-300,142}}, color={0,0,127}));

  // ── Controller core → Fault compensator ──
  connect(controller_core.y, fault_compensator.command_in[1]) 
    annotation(Line(points={{15,230},{205,230},{205,35},{270,35}}, color={55,80,115}));
  connect(controller_core.y1, fault_compensator.command_in[2]) 
    annotation(Line(points={{15,206},{205,206},{205,25},{270,25}}, color={55,80,115}));
  connect(controller_core.y2, fault_compensator.command_in[3]) 
    annotation(Line(points={{15,182},{205,182},{205,15},{270,15}}, color={55,80,115}));
  connect(controller_core.y3, fault_compensator.command_in[4]) 
    annotation(Line(points={{15,158},{205,158},{205,5},{270,5}}, color={55,80,115}));

  // ── Fault compensator → ESC → Motors → Plant ──
  connect(fault_compensator.command_out[1], esc.motor_command_raw[1]) 
    annotation(Line(points={{370,30},{245,30},{245,12},{140,12}}, color={55,80,115}));
  connect(fault_compensator.command_out[2], esc.motor_command_raw[2]) 
    annotation(Line(points={{370,20},{245,20},{245,2},{140,2}}, color={55,80,115}));
  connect(fault_compensator.command_out[3], esc.motor_command_raw[3]) 
    annotation(Line(points={{370,10},{245,10},{245,-8},{140,-8}}, color={55,80,115}));
  connect(fault_compensator.command_out[4], esc.motor_command_raw[4]) 
    annotation(Line(points={{370,0},{245,0},{245,-18},{140,-18}}, color={55,80,115}));

  connect(esc.motor_command[1], motor1.command) 
    annotation(Line(points={{240,30},{300,30},{300,220},{436,220}}, color={55,80,115}));
  connect(esc.motor_command[2], motor2.command) 
    annotation(Line(points={{240,20},{300,20},{300,142},{436,142}}, color={55,80,115}));
  connect(esc.motor_command[3], motor3.command) 
    annotation(Line(points={{240,10},{300,10},{300,64},{436,64}}, color={55,80,115}));
  connect(esc.motor_command[4], motor4.command) 
    annotation(Line(points={{240,0},{300,0},{300,-14},{436,-14}}, color={55,80,115}));

  connect(motor1.command_to_plant, plant.rotor_command[1]) 
    annotation(Line(points={{494,220},{522,220}}, color={55,80,115}));
  connect(motor2.command_to_plant, plant.rotor_command[2]) 
    annotation(Line(points={{494,142},{522,142}}, color={55,80,115}));
  connect(motor3.command_to_plant, plant.rotor_command[3]) 
    annotation(Line(points={{494,64},{522,64}}, color={55,80,115}));
  connect(motor4.command_to_plant, plant.rotor_command[4]) 
    annotation(Line(points={{494,-14},{522,-14}}, color={55,80,115}));

  connect(plant.rotor_speed[1], motor1.speed) 
    annotation(Line(points={{777,220},{805,220},{805,-120},{410,-120},{410,220},{494,220}}, color={130,0,130}));
  connect(plant.rotor_speed[2], motor2.speed) 
    annotation(Line(points={{777,142},{805,142},{805,-120},{410,-120},{410,142},{494,142}}, color={130,0,130}));
  connect(plant.rotor_speed[3], motor3.speed) 
    annotation(Line(points={{777,64},{805,64},{805,-120},{410,-120},{410,64},{494,64}}, color={130,0,130}));
  connect(plant.rotor_speed[4], motor4.speed) 
    annotation(Line(points={{777,-14},{805,-14},{805,-120},{410,-120},{410,-14},{494,-14}}, color={130,0,130}));

  connect(battery.bus_voltage, esc.bus_voltage) 
    annotation(Line(points={{105,30},{140,30}}, color={80,80,80}));
  connect(battery.power_ok, esc.power_ok) 
    annotation(Line(points={{105,20},{140,20}}, color={80,80,80}));
  connect(plant.position, perception.position_raw) 
    annotation(Line(points={{522,150},{700,150},{700,-100},{-400,-100},{-400,30},{-430,30}}, color={0,100,150}));
  connect(perception.gps_position, flight_controller.gps_position) 
    annotation(Line(points={{-330,30},{-145,30}}, color={0,100,150}));
  connect(perception.gps_valid, flight_controller.gps_valid) 
    annotation(Line(points={{-330,-20},{-145,-20}}, color={0,100,150}));
  connect(plant.attitude, flight_controller.attitude_raw) 
    annotation(Line(points={{777,191},{820,191},{820,-80},{-170,-80},{-170,15},{-145,15}}, color={0,100,150}));
  connect(plant.rotor_speed, flight_controller.motor_speed_raw) 
    annotation(Line(points={{777,161},{840,161},{840,-90},{-180,-90},{-180,-5},{-145,-5}}, color={130,0,130}));
  connect(perception.local_position, mission_computer.local_position) 
    annotation(Line(points={{-330,10},{-285,10}}, color={0,100,150}));
  connect(flight_controller.position_est, mission_computer.aircraft_position) 
    annotation(Line(points={{-45,25},{-30,25},{-30,-60},{-300,-60},{-300,25},{-285,25}}, color={100,70,20}));
  connect(perception.obstacle_margin, mission_computer.obstacle_margin) 
    annotation(Line(points={{-330,-5},{-285,-5}}, color={0,100,150}));
  connect(flight_controller.estimator_quality, mission_computer.estimator_quality) 
    annotation(Line(points={{-45,-15},{-30,-15},{-30,-60},{-300,-60},{-300,-15},{-285,-15}}, color={100,70,20}));

  position_ref = reference.position_command;
  position = plant.position;
  attitude = plant.attitude;
  rotor_command[1] = fault_compensator.command_out[1];
  rotor_command[2] = fault_compensator.command_out[2];
  rotor_command[3] = fault_compensator.command_out[3];
  rotor_command[4] = fault_compensator.command_out[4];
