...
model CascadePidContinuousInnerFeedbackSmoke
  "Diagnostic: sampled cascade outer loop with continuous low-level attitude feedback"

  parameter Real controller_sample_period_s = 0.01
    "Sample boundary retained only at the discrete C-function inputs";
...
equation
  ...
  connect(plant.attitude, offline_inner_allocator.attitude_mea) 
    annotation(Line(points = {{113, -45}, {102, -45}, {102, 45}, {10, 45}}, color = {0, 0, 127}));
  ...
  outer_sampled_attitude = sampled_attitude.y;
  ...
  annotation(
    experiment(Algorithm = Dassl, StartTime = 0, StopTime = 0.2, Tolerance = 0.0001, Interval = 0.01),
    Diagram(coordinateSystem(extent = {{-250, -140}, {240, 140}}, grid = {2, 2})),
    __MWORKS(version = "26.3.0"));
end CascadePidContinuousInnerFeedbackSmoke;