...
block ThreeUavPairwiseEcbfReferenceSmoother
  "Continuous first-order governor between raw ECBF corrections and the sampled PX4CTRL reference boundary"

  parameter Real position_time_constant_s(min = 0.02, unit = "s") = 0.20
    "Position-correction smoothing time constant";
  parameter Real velocity_time_constant_s(min = 0.02, unit = "s") = 0.12
    "Velocity-correction smoothing time constant";
  parameter Real acceleration_time_constant_s(min = 0.02, unit = "s") = 0.12
    "Acceleration-correction smoothing time constant";
  ...
  Modelica.Blocks.Interfaces.RealInput raw_safe_velocity_1[3];
  Modelica.Blocks.Interfaces.RealInput raw_safe_acceleration_1[3];
  Modelica.Blocks.Interfaces.RealInput raw_safe_position_2[3];
  Modelica.Blocks.Interfaces.RealInput raw_safe_velocity_2[3];
  Modelica.Blocks.Interfaces.RealInput raw_safe_acceleration_2[3];
  Modelica.Blocks.Interfaces.RealInput raw_safe_position_3[3];
  Modelica.Blocks.Interfaces.RealInput raw_safe_velocity_3[3];
  Modelica.Blocks.Interfaces.RealInput raw_safe_acceleration_3[3];
...
protected
  ...
  Real rawSafeVelocity[3, 3];
  Real rawSafeAcceleration[3, 3];
  Real positionCorrectionTarget[3, 3];
  Real velocityCorrectionTarget[3, 3];
  Real accelerationCorrectionTarget[3, 3];
  Real positionCorrection[3, 3](each start = 0, each fixed = true);
  Real velocityCorrection[3, 3](each start = 0, each fixed = true);
  Real accelerationCorrection[3, 3](each start = 0, each fixed = true);
...
equation
  for axis in 1:3 loop
    ...
    rawSafeVelocity[1, axis] = raw_safe_velocity_1[axis];
    rawSafeAcceleration[1, axis] = raw_safe_acceleration_1[axis];
    rawSafePosition[2, axis] = raw_safe_position_2[axis];
    rawSafeVelocity[2, axis] = raw_safe_velocity_2[axis];
    rawSafeAcceleration[2, axis] = raw_safe_acceleration_2[axis];
    rawSafePosition[3, axis] = raw_safe_position_3[axis];
    rawSafeVelocity[3, axis] = raw_safe_velocity_3[axis];
    rawSafeAcceleration[3, axis] = raw_safe_acceleration_3[axis];
  end for;
  ...
  for vehicle in 1:3 loop
    for axis in 1:3 loop
      ...
      velocityCorrectionTarget[vehicle, axis] = rawSafeVelocity[vehicle, axis]
        - nominalVelocity[vehicle, axis];
      accelerationCorrectionTarget[vehicle, axis] = rawSafeAcceleration[vehicle, axis]
        - nominalAcceleration[vehicle, axis];
      der(positionCorrection[vehicle, axis]) = (positionCorrectionTarget[vehicle, axis]
        - positionCorrection[vehicle, axis]) / position_time_constant_s;
      der(velocityCorrection[vehicle, axis]) = (velocityCorrectionTarget[vehicle, axis]
        - velocityCorrection[vehicle, axis]) / velocity_time_constant_s;
      der(accelerationCorrection[vehicle, axis]) = (accelerationCorrectionTarget[vehicle, axis]
        - accelerationCorrection[vehicle, axis]) / acceleration_time_constant_s;
    end for;
  end for;
  ...
  for axis in 1:3 loop
    ...
    safe_acceleration_1[axis] = nominalAcceleration[1, axis] + accelerationCorrection[1, axis];
    ...
    safe_acceleration_2[axis] = nominalAcceleration[2, axis] + accelerationCorrection[2, axis];
    ...
    safe_acceleration_3[axis] = nominalAcceleration[3, axis] + accelerationCorrection[3, axis];
  end for;
