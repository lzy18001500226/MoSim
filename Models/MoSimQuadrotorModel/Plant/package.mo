...
package Plant "四旋翼无人机模型"
  ...
  package Mechanics "机械多体库"
    ...
    model QuadrotorBody "四旋翼机身"
      ...
      Modelica.Mechanics.MultiBody.Parts.BodyShape body(
        ...
        I_31 = 0,
        I_32 = 0,
        shapeType = "modelica://MoSimQuadrotorModel/Plant/Resources/Visualization/sunray150_mid360_body.stl",
        r_shape = {0, 0, 0.0525},
    ...
    model Rotor "旋翼模型"
      Modelica.Mechanics.MultiBody.Parts.BodyShape propellers1(
        ...
        I_31 = 0,
        I_32 = 0,
        shapeType = "modelica://MoSimQuadrotorModel/Plant/Resources/Visualization/sunray150_mid360_propeller.stl",
        r_shape = {0, 0, 0},
    ...
    model QuadChassis "四旋翼本体+地面"
      annotation (Icon(coordinateSystem(extent = {{-200.0, -200.0}, {200.0, 200.0}},
        ...
        fileName = "modelica://MoSimQuadrotorModel/Plant/Resources/Images/Sunray150.png"), Line(origin = {0.0, 35.0},
        rotation = 270.0,
      ...
      Modelica.Mechanics.MultiBody.Parts.FixedRotation fixedRotation(r = {0, 0, -0.05},
        animation = false) annotation (Placement(transformation(origin = {158.25275041021322, 90.48433844745136},
          extent = {{-10.0, -10.0}, {10.0, 10.0}})));
      Modelica.Mechanics.MultiBody.Visualizers.FixedShape2 fixedShape2_1(shapeType = "modelica://MoSimQuadrotorModel/Plant/Resources/Visualization/Jichang1.stl",
  ...
  package GroundModel "地面模型"
    ...
    equation
      ...
      x = max((R1 - P[3]), 0);
      //接触阻尼方程
      F_C = MoSimQuadrotorModel.Plant.Utilities.Functions.Step(x, 0, 0, delta, C);
      //接触弹力方程
      F[1] = 0;
      F[2] = 0;
      F[3] = if x > 0 then max((K * x ^ n - F_C * V[3]), 0) else 0;
      //接触摩擦力方程
      f[1] = if abs(V[1]) > 0 then MoSimQuadrotorModel.Plant.Utilities.Functions.Friction(F[3], V[1], V_s, Cst, Vtr, Cdy) else 0;
      f[2] = if abs(V[2]) > 0 then MoSimQuadrotorModel.Plant.Utilities.Functions.Friction(F[3], V[3], V_s, Cst, Vtr, Cdy) else 0;
      f[3] = 0 * V[3];
      //参数传递方程
      V = R_v.v_rel;
      P = R_p.r_rel;
...
(to be further processed with blocks of the Modelica.Blocks library).
  ...
  annotation (Icon(coordinateSystem(extent = {{-100.0, -100.0}, {100.0, 100.0}},
    ...
    fileName = "modelica://MoSimQuadrotorModel/Plant/Resources/Images/Sunray150.png")}));
  package Utilities "附件"
    ...
    package Functions "函数库"
      ...
      function Friction "摩擦力"
        ...
        //中间变量
      algorithm
        F_f := N * MoSimQuadrotorModel.Plant.Utilities.Functions.Step(V, -V_s, -1, V_s, 1) * MoSimQuadrotorModel.Plant.Utilities.Functions.Step(abs(V), V_s, Cst, Vtr, Cdy);
      end Friction;
    end Functions;
  end Utilities;
end Plant;
