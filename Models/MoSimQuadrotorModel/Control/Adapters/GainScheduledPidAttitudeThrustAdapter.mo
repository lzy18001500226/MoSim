...
model GainScheduledPidAttitudeThrustAdapter
  "Gain-scheduled PID core adapted to the shared ATTITUDE_THRUST boundary"
  ...
  MoSimQuadrotorModel.Control.Bridges.GainScheduledPidEquationBridge core;
...
equation
  ...
  // Match the accepted px4ctrl mapping: bounded magnitude of position error.
  core.schedule_x_in = min(max(abs(position_ref[1] - position_mea[1]), 0), 1);
  core.schedule_y_in = min(max(abs(position_ref[2] - position_mea[2]), 0), 1);
  core.schedule_z_in = min(max(abs(position_ref[3] - position_mea[3]), 0), 1);
...
end GainScheduledPidAttitudeThrustAdapter;