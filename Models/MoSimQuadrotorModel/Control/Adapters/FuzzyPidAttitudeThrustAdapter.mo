...
model FuzzyPidAttitudeThrustAdapter
  "Bounded fuzzy-scheduled PID core adapted to the shared ATTITUDE_THRUST boundary"
  ...
  MoSimQuadrotorModel.Control.Bridges.FuzzyPidEquationBridge core;
...
equation
  ...
  // Match the accepted px4ctrl mapping: bounded signed position error.
  core.fuzzy_error_x_in = min(max(position_ref[1] - position_mea[1], -1), 1);
  core.fuzzy_error_y_in = min(max(position_ref[2] - position_mea[2], -1), 1);
  core.fuzzy_error_z_in = min(max(position_ref[3] - position_mea[3], -1), 1);
...
end FuzzyPidAttitudeThrustAdapter;
