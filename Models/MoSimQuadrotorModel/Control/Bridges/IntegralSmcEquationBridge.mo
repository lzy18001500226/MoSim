...
model IntegralSmcEquationBridge
  "Equation bridge for the graphical integral sliding-mode outer-loop law"
  ...
  parameter Real integral_surface_gain[3] = {0.2, 0.2, 0.25};
  parameter Real integral_limit[3] = {1.5, 1.5, 1.8};
...
protected
  discrete Real integral_memory[3](each start = 0, each fixed = true);
  Real position_error[3];
  Real velocity_error[3];
  Real reference_acceleration[3];
  Real integral_term[3];
...
equation
  ...
  for axis in 1:3 loop
    // Source: UnitDelay + 0.01 * position_error followed by saturation.
    integral_term[axis] = min(max(integral_memory[axis] + dt * position_error[axis], -integral_limit[axis]), integral_limit[axis]);
    sliding_surface[axis] = velocity_error[axis] + position_surface_gain[axis] * position_error[axis] + integral_surface_gain[axis] * integral_term[axis];
  ...
  when sample(sample_time_s, sample_time_s) then
    for axis in 1:3 loop
      integral_memory[axis] = min(max(pre(integral_memory[axis]) + dt * position_error[axis], -integral_limit[axis]), integral_limit[axis]);
    end for;
  end when;
