...
model AdaptiveBacksteppingEquationBridge
  "Adaptive-backstepping law copied from the readable P2 graphical core"
  ...
  parameter Real adaptive_increment_gain[3] = {0.0035, 0.0035, 0.0045};
  parameter Real adaptive_limit[3] = {1.0, 1.0, 1.2};
  ...
  output Real adaptive_disturbance_x_out;
  output Real adaptive_disturbance_y_out;
  output Real adaptive_disturbance_z_out;

protected
  Modelica.Blocks.Discrete.UnitDelay adaptive_state[3](
    each samplePeriod = sample_time_s,
    each y_start = 0);
  Real position_error[3];
  Real velocity_error[3];
  Real sliding_surface[3];
  Real adaptive_disturbance[3];
...
equation
  ...
  for axis in 1:3 loop
    sliding_surface[axis] = velocity_error[axis]
      + position_gain[axis] * position_error[axis];
    adaptive_disturbance[axis] = min(max(adaptive_state[axis].y
  ...
  adaptive_disturbance_x_out = if enabled then adaptive_disturbance[1] else 0;
  adaptive_disturbance_y_out = if enabled then adaptive_disturbance[2] else 0;
  adaptive_disturbance_z_out = if enabled then adaptive_disturbance[3] else 0;

  annotation(__MWORKS(version = "26.3.0"));
end AdaptiveBacksteppingEquationBridge;
