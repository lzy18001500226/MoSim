...
model NonsingularTerminalSmcEquationBridge
  "Equation bridge for the graphical nonsingular terminal sliding-mode law"

  parameter Real position_surface_gain[3] = {1.2, 1.2, 1.4};
  parameter Real position_power_exponent[3] = {1.5, 1.5, 1.5};
  parameter Real terminal_shape_gain[3] = {0.35, 0.35, 0.4};
...
equation
  ...
  for axis in 1:3 loop
    sliding_surface[axis] = velocity_error[axis] + position_surface_gain[axis] * position_error[axis] + terminal_shape_gain[axis] * sign(position_error[axis]) * abs(position_error[axis]) ^ position_power_exponent[axis];
...
end NonsingularTerminalSmcEquationBridge;
