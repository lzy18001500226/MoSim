...
model AdaptiveSmcEquationBridge
  "Equation bridge for the graphical adaptive sliding-mode outer-loop law"
  ...
  parameter Real adaptive_threshold[3] = {0.05, 0.05, 0.05};
  parameter Real adaptation_rate[3] = {0.008, 0.008, 0.01};
  parameter Real adaptive_gain_min[3] = {2.2, 2.2, 2.8};
  parameter Real adaptive_gain_max[3] = {5.0, 5.0, 6.0};
...
protected
  discrete Real adaptive_gain_memory[3](start = adaptive_gain_min, each fixed = true);
  Real position_error[3];
  Real velocity_error[3];
  Real reference_acceleration[3];
  Real adaptive_gain[3];
...
equation
  ...
  for axis in 1:3 loop
    ...
    adaptive_gain[axis] = adaptive_gain_memory[axis];
    desired_acceleration[axis] = reference_acceleration[axis] + velocity_reaching_gain[axis] * velocity_error[axis] + surface_feedback_gain[axis] * sliding_surface[axis] + adaptive_gain[axis] * min(max(boundary_normalization[axis] * sliding_surface[axis], -1.0), 1.0) + (if axis == 3 then gravity_mps2 else 0);
  end for;
  when sample(sample_time_s, sample_time_s) then
    for axis in 1:3 loop
      adaptive_gain_memory[axis] = min(max(pre(adaptive_gain_memory[axis]) + adaptation_rate[axis] * (abs(sliding_surface[axis]) - adaptive_threshold[axis]), adaptive_gain_min[axis]), adaptive_gain_max[axis]);
    end for;
  end when;
