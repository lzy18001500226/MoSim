...
model AwffL1IndiOuterLoopGraphical
  ...
  annotation(__MWORKS(version="26.3.0",modelType=Control,PortArrangement(Left(x_error,y_error,z_error,z_ref_rate),Right(pitch_ref,roll_ref,thrust_ref)),BlockSystem(blockKind=BlockKind.userModel,SampleTime(auto=true,group="")=0.01,OutputInterval=0.01),SysblockVersion="1.0"),
    ...
    Diagram(coordinateSystem(extent={{-220,-140},{220,140}},grid={2,2})),experiment(Algorithm=Euler,IntegratorStep=0.01,Interval=0.01,StartTime=0,StopTime=1,StoreEventValue=0));
...
equation
  connect(x_error,x_rate_gain.u) annotation(Line(points={{-190,90},{-175,90},{-175,105},{-157,105}},color={0,0,0}));
  connect(x_error,x_decay_gain.u) annotation(Line(points={{-190,90},{-175,90},{-175,75},{-157,75}},color={0,0,0}));
  connect(x_error,x_kp_gain.u) annotation(Line(points={{-190,90},{-128,90},{-128,125},{-107,125}},color={0,0,0}));
  ...
  connect(x_rate_gain.y,x_kd_gain.u) annotation(Line(points={{-133,105},{-118,105},{-118,125},{-57,125}},color={0,0,0}));
  ...
  connect(y_error,y_rate_gain.u) annotation(Line(points={{-190,30},{-175,30},{-175,35},{-157,35}},color={0,0,0}));
  connect(y_error,y_decay_gain.u) annotation(Line(points={{-190,30},{-175,30},{-175,5},{-157,5}},color={0,0,0}));
  connect(y_error,y_kp_gain.u) annotation(Line(points={{-190,30},{-128,30},{-128,55},{-107,55}},color={0,0,0}));
  ...
  connect(y_rate_gain.y,y_kd_gain.u) annotation(Line(points={{-133,35},{-118,35},{-118,55},{-57,55}},color={0,0,0}));
  ...
  connect(z_error,z_rate_gain.u) annotation(Line(points={{-190,-30},{-175,-30},{-175,-40},{-157,-40}},color={0,0,0}));
  connect(z_error,z_decay_gain.u) annotation(Line(points={{-190,-30},{-175,-30},{-175,-70},{-157,-70}},color={0,0,0}));
  connect(z_error,z_kp_gain.u) annotation(Line(points={{-190,-30},{-128,-30},{-128,-105},{-107,-105}},color={0,0,0}));
  ...
  connect(z_rate_gain.y,z_kd_gain.u) annotation(Line(points={{-133,-40},{-120,-40},{-120,-105},{-57,-105}},color={0,0,0}));
...
end AwffL1IndiOuterLoopGraphical;