...
model Px4CtrlCoreEquationBridge
  "px4ctrl ATTITUDE_THRUST core equation bridge for Golden Slice G6"
  ...
  annotation(__MWORKS(version="26.3.0",modelType=Control,BlockSystem(blockKind=BlockKind.userModel,SampleTime(auto=true,group="")=0.01,OutputInterval=0.01),SysblockVersion="1.0"),
    experiment(DoublePrecision=true,Algorithm=Euler,IntegratorStep=0.01,Interval=0.01,StartTime=0,StopTime=0.03,StoreEventValue=0),
...
equation
  ...
  yaw_odom = atan2(2.0 * (nq_x * nq_y + nq_w * nq_z), nq_w * nq_w + nq_x * nq_x - nq_y * nq_y - nq_z * nq_z);
