...
model FactoryTraceIso20RollPitchYawEstimatorSmoke
  "Yaw decoupling 20: project-owned first-order roll+pitch+yaw attitude extraction before controller inputs"
  ...
  parameter Real attitude_extraction_T = 0.03
    "Project-owned probe extraction time constant; not a controller gain";

  MoSimQuadrotorModel.Vehicle.Mechanics.QuadChassis quadChassisTest20_1(
  ...
  Modelica.Blocks.Sources.RealExpression roll_estimated(y = roll_est_state);
  Modelica.Blocks.Sources.RealExpression pitch_estimated(y = pitch_est_state);
  Modelica.Blocks.Sources.RealExpression yaw_estimated(y = yaw_est_state);
  ...
  Real roll_est_state(start = 0.0, fixed = true);
  Real pitch_est_state(start = 0.0, fixed = true);
  Real yaw_est_state(start = 0.0, fixed = true);
  Real motor_cmd_1;
  Real motor_cmd_2;
  Real motor_cmd_3;
  Real motor_cmd_4;
...
equation
  connect(actuator1_1.flange_a, quadChassisTest20_1.flange_a);
  connect(actuator1_2.flange_a, quadChassisTest20_1.flange_a1);
  connect(actuator1_3.flange_a, quadChassisTest20_1.flange_a2);
  connect(actuator1_4.flange_a, quadChassisTest20_1.flange_a3);
  connect(quadChassisTest20_1.frame_a, sensors1_1.frame_a);
  ...
  der(roll_est_state) = (sensors1_1.AngleMea[1] - roll_est_state) / attitude_extraction_T;
  der(pitch_est_state) = (sensors1_1.AngleMea[2] - pitch_est_state) / attitude_extraction_T;
  der(yaw_est_state) = (sensors1_1.AngleMea[3] - yaw_est_state) / attitude_extraction_T;
  ...
  connect(roll_estimated.y, controller3_2.roll_mea);
  connect(pitch_estimated.y, controller3_2.pitch_mea);
  connect(yaw_estimated.y, controller3_2.yaw_mea);
