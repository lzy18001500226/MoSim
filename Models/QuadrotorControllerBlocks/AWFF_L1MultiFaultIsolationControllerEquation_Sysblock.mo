model AWFF_L1MultiFaultIsolationControllerEquation_Sysblock
  "Deprecated compatibility alias; canonical implementation is MoSimQuadrotorModel.Controllers.Sysblocks.AWFF_L1MultiFaultIsolationControllerEquation_Sysblock"
  extends MoSimQuadrotorModel.Controllers.Sysblocks.AWFF_L1MultiFaultIsolationControllerEquation_Sysblock;
  annotation(__MWORKS(hide=true));
end AWFF_L1MultiFaultIsolationControllerEquation_Sysblock;
