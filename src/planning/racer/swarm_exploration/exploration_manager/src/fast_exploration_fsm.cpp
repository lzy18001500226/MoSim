...
void FastExplorationFSM::init(ros::NodeHandle& nh) {
  ...
  nh.param("fsm/mosim_d3_pair_opt_after_trigger_only",
      fp_->mosim_d3_pair_opt_after_trigger_only_, false);
...
void FastExplorationFSM::optTimerCallback(const ros::TimerEvent& e) {
  ...
  if (fp_->mosim_d3_pair_opt_after_trigger_only_ && state_ == WAIT_TRIGGER) {
    ROS_WARN_THROTTLE(2.0,
        "MoSim D3 skip pair opt in WAIT_TRIGGER for drone %d until planner trigger",
        getId());
    return;
  }
  ...
  if (ego_ids.empty() || other_ids.empty()) {
    ROS_WARN("MoSim D3 reject pair opt %d & %d: empty allocation ego=%zu other=%zu",
        getId(), select_id, ego_ids.size(), other_ids.size());
    return;
  }
  ...
  if (inconsistent) {
    ROS_WARN("MoSim D3 reject pair opt %d & %d: inconsistent grid path", getId(), select_id);
    return;
  }
...
void FastExplorationFSM::optMsgCallback(const exploration_manager::PairOptConstPtr& msg) {
  ...
  if (fp_->mosim_d3_pair_opt_after_trigger_only_ && state_ == WAIT_TRIGGER) {
    ROS_WARN("MoSim D3 reject pre-trigger pair opt request from %d to %d",
...
void FastExplorationFSM::optResMsgCallback(
  ...
  if (fp_->mosim_d3_pair_opt_after_trigger_only_ && state_ == WAIT_TRIGGER) {
    ROS_WARN("MoSim D3 ignore pre-trigger pair opt response from %d to %d",
        msg->from_drone_id, getId());
    return;
  }
