...
__global__ void render(float3 *data_devptr, Parameter *para_devptr,
  ...
  float3 trans_point;
  trans_point.x =
      my_point.x * para.r[0][0] + my_point.y * para.r[0][1] + my_point.z * para.r[0][2] + para.t[0];
  trans_point.y =
      my_point.x * para.r[1][0] + my_point.y * para.r[1][1] + my_point.z * para.r[1][2] + para.t[1];
  trans_point.z =
      my_point.x * para.r[2][0] + my_point.y * para.r[2][1] + my_point.z * para.r[2][2] + para.t[2];
  ...
  int r = para.radius * para.fx / dist + 0.5f;
  for (int i = -r; i <= r; i++)
    for (int j = -r; j <= r; j++) {
...
DepthRender::DepthRender()
    : cloud_size(0), host_cloud_ptr(NULL), dev_cloud_ptr(NULL), has_devptr(false) {}
...
void DepthRender::set_para(float _fx, float _fy, float _cx, float _cy, int _width, int _height, float _radius) {
  parameter.fx = _fx;
  parameter.fy = _fy;
  parameter.cx = _cx;
  parameter.cy = _cy;
...
void DepthRender::set_data(vector<float> &cloud_data) {
  ...
  for (int i = 0; i < cloud_size; i++)
    host_cloud_ptr[i] =
        make_float3(cloud_data[3 * i], cloud_data[3 * i + 1], cloud_data[3 * i + 2]);
...
void DepthRender::render_pose(double *transformation, int *host_ptr) {
  ...
  cudaError err =
      cudaMemcpy(parameter_devptr, &parameter, sizeof(Parameter), cudaMemcpyHostToDevice);
  if (err != cudaSuccess)
    throw CudaException("DeviceLinear: unable to copy data from host to device.", err);
