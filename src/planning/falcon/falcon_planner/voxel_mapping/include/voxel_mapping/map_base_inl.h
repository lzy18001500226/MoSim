...
void MapBase<VoxelType>::indexToPosition(const VoxelIndex &idx, Position &pos) {
  for (int i = 0; i < 3; ++i)
    pos(i) = ((double)idx(i) + 0.5) * map_config_.resolution_ + map_config_.map_min_(i);
}
...
VoxelAddress MapBase<VoxelType>::positionToAddress(const Position &pos) {
  return indexToAddress(positionToIndex(pos));
}
...
template <typename VoxelType>
void MapBase<VoxelType>::addressToIndex(const VoxelAddress &addr, VoxelIndex &idx) {
  idx[0] = addr / (map_config_.map_size_idx_(1) * map_config_.map_size_idx_(2));
  idx[1] = (addr % (map_config_.map_size_idx_(1) * map_config_.map_size_idx_(2))) /
           map_config_.map_size_idx_(2);
  idx[2] = addr % map_config_.map_size_idx_(2);
}

template <typename VoxelType>
VoxelIndex MapBase<VoxelType>::addressToIndex(const VoxelAddress &addr) {
  VoxelIndex idx;
...
template <typename VoxelType>
void MapBase<VoxelType>::addressToPosition(const VoxelAddress &addr, Position &pos) {
  indexToPosition(addressToIndex(addr), pos);
}

template <typename VoxelType>
Position MapBase<VoxelType>::addressToPosition(const VoxelAddress &addr) {
  Position pos;
  indexToPosition(addressToIndex(addr), pos);
  return pos;
}
...
Position MapBase<VoxelType>::closestPointInMap(const Position &point,
  ...
  return sensor_position + (min_t - 0.1) * diff;
}
...
template <typename VoxelType> VoxelType MapBase<VoxelType>::getVoxel(const VoxelAddress &addr) {
  CHECK_LT(addr, map_data_->data.size()) << "Address out of range: " << addr;
