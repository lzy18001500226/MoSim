...
PerceptionUtils::PerceptionUtils(ros::NodeHandle& nh) {
  ...
  nh.param("perception_utils/omni_horizontal", omni_horizontal_, false);
  ROS_WARN("[FUEL_PERCEPTION] omni_horizontal=%s", omni_horizontal_ ? "true" : "false");
...
bool PerceptionUtils::insideFOV(const Vector3d& point) {
  ...
  dir.normalize();
  const int normal_count = omni_horizontal_ ? 2 : static_cast<int>(normals_.size());
