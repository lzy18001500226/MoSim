...
void bsplineCallback(const bspline::BsplineConstPtr& msg) {
  ...
  start_time_ = align_start_time_to_receive_ ? received_time : msg->start_time;
  traj_id_ = msg->traj_id;
  if (align_start_time_to_receive_) {
    ROS_WARN("[Traj server]: align traj_id %d start_time from %.6f to receive time %.6f",
             traj_id_, msg->start_time.toSec(), start_time_.toSec());
  }
...
int main(int argc, char** argv) {
  ...
  nh.param("traj_server/align_start_time_to_receive", align_start_time_to_receive_, false);
