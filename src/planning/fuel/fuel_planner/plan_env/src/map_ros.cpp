...
void MapROS::init() {
  ...
  node_.param("map_ros/publish_local_map", publish_local_map_, true);
  node_.param("map_ros/all_map_publish_period", all_map_publish_period_, 0.1);
  all_map_publish_period_ = max(0.05, all_map_publish_period_);
  node_.param("map_ros/frame_id", frame_id_, string("world"));
  node_.param("map_ros/cloud_free_space_rays_enable", cloud_free_space_rays_enable_, false);
  node_.param("map_ros/cloud_free_space_ray_count", cloud_free_space_ray_count_, 72);
  node_.param("map_ros/cloud_free_space_ray_length_margin", cloud_free_space_ray_length_margin_, 0.5);
...
void MapROS::cloudPoseCallback(const sensor_msgs::PointCloud2ConstPtr& msg,
  ...
  if (cloud_free_space_rays_enable_ && map_->mp_->max_ray_length_ > 0.0) {
    const int ray_count = std::max(4, cloud_free_space_ray_count_);
    const double ray_len = map_->mp_->max_ray_length_ + std::max(0.1, cloud_free_space_ray_length_margin_);
    const double pi = 3.14159265358979323846;
    const double pitches[] = {0.0, -0.12, 0.12};
    for (double pitch : pitches) {
      const double cp = std::cos(pitch);
      const double sp = std::sin(pitch);
      for (int i = 0; i < ray_count; ++i) {
        const double yaw = 2.0 * pi * static_cast<double>(i) / static_cast<double>(ray_count);
        const Eigen::Vector3d endpoint =
            camera_pos_ + ray_len * Eigen::Vector3d(std::cos(yaw) * cp, std::sin(yaw) * cp, sp);
        pcl::PointXYZ pt;
        pt.x = endpoint.x();
        pt.y = endpoint.y();
        pt.z = endpoint.z();
