...
void Astar::init(ros::NodeHandle& nh, const EDTEnvironment::Ptr& env) {
  ...
  nh.param("astar/diagnose_rejection", diagnose_rejection_, false);
  nh.param("astar/diagnose_rejection_throttle_s", diagnose_rejection_throttle_s_, 1.0);
...
int Astar::search(const Eigen::Vector3d& start_pt, const Eigen::Vector3d& end_pt) {
  int reject_out_of_box = 0;
  int reject_inflate = 0;
  int reject_unknown = 0;
  int reject_ckpt_inflate = 0;
  int reject_ckpt_unknown = 0;
  int reject_closed = 0;
  int reject_open_not_better = 0;
  auto logNoPath = [&](const char* reason) {
    if (!diagnose_rejection_) return;
    ROS_WARN_THROTTLE(
        diagnose_rejection_throttle_s_,
        "[FUEL_ASTAR_DIAG] no_path reason=%s start=(%.3f %.3f %.3f) end=(%.3f %.3f %.3f) "
        "use_nodes=%d iter=%d reject_outbox=%d reject_inflate=%d reject_unknown=%d "
        "reject_ckpt_inflate=%d reject_ckpt_unknown=%d reject_closed=%d reject_open_not_better=%d",
        reason, start_pt.x(), start_pt.y(), start_pt.z(), end_pt.x(), end_pt.y(), end_pt.z(),
        use_node_num_, iter_num_, reject_out_of_box, reject_inflate, reject_unknown,
        reject_ckpt_inflate, reject_ckpt_unknown, reject_closed, reject_open_not_better);
  };
    ...
    for (double dx = -resolution_; dx <= resolution_ + 1e-3; dx += resolution_)
      for (double dy = -resolution_; dy <= resolution_ + 1e-3; dy += resolution_)
        for (double dz = -resolution_; dz <= resolution_ + 1e-3; dz += resolution_) {
          ...
          if (edt_env_->sdf_map_->getOccupancy(nbr_pos) == SDFMap::UNKNOWN) {
            ++reject_unknown;
            continue;
          }
