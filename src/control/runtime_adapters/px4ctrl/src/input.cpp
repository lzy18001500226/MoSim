...
void Odom_Data_t::init_config()
{
    if (config_initialized)
        return;

    ros::NodeHandle nh("~");
    nh.param<std::string>("odom_velocity_frame", velocity_frame, "world");

    transform_velocity_to_world = (velocity_frame == "body" || velocity_frame == "base_link");
    if (transform_velocity_to_world)
    {
        ROS_WARN_STREAM("[px4ctrl] odom_velocity_frame=" << velocity_frame
                        << "; rotating odometry twist.linear into world frame before control.");
    }
    else if (velocity_frame == "world" || velocity_frame == "map")
    {
        ROS_INFO_STREAM("[px4ctrl] odom_velocity_frame=" << velocity_frame
                        << "; using odometry twist.linear directly.");
    }
    else
    {
        ROS_ERROR_STREAM("[px4ctrl] Unsupported odom_velocity_frame=" << velocity_frame
                         << "; expected world/map/body/base_link. Falling back to world.");
        velocity_frame = "world";
        transform_velocity_to_world = false;
    }
...
void Odom_Data_t::feed(nav_msgs::OdometryConstPtr pMsg)
{
    ...
    if (transform_velocity_to_world)
    {
        v = q.normalized().toRotationMatrix() * v;
    }

    // Legacy compile-time switch. Prefer the runtime odom_velocity_frame
    // parameter so each run records the odometry velocity semantics.
    // #define VEL_IN_BODY
