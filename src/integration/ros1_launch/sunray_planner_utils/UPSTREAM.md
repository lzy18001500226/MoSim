# sunray_planner_utils Source Provenance

- Upstream repository and revision: not recoverable from the retained imported
  Sunray snapshot. Do not represent this copy as pinned to an upstream Git
  commit until a source-to-upstream comparison supplies one.
- Imported source:
  `References/Sunray/General_Module/sunray_planner_utils`
- Retained raw snapshot and imported deliverable payload: 40 files, tree
  SHA-256 `ea21ddffacb2f555eb68d1ae38fd8c5be3f622e1ca1e5a8a35bda6ee3105e697`.
  There are no generated-directory, cache, build, media, or editor-backup
  exclusions in this component.
- Last project commit affecting the retained imported path:
  `0bdabf6b0dcf511367ae56c1afeabad22d674c91`
  (`chore: normalize MoSim project structure`, 2026-05-24).
- License evidence: `package.xml` declares `TODO` and the retained component
  has no standalone license file. A release-license audit is required before
  redistribution.
- Build-layout constraint: `CMakeLists.txt` includes the legacy sibling path
  `${PROJECT_SOURCE_DIR}/../sunray_common/common_lib`. The copy is therefore
  not independently activatable until the Sunray common dependency and all
  consumers are audited together.

The 40-file deliverable payload is a byte-for-byte copy of the retained source.
`.gitattributes`, `UPSTREAM.md`, and `PATCHES.md` are migration metadata
outside that payload. Its registry state is `copied_pending_activation`: the
legacy path remains the only active runtime path until its ROS1 overlay,
entrypoints, source revision, license evidence, and sibling dependency are
validated.
