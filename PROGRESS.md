...
| AirSim batch migration | `AirSimMigrationCoordinator` + `AirSimGitBatchOwner` | running | `main` is synced with `origin/main` at `f7dd12c922`. Pushed: `AirSim`, `AirSim360`, `PEDRA`, `PegasusSimulator`, `ProjectAirSim`, `UESVONavigation-develop`, `unrealcv-5.2`, and Cosys-AirSim source subset. Remaining local/ignored by policy: `carla-ue5-dev`, `IsaacSim`, `spear`, and Cosys-AirSim 50-100 MB tutorial/content assets. Continue only with small path-scoped batches; do not retry old aggregate commits. |
