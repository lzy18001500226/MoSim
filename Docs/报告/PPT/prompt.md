...
- Dashed arrows showing gradient backpropagation from ESDF to trajectory parameterization.
- Red arrows showing constraint violation feedback from constraints box to optimization layer.

Negative constraints:
Do not use photo-realistic RViz visualizations. Do not use actual trajectory animation frames. Do not use decorative optimization convergence plots. Do not use curved arrows; only straight orthogonal connectors. Do not use gradient fills on major boxes. Keep distance field as gradient heatmap, B-spline as smooth curve, and gradient vectors as small arrows.
```
...
## PPT-20 ECBF Safety Supervisor and Degradation Handling

```text
Figure Subject:
ECBF safety supervisor and degradation handling flow: real-time collision detection and reference correction.

Diagram type:
Safety architecture flowchart.

Layout:
16:9 landscape, left-to-right safety monitoring pipeline.
...
- Characteristics box: light yellow #FFF9E6
...
Negative constraints:
Do not use photo-realistic UAV models. Do not use actual flight trajectory plots. Do not use decorative safety icons (shields, warning signs). Arrows must be orthogonal or 45 degrees, no curves. Do not use gradient fills on major boxes. Do not use drop shadows on decision diamond.
```
