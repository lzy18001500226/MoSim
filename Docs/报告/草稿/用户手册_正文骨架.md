...
MoSim Studio 是任务配置与操作引导界面，串联模型验证和运行时复现的分段入口；它不替代 MWORKS、Syslab、ROS/Gazebo 或 QGC 的原生操作与验收。首次使用时，无论从 GitHub 克隆还是从百度网盘解压源码，都先把 `MOSIM_ROOT` 设置为同时包含`AGENTS.md`、`Models/` 和 `Config/` 的 `MoSim` 目录。不要把外层下载目录或重复嵌套的 `MoSim/MoSim` 目录当作项目根。优先复现 APP 和 MWORKS 单机仿真时，只需要 Windows/目标平台的源码包、已授权的 MWORKS Syslab、`Models/`、`Config/`和 `Scripts/`；Gazebo、ROS、PX4、QGC 与大型运行资产属于可选运行时扩展。
...
Sunray150 的任务参考、px4ctrl 控制层、控制分配、机体与反馈在 MWORKS 正式模型链路中组成闭环，如图 1-1 所示。

![Sunray150 系统模型关系](图/云纵150/架构图.png)

图 1-1　Sunray150 px4ctrl 系统模型关系
...
| Studio 可手动打开入口           | 路由表 48 条，其中 48 条 available=true                       | Config/control_platform/model_studio_task_routes_v1.toml                                      |
| FormalRunner 文件候选           | 53 个                                                         | Models/MoSimQuadrotorModel/Experiment/Runners/Formal                                          |
| 名义 ClimbPath 50 s（正文当前汇总） | 30/48 达到门限；18/48 未达到判据或未形成完整 50 s 记录（9 条终端误差超限、8 条超时、1 条 MWORKS 原生仿真提前终止） | Docs/报告/仿真分析报告_正文骨架.md 第七章；Results/control_platform/phase2_full_48_climbpath/g3_repair/ |
...
正文按 48 条控制器路线组织：46 条算法控制器加 Official PID、px4ctrl 两条基线。当前名义 ClimbPath 主口径是 30/48 达到门限；其余 18 条包括 9 条终端误差超限、8 条执行超时和 1 条 MWORKS 原生仿真提前终止。`pole_placement_luenberger` 已通过当前 CheckModel 并完成 50 s，但其终端位置误差为 `402.1409427651827 m`，因此属于前一类性能失败。历史 G3 的 `improved_pid` 也按 MWORKS 原生仿真提前终止保留，但不计入当前 48 条分母；当前目录的 `adaptive_mpc` 仍计入当前 48 条并保持原生 `simulate_failed` 状态，直到最终源面复验完成。48 条可用入口和 53 个 Runner 文件候选属于不同统计层，不能替代当前状态。
当前固定目录对账为 **30/48 通过，18/48 完成失败，0/48 未运行**。冻结历史 G3 快照来自 `G3_STATUS.json`，保留其原始逐条状态字段但不替代当前目录分类；本手册的当前分类以正文第七章为准。完整数字、证据类别和限制见 Docs/Design/报告手册交付证据总账_P0_20260731.md 与 Docs/报告/审计/当前目录48条ClimbPath口径对齐_20260801.md。
...
从零配置 ROS1/Gazebo/PX4/MAVROS 或 UE5 时，不应只根据 Studio、QGC、UE 或 Gazebo
窗口是否出现作判断。项目已将这两条可选环境的版本、源码路径、构建顺序、预检、资产包和
常见阻断整理为[外部运行时环境部署与复现](../Workflows/外部运行时环境部署与复现.md)：

表 2-2　可选外部运行时环境与首检边界
| 使用目标 | 必需环境 | 完成后的第一检查 | 不代表的结论 |
| --- | --- | --- | --- |
| C99 单机运行时复现 | WSL2 Ubuntu-20.04、ROS Noetic、Gazebo Classic、PX4、MAVROS 和项目内 src 源码 | Scripts/cmd/00_准备C99单机环境.cmd 输出 SUNRAY_ROS1_PREFLIGHT=PASS | 控制性能、其他控制器部署或实机通过 |
| UE 场景与视频展示 | UE 5.5、UE5/MoSimSceneLibrary、桥接插件和对应场景资产包 | .uproject 能以 UE 5.5 打开且所需地图/插件可读 | Gazebo/PX4/MAVROS 运行时或控制闭环通过 |

基础 MWORKS 使用者可跳过这两个环境。需要运行时或 UE 视频复现时，先按上述指南完成系统
部署和预检，再回到本手册的 C99 运行、结果读取和显示层边界章节。
...
在线建模验证页将任务选择、场景参数和 Studio 操作日志集中在一个页面，如图 4-1 所示。

![在线建模验证工作台](图/APP/01_在线建模验证.png)

图 4-1　Studio 在线建模验证页
...
实时联合仿真页用于填写链路参数并执行连接预检，如图 5-1 所示。连接通过只说明链路可用，不能代替控制性能结论。

![实时联合仿真工作台](图/APP/02_实时联合仿真.png)

图 5-1　Studio 实时联合仿真页
...
首次在新机器上配置 WSL2、Ubuntu 20.04、ROS Noetic、Gazebo Classic、PX4、MAVROS
或 UE5 场景工程时，先完成[外部运行时环境部署与复现](../Workflows/外部运行时环境部署与复现.md)
中的系统安装和项目内源树预检。本节仅说明在预检通过后，如何选择并核验 C99 演示运行。
...
MoSim 助手是 Studio 中的可选只读分析入口，如图 7-1 所示。

![MoSim 助手工作台](图/APP/AI助手.png)

图 7-1　MoSim 助手本机只读分析界面
...
如果只使用 Model Studio 的任务配置、模型打开和 MWORKS 手动仿真，Codex CLI 不是
基础 APP 的前置依赖；只有打开“MoSim 助手”工作台时才需要安装。Windows 用户按下列
步骤准备 CLI，官方安装说明见
[OpenAI Codex CLI Getting Started](https://help.openai.com/en/articles/11096431)。

(1) 安装 Node.js LTS（随附 npm），安装后重新打开 PowerShell，并确认：
...
若提示找不到命令，先关闭并重新打开 PowerShell；仍找不到时用 `npm prefix -g` 查看
npm 全局目录，并把该目录加入当前用户的 PATH。安装完成后再执行下面的登录与自检。
...
按 CLI 的交互提示完成 ChatGPT/device 登录。认证信息不写入项目目录、Studio 配置、文档
或交接 JSON，而由 Codex CLI 的本机认证存储管理。更换 Provider、登录身份或 Codex CLI
后，应关闭并重新打开 Studio，再重新检查上述两条命令。

当前 CLI 的 API-Key 模式从标准输入读取；需要使用该模式时按
`codex login --help` 的提示通过标准输入提供密钥，不要把密钥直接写入命令、项目文件或
交接 JSON。
...
(4) 汇总状态文件：当前目录使用 G3_CATALOG_48_CURRENT_STATUS.json；
...
| simulation_timeout                                   | 执行时间到限，未形成完整 50 s 结果 | 保留执行记录，不能写作通过         |
| MWORKS 原生仿真提前终止（原始字段可为 `simulate_failed`） | 原生仿真未到 50 s 即停止；可有部分结果但无完整记录 | 保留原始状态与部分结果，不能写作通过 |
| check_model_failed                                   | CheckModel 未通过，未形成可用仿真记录 | 保留原始检查记录，不能写作通过      |
| blocked                                                 | 关键门未满足或执行受阻           | 记录阻断条件，不推断能力已验证     |
| invalid                                                 | 记录保留但不满足该实验有效性合同 | 作为无效负样本，不并入有效通过分母 |

当前正文汇总的 18 条未达判据或未完成记录由 9 条终端误差超限、8 条仿真超时和
1 条 MWORKS 原生仿真提前终止组成。历史 G3 的
`improved_pid` 与当前目录中的 `adaptive_mpc` 同属原生仿真提前终止，但只有前者不计入当前 48 条分母；`adaptive_mpc` 仍属于当前目录口径。七场景
...
Gazebo Classic 是 C99 运行时闭环的物理仿真后端。§6.5 已给出完整的启动和复现步骤；本节补充环境本身的说明和辅助显示工具的使用方式。图 8-1 展示当前使用的 `sunray150_with_mid360` 四旋翼模型；物理步长为 `0.001 s`，传感器包括 MID360 激光雷达和 IMU。

![Gazebo 仿真环境](图/Gazebo/Gazebo.png)

图 8-1　Gazebo 中的 sunray150_with_mid360 四旋翼模型
...
导出网格通过 `factory_model_path` 指定目录后可由 Gazebo 加载，图 8-4 给出加载效果。

![UE 场景导入 Gazebo](图/Gazebo/Gazebo-UE.png)

图 8-4　UE 工厂场景网格导入 Gazebo 后的加载效果
...
图 8-7　QGC 运行状态、飞机数量、模式和位置

![QGC点云、栅格和UE显示入口](图/QGC/3d789b418cfd4b05b303b492ed282836.png)

图 8-8　QGC 显示页中的 RViz 点云、栅格地图与 UE 入口
...
(1) FAST-LIO 实时建图：在启用对应 ROS/Gazebo 运行链路时，融合 MID360 激光雷达点云和 IMU，
...
(2) Diff-Planner 局部避障：可在登记的 ROS/Gazebo 规划链路中使用 FAST-LIO 地图求解局部轨迹；
本手册不固定宣称实时性、毫秒级重规划或 planner 成功，须由对应运行证据单独证明。

![Diff-Planner 轨迹](图/Rviz/diff.png)

图 8-11　Diff-Planner 规划轨迹与栅格地图

绿色线表示参考路径，红色线表示优化后的实际轨迹。

(3) FUEL 自主探索：在对应探索运行链路中可依据未知区域信息增益选择观测目标；覆盖率、终止
条件和是否完成探索以该次运行证据为准。
...
(4) Figure-8 轨迹跟踪：用于展示参考路径与跟踪轨迹的审核图；只有绑定的运行记录和指标
...
复现包最小清单：一次可交接的复现实验至少应包含当前提交或发布版本、使用的 `controller_id` 和
