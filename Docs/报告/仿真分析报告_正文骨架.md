...
| Studio 手动工作流    | 48 条已登记全机 Runner 路由（含 2 条 Experimental 命名空间 Runner） | `model_studio_task_routes_v1.toml`           |

### 1.6 报告阅读说明

本报告所有定量结论均回链至原始数据路径与指标文件。正文展示口径为 48 条控制器路线：其中 46 条算法控制器按七个算法族展开，Official PID 为官方基线，px4ctrl 为自研基线；Studio 清单当前已登记 48 条可手动打开的全机 Runner 路由，其中 2 条保留 Experimental 命名空间，当前名义目录已有 30/48 条路线达到终端误差门限。第七章的族系汇总图按当前 30 条达标记录生成，第 7.6 节仍保留冻结历史 28 条详图作为可追溯图集来源。各章图表均标注数据来源脚本与结果目录。
...
本章聚焦 46 条算法控制器的图形化建模与统一实现；Official PID 和 px4ctrl 的完整推导分别保留在第五章和第八章，三部分合计覆盖正文的 48 条控制器路线。Studio 路由表当前提供 48 条已登记全机入口，其中 `smc_boundary_layer` 与 `nmpc_outer` 保留 Experimental Runner 命名空间，便于用户从配置、建模到结果复核逐步展开。
...
本章的结构图证据也要按用途阅读：报告目录的 48 张控制器结构图均已完整归位，用于说明各控制器的模块组织、接口位置和实现形态；它们不替代第七章的逐控制器结果图、`Result.msr`、指标或当前源绑定的整机性能证据。
...
| `smc_boundary_layer` | 6.5 | `MoSim_G9_SMC_BOUNDARY_LAYER_GRAPHICAL_OVERVIEW.mo` / `SmcBoundaryLayerFormalRunner`（Experimental） |
...
| `nmpc_outer` | 6.6 | `MoSim_G9_NMPC_OUTER_GRAPHICAL_OVERVIEW.mo` / `NmpcOuterFormalRunner`（Experimental） |
