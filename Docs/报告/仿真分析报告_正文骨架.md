...
本报告所有定量结论均回链至原始数据路径与指标文件。正文展示口径为 48 条控制器路线：其中 46 条算法控制器按七个算法族展开，Official PID 为官方基线，px4ctrl 为自研基线；44 是 Studio 可手动打开的正式路由，当前名义目录已有 30/48 条路线达到终端误差门限。第七章的族系汇总图按当前 30 条达标记录生成，第 7.6 节仍保留冻结历史 28 条详图作为可追溯图集来源。各章图表均标注数据来源脚本与结果目录。
...
px4ctrl 系统模型关系图重点展示任务参考、控制层、控制分配、机体和反馈之间的 MWORKS 正式模型链路；该图不是旧 AWFF 图形模型截图，也不把显示层或运行时工具误判为控制器本体。

![云纵150系统级硬件架构](图/云纵150/架构图.png)

图 3-8　云纵150 px4ctrl 系统模型关系
...
下表只登记每条控制器正文公式的位置和当前源码映射，不再把登记表写成旧批次成绩单。46 条算法控制器的正文路径与对应源码由本章登记表和`Config/control_platform/current_model_entry_map.json`共同回链；Official PID、px4ctrl 以及 `pid_awff_linear_eso` 保留各自的图形模型与 Runner 边界。名义筛查结果统一在第七章按当前总账和冻结历史证据分别叙述。
48 条的三栏登记如表 6-1 所示，逐行横读可追溯每条控制器从设计存在（公式位置）到实现存在（源码映射）的链路；它不承担通过/失败判定。
...
| `fixed_awff_l1_residual` | 6.2 | `FixedAwffL1Residual.mo` / 无 G3 Runner |
| `fixed_awff_l1_indi` | 6.2 | `FixedAwffL1Indi.mo` / 无 G3 Runner |
...
| `smc_boundary_layer` | 6.5 | `MoSim_G9_SMC_BOUNDARY_LAYER_GRAPHICAL_OVERVIEW.mo` / Experimental Runner |
...
| `nmpc_outer` | 6.6 | `MoSim_G9_NMPC_OUTER_GRAPHICAL_OVERVIEW.mo` / Experimental Runner |
| `fixed_linear_mpc_l1_indi` | 6.6 | `FixedLinearMpcL1Indi.mo` / 无 G3 Runner |
| `fixed_qp_nmpc_l1_indi_cbf` | 6.6 | `FixedQpNmpcL1IndiCbf.mo` / 无 G3 Runner |
...
48 条目录项均有终态记录，当前固定目录对账为 30/48：30 个满足“已完成且终端误差 <5 m”门限、18 个进入完成失败分类、0 个保持 `not_run`。其中 8 条仿真超时、1 条仿真 API 失败和 1 条 CheckModel 失败并不等同于已产出完整 50 s 数据集。与早期 G2 轮的 17/48、冻结历史 G3 快照的 28/48 相比，当前有效通过数增加至 30 条；`completed=false` 仍表示 G3 的 48/48 接受目标尚未收口。

冻结快照来源为 `Results/control_platform/phase2_full_48_climbpath/g3_repair/G3_STATUS.json`（`effective_passed_count=28`、`effective_failed_count=20`，`generated_at=2026-07-30T11:58:56+08:00`）。当前总账改由 `Results/control_platform/phase2_full_48_climbpath/g3_repair/G3_CATALOG_48_CURRENT_STATUS.json` 提供（30/48、18/48、0/48，`inventory_reconciled=true`）。正文不再把这几层计数并列成表：当前 48 条目录状态是主口径；历史执行人口为 49 个单元（冻结 G3 总账 48 行，加上单独运行的 `pid_awff_linear_eso`），其中 38 个产出完整数据、28 个历史条目达到门限。历史执行人口与当前目录对账属于不同证据层，不能相加或相减；权威定义仍为`Config/control_platform/climbpath_baseline_count_definition.json`。
按族分组的条目数与达标数如图 7-1 所示。该图直接使用当前总账中的 30 条达标记录，覆盖六个有数据的算法族、单独的工程基线以及作为覆盖位保留的学习增强族，先给出整体分布，再进入族内细节。
...
上述两图基于 `.tmp/chapter10_typlot/accepted_controller_index.csv` 的当前 30 条有效记录绘制，族系归属来自 `control_scheme_catalog.json` 的 `category` 字段；冻结历史 28 条详图仍在第 7.6 节单独保留，不与当前汇总分母混用。

### 7.2 冻结历史 G3 有效失败分类

当前目录的 18 条终态失败与冻结历史 G3 的 20 条失败必须分开读：当前目录由 8 条终端误差超限、8 条仿真超时、1 条仿真 API 失败和 1 条 CheckModel 失败组成；表 7-3 和表 7-4 保留冻结历史快照的完整失败清单与误差分段，用来说明失败形态和后续排查入口，不把历史条目冒充当前目录状态。对当前目录而言，`pid_awff_linear_eso`、`smc_boundary_layer` 以及补充纳入的固定输入记录继续作为独立负性能证据保留。
...
图 7-36　PID 族 2 条达标条目的位置 RMSE（0.138 至 0.173 m）
...
控制能量柱状图保留两条 PID 的输入代价记录，但两条路线的执行器边界与能量统计尺度不同，不能把柱高直接解释为精度优劣。RMSE 与终端误差仍以同一 ClimbPath 口径比较，能量只作为实现边界的补充观察。

![PID族终端误差](figures/第10章/pid_family_comparison/figures/terminal_error_bar.png)

图 7-39　PID 族终端位置误差（0.0005 至 0.0065 m）
...
图 7-48　滑模族 5 条条目的位置 RMSE（1.453 至 2.705 m；5 条均通过当前终端误差筛查）
...
图 7-52　最优与预测控制族 7 条条目的位置 RMSE（0.135 至 1.243 m）

MPC 族 RMSE 柱状图高度集中，滚动优化和约束处理在统一 ClimbPath 下产生了稳定且可重复的精度区间。
当前 7 条记录既包含滚动预测变体，也包含 iLQR、MPPI 和补充路线，族内差异因此比早期五条历史记录更宽；图中仍可直接比较各实现的标称跟踪区间。
...
控制能量图仅对 6 条存在可用能量指标的记录绘制，`nmpc_outer` 的能量项缺失，因此这里不把能量柱状图解释成完整的 7 条排序。不同执行边界和统计口径造成的量级差异应与 RMSE、终端误差分开阅读。

![MPC族终端误差](figures/第10章/mpc_family_comparison/figures/terminal_error_bar.png)

图 7-55　最优与预测控制族终端位置误差（0.0005 至 0.143 m）
...
**ilqr**
位置 RMSE 0.219 m、终端误差 0.006 m，为冻结历史详图中的 MPC 五条记录中终端误差最大的一条。迭代 LQR 在每步重解局部线性化问题，其误差时程的过渡段与线性族 LQR 形态接近，这与两者共享同一局部近似结构相符。
...
**robust_mpc**
位置 RMSE 0.203 m、终端误差 0.003 m，是冻结历史详图中的 MPC 五条记录中精度最好的一条。在本标称无扰工况下，鲁棒约束未被激活，因此该值反映的是其标称性能上界，鲁棒性收益需由第九章的扰动场景考核。
...
**tube_mpc**
位置 RMSE 0.227 m、终端误差 0.004 m，为冻结历史详图中的 MPC 五条记录中 RMSE 最大的一条。管道约束在标称工况下收缩了可用控制余量，代价体现为轨迹跟踪的一致偏保守，与 `robust_mpc` 的 0.024 m 差值即该保守性的量化结果。
...
第七章的当前族系汇总图使用 30 条达标记录，冻结历史详图仍覆盖 46 条既有控制器中的 28 条达标条目；当前固定目录对账则为 30/48 通过、18/48 完成失败、0/48 未运行。汇总与详图分别回答分布和形态两个问题，标称分辨力仍需通过扰动条件进一步区分。在此基础上，本章转向自研控制器 px4ctrl 的图形化设计与全机闭环验证——它是本项目从”多族控制器可用”推进到”自研控制器经深度验证后可导出部署”的核心环节。
