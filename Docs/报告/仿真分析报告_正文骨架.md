...
图 6-11　pid_awff_linear_eso AWFF-ESO 图形模型结构
...
| `pid_awff_linear_eso`         | 6.2      | 目录`planned`；`Models/MoSimQuadrotorModel/Control/Implementations/Sysblocks/MoSim_PID_AWFF_LINEAR_ESO_GRAPHICAL_MIL.mo` / `PidAwffLinearEsoFormalRunner` | 图形核心 CheckModel 通过；独立 50 s 负证据，终端误差 3412.359 m；非 G3 行 |
