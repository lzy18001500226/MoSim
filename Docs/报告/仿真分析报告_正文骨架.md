...
本报告所有定量结论均回链至原始数据路径与指标文件。正文展示口径为 48 条控制器路线：其中 46 条算法控制器按七个算法族展开，Official PID 为官方基线，px4ctrl 为自研基线；Studio 清单当前已登记 48 条可手动打开的全机 Runner 路由，其中 2 条保留 Experimental 命名空间，当前名义目录已有 30/48 条路线达到终端误差门限。第七章的族系汇总图按当前 30 条达标记录生成，第 7.6 节保留冻结历史轨迹图集作为可追溯来源。各章图表均标注数据来源脚本与结果目录。
...
它定义的是单行结果状态；根 `G3_STATUS.json` 的 `completed=false` 是冻结历史 G3 快照的项目状态，当前目录对账文件同样为 `completed=false`。二者都表示 48/48 通过目标尚未收口，不能把当前 30 条通过写成 G3 总体完成。
...
以冻结历史 G3 快照的控制器详图（每条 7 张）的生成脚本为例，其核心绘图逻辑为：
...
完整脚本位于 `Scripts/syslab/plot_28_passed_detail_typlot.jl`（冻结历史 G3 详图图集）、`plot_chapter10_comparison_typlot.jl`（族系汇总、雷达与排名）、`plot_chapter11_typlot.jl`（七场景、灵敏度、三机编队与 ECBF）、`plot_extension_openblocks_typlot.jl`（OpenBlocks 拓展轨迹）。`plot_sensitivity_analysis.jl` 仅保留为旧的手写 SVG 生成器，不是当前 PNG 证据入口。
...
48 条目录项均有终态记录，当前固定目录对账为 30/48：30 个满足“已完成且终端误差 <5 m”门限、18 个进入完成失败分类、0 个保持 `not_run`。其中 8 条仿真超时、1 条仿真 API 失败和 1 条 CheckModel 失败并不等同于已产出完整 50 s 数据集。与早期 G2 轮和冻结历史 G3 快照相比，当前有效通过数增加至 30 条；`completed=false` 仍表示 G3 的 48/48 接受目标尚未收口。
...
上述两图基于 `.tmp/chapter10_typlot/accepted_controller_index.csv` 的当前 30 条有效记录绘制，族系归属来自 `control_scheme_catalog.json` 的 `category` 字段；冻结冻结历史详图仍在第 7.6 节单独保留，不与当前汇总分母混用。
...
本节按当前可复现的控制器图集清单提供完整的 7 张详图（XY 轨迹、高度 Z、位置误差、控制输入、三维轨迹、速度、姿态角）；Official PID 与 px4ctrl 基线在 §7.4 单独展示。每条控制器的图集不仅展示跟踪精度（前 4 张），还展示实现该精度的机体状态代价（后 3 张）——相同 RMSE 可能对应平稳飞行或剧烈抖振，完整图集使这种差异一目了然。数据源为各控制器 50 s/25001 样本点的标准 CSV，图件采用 600 dpi/Times New Roman 统一排版。
...
第七章的当前族系汇总图使用当前总账中的 30 条达标记录；第 7.6 节保留冻结历史图集作为可追溯的形态证据，二者不与 48 条当前目录状态混用。当前固定目录对账为 30/48 通过、18/48 完成失败、0/48 未运行。汇总与详图分别回答分布和形态两个问题，标称分辨力仍需通过扰动条件进一步区分。在此基础上，本章转向自研控制器 px4ctrl 的图形化设计与全机闭环验证——它是本项目从”多族控制器可用”推进到”自研控制器经深度验证后可导出部署”的核心环节。
