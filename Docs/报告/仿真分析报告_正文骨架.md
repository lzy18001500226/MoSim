...
不在冻结 G3 批次内的 `pid_awff_linear_eso` 同属"跑通但未达标"：项目源、Adapter 和
`PidAwffLinearEsoFormalRunner` 均已落盘，50 s 运行完成并产出 25001 个可读样本，约
...
`control_scheme_catalog.json` 和 `current_model_entry_map.json` 中的 `role=planned` /
`planned/not_runnable` 是 2026-07-27 冻结目录的历史元数据，不能再解释为“没有实现、没有
Runner 或没有结果”；当前状态应写成“实现与运行记录已存在，性能门失败，正式选择资格仍待
后续评估”。合计跑通未达标 10 条。
