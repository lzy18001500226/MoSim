...
公式块给出控制器的核心代数关系，图形模型截图展示同一关系在 Modelica 侧的模块组织与信号连接；两者按控制器逐一配对排列。设计阶段的最优化、Riccati、观测器或稳定性关系用于说明增益/结构的来历，不表示当前 FormalRunner 在每个采样时刻都重新求解这些离线设计问题。
...
在 Modelica 侧，调度变量进入三条增益通道，积分状态、比例误差和微分误差在 PID 输出端汇合；该式不把增益调度误写成在线重新辨识系统模型。
...
该控制器以线性 ESO 估计扩张状态，再将估计扰动从基础控制量中扣除；目录中保留 planned 标识，名义记录作为异常响应样本纳入对照。ESO 三个状态、基础命令和补偿输出是连续定义，不能拆成无说明的多行公式。
...
在 Modelica 侧，FormalRunner 接口面展示 ESO 控制核心的调度入口和信号边界；\(z_3/b_0\) 是扩张状态对总扰动的补偿估计，不应被表述为已经由独立全机 G3 行验证的性能结论。
...
该控制器以二次型性能指标定义状态反馈增益，并保留参考前馈通道。原有公式中的积分项没有等号或优化关系，不能说明它是性能泛函；这里把“离线设计”和“在线固定增益输出”放在同一连续公式组中。
...
在 Modelica 侧，积分状态由参考与输出的误差以 \(T_s=0.01\,\mathrm s\) 离散累积并限幅，再回接到状态反馈通道；这避免把理论连续积分器误写成无界的工程实现。
...
在 Modelica 侧，\(H_2\) 路线经 CFunction/EquationBridge 输出期望加速度、姿态四元数和总推力，再由 Adapter 转为 ATTITUDE_THRUST；上述范数优化是增益设计解释，不表示 FormalRunner 在线执行范数最小化。
...
在 Modelica 侧，状态反馈直接连接到力与力矩输出端口，当前桥接器把总力限制在 \([0,25]\,\mathrm N\)，各力矩按规定上限限幅，再交由共享分配器完成 wrench-to-rotor 转换。范数不等式是悬停线性化设计条件，不应扩展为任意机动范围内的全局性能证明。
...
该族直接利用非线性模型的状态关系构造控制律，成员覆盖反步递推、动态逆、无源性控制、模型参考自适应和扰动估计。为避免把“非线性”写成一个空标签，以下公式分别给出虚拟控制、逆模型、储能函数或参数更新的实际作用位置。

#### backstepping_baseline

该控制器按误差递推构造虚拟控制量，再由第二级误差闭合实际输入。位置误差、虚拟速度和第二级误差共同构成反步递推，不能只保留首末两式而遗漏中间状态。
...
在 Modelica 侧，控制效能矩阵求逆块与误差反馈块连接到动态逆输出；若效能矩阵接近奇异，工程实现仍需依赖限幅、接口约束和后续验证，正文不把该理想式误写成无条件可逆的保证。
...
在 Modelica 侧，模糊调节块输出 \(\mathbf K_f(\mathbf s)\)，再与连续边界层到达项汇合；这保留了滑模鲁棒性，同时避免把图形实现误描述为硬符号切换。
...
在 Modelica 侧，饱和块位于滑模面与输出之间，直接限制到达项的变化幅值；该独立 Experimental 路线保留结构表达，但不把其单独记录写成 G3 全机通过结论。
...
预测与优化族用滚动预测、扰动集合或采样搜索处理未来误差，十条控制器的差异集中在模型、约束和求解器接口。以下以 \(\mathbf x_{i|k}\) 表示时刻 \(k\) 对第 \(i\) 个预测步的状态，以 \(\mathbf U_k\) 表示候选控制序列；所有优化式均须给出状态转移、代价或安全约束以及实际施加的首个控制量，不能只保留一个孤立的 \(\arg\min\)。
...
在 Modelica 侧，扰动裕度进入预测增益和加速度限幅通道，最坏情形的优化结果经统一执行接口输出；当前图形内核的鲁棒项以有界修正实现，不把报告中的集合表述误写成未实现的大规模在线 min-max 求解器。
...
在 Modelica 侧，当前桥接器把自适应尺度限制在预设上下界后回写预测增益，更新后的模型再进入在线求解器；\(\mathcal I(\cdot)\) 表示辨识更新算子，不把它虚构成已经验证的全参数辨识器。
...
在 Modelica 侧，调度变量和状态共同进入分区查表，选定的局部增益直接连接到输出；当前桥接器用归一化误差形成调度量，避免把该项目实现误写成离线枚举的全部多面体分区库。
...
在 Modelica 侧，轨迹展开、局部线性化和有限次梯度/反向递推结果汇入时变控制量通道；当前可读内核采用固定次数迭代与加速度/增量限幅，不能把它夸大为无约束收敛到全局最优。
...
该控制器在设计层以非线性状态转移作为预测约束，在外环滚动处理轨迹代价和控制序列。为避免把当前图形外环夸大为通用大型 NMPC 求解器，先给出标准优化表达，再给出已落地的有界预测/增量执行关系。
...
该组是当前图形外环的实际执行边界；标准 NMPC 优化式说明其设计方向，二者不应被混写为“已在线求解全非线性约束优化”。
...
该控制器把线性 MPC 的名义控制与 L1 残差估计、INDI 增量补偿串成一条固定集成链，增强模型失配下的快速修正。名义、残差和增量补偿均需保留，不能压缩为一行长加法。
...
该控制器在 NMPC 名义输出外增加 QP 投影、L1 残差与 INDI 增量补偿，把安全约束和快速执行修正合并到同一链路。原有公式只写出一个没有上下文的梯度不等式，现补全障碍函数约束和投影后的执行链。
...
上式是一组连续的几何合力/力矩设计关系，因此保留在同一个大括号组内。当前 `se3_basic` 图形桥接器并不把完整 \(\mathbf M_c\) 作为额外 WRENCH 执行通道，而是将公共误差记号 \(\mathbf e_p=\mathbf p_r-\mathbf p\)、\(\mathbf e_v=\mathbf v_r-\mathbf v\) 投影为有界的 ATTITUDE_THRUST 指令：
...
其中 \(c_\phi,c_\theta,c_T\) 为当前桥接器的固定投影系数。这样既保留 SE(3) 的设计语义，也不把图形概览的有界平移投影误写成一条已经独立执行完整力矩控制的 WRENCH 路线。
...
该控制器利用平坦输出的参考加速度，加上位置/速度反馈与一个延迟辅助补偿项形成期望加速度。这里的辅助项来自当前桥接器的固定离散支路，不应写成未经实现支撑的在线扰动估计 \(\hat{\mathbf d}\)。
...
该控制器沿用同一条平滑鲁棒平移链，只在最后一步把横向加速度投影为 BODY_RATE_THRUST。当前桥接器不从 \(R_r^T\dot R_r\) 计算角速度，而是使用固定比例和限幅，因此公式应写出实际投影关系。
...
学习族把网络作为名义控制器外围的增益调度或残差补偿模块，通过回退、限幅和统一输出边界接入控制链。网络的内部层数、训练数据与泛化性能不由这两条结构公式断言；正文只说明当前 CFunction 桥接器已暴露的特征、输出、限幅和回退关系。
...
在 Modelica 侧，冻结的学习 CFunction 输出逐轴 `scheduled_gain_x/y/z` 与状态码；`rl_gain_scheduler` 只选择增益调度模式。上述公式描述该模式的有界接口结构，不把结构存在误写为策略训练充分性或全工况性能结论。
...
在 Modelica 侧，学习 CFunction 输出三轴 `learning_action`、受调度增益和 `fallback_active`。上式中的 \(b_{\mathrm{fb},k}\) 对应该回退标志：回退激活时保留名义控制输出，未激活时才使用已限幅的残差合成命令。因此，本节强化的是可审查的保护结构，而不是凭一条公式宣称神经残差已覆盖全部未知扰动。
...
# 两臂同轴绑图，图例标注冻结 RMSE（取自 SCENARIO_RMSE_MATRIX，不现算）
