...
48、46、44、53、当前 30/18/0 与冻结历史 28/20 是不同口径，不能互相替换或相加。
...
- 其他控制器的 Gazebo/PX4/ROS/MAVROS 运行时部署；
...
QGC、RViz 和 UE 属于运行显示与操作辅助层。QGC用于查看任务、飞行状态、故障配置
和工厂二维地图；RViz用于查看点云、轨迹和栅格地图。它们的窗口出现、地图显示或
截图不能替代 `Result.msr`、运行日志和指标，也不能单独证明控制器性能或部署成功。

本节沿用项目现有界面资料，图片中的旧窗口标题和历史嵌入式 UE 布局只用于说明功能
组织，不代表当前版本名称或当前运行状态。手册不要求额外制作命令行截图，复现命令
仍以正文中的文字步骤和脚本路径为准。

![QGC任务与工厂地图界面](图/QGC/2bf9bb6d9a0fe79cfd6c1c48d399af62.png)

图 8-1 QGC任务页、控制器选择、任务状态和工厂地图显示。

![QGC故障注入与恢复界面](图/QGC/3984ec040b9bb0fe294d7204f8176148.png)

图 8-2 QGC中的风扰、电机效率故障以及恢复入口。

![QGC运行状态界面](图/QGC/46660730422f48e2295919217e78dce0.png)

图 8-3 QGC运行状态、飞机数量、模式和位置状态展示。

![QGC点云、栅格和UE显示入口](图/QGC/3d789b418cfd4b05b303b492ed282836.png)

图 8-4 QGC显示页中的RViz点云、栅格地图和UE显示入口。

![QGC工厂二维地图与航迹示意](图/QGC/e64311862f9004d6a0572684a1d27873.png)

图 8-5 工厂二维地图展开视图及轨迹示意。该图用于说明显示对象，不作为航点规划
或实时通信通过证据。

![RViz轨迹与MID360点云](图/rviz/8字.png)

图 8-6 RViz中的轨迹、参考路径和MID360点云查看。

![RViz点云与Grid3D栅格地图](图/rviz/diff.png)

图 8-7 点云地图与Grid3D栅格地图转换结果的RViz查看。

![FUEL点云地图与探索路径](图/rviz/fuel.png)

图 8-8 FUEL场景中的点云地图和探索路径显示。该图用于展示规划与感知结果，不能
替代对应的运行日志、轨迹数据和覆盖率指标。
