# 控制器原生截图重建清单

状态：user_reviewed_graphical_assets。本文是 46 条 MWORKS 结构截图槽位的子台账，
不是 47 条已完成运行证据的总账。46 张历史图形结构候选已在 2026-07-28 经用户
视觉审核并按当前族类归位；它们不等价于当前源最小闭环、性能、代码生成或运行时通过。
47 条运行/截图覆盖见
`FORMAL_RUN_SCREENSHOT_COVERAGE_20260731.md`。

| 项目 | 数量 |
|---|---:|
| 历史/项目方案注册表 | 48 |
| 当前原生窗口截图范围 | 46 |
| 已初始化截图目录 | 46 |
| 已版本化目录占位符 | 46 |
| 已归档旧导出资产 | 完整 |
| 当前 G5 绑定原生结构图 | 0 |
| 用户审核历史图形结构图 | 46 |
| 已采集最小闭环结果原生图 | 0 |

七个语义控制族分别入库；五条命名整机 Profile 按其 PID 或最优/预测归属入库，不另设控制器族。用户审核通过的历史图形结构候选可按本清单归位；旧结果截图继续归档，不能作为当前性能结论。

| 目录 | 路线数 |
|---|---:|
| 01_PID族 | 9 |
| 02_线性与鲁棒状态反馈 | 6 |
| 03_非线性与自适应 | 6 |
| 04_滑模控制 | 7 |
| 05_最优与预测控制 | 10 |
| 06_几何与微分平坦 | 6 |
| 07_智能与学习 | 2 |

## 不进入本批 46 条结构截图

- pid_awff_linear_eso：当前正在实现和运行；终端 `RUN_RECORD` 完成前不进入本批结构截图或 47 条已完成运行分母。
- px4ctrl：工程/部署基线；已有独立 MWORKS 图形化闭环证据，但不纳入本批 46 条 MWORKS 控制器图审/对比范围。

当前新采集的 01_图形模型.png 必须来自冻结模型入口的 Windows MCP 原生整窗/桌面采集，保持窗口原生宽高比，并与当前 G5 截图 manifest、图审 packet 绑定。本批用户审核的历史图形结构候选仅用于结构展示。02_最小闭环结果原生窗口.png 只能在后续当前源正式仿真后写入，不能用结构图、历史结果或空白窗口代替。
