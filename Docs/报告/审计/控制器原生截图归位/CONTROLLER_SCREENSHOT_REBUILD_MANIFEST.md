...
状态：report_structure_assets_complete_with_preserved_acquisition_provenance。48 条控制器均已有报告目录结构图，当前统一作为历史结构展示资产管理；原始采集来源仍保留 44 张用户审核候选与 4 张原生窗口捕获的 provenance 细分。它们不等价于当前源最小闭环、性能、代码生成或运行时通过。
...
| 已版本化目录占位符 | 48 |
| 已归档旧导出资产 | 完整 |
| 采集 provenance：当前原生窗口结构图 | 4 |
| 当前 G5 packet 绑定结构图 | 0 |
| 采集 provenance：用户审核历史图形结构图 | 44 |
| 已采集最小闭环结果原生图 | 0 |
