...
- `pid_awff_linear_eso` 正在运行，尚无本轮终端 `RUN_RECORD`，暂不计入 47。

这个 47 不是冻结 G2/G3 的性能通过数，也不是 47 条同一类型的整机闭环
...
| 检查项 | 当前覆盖 | 结论 |
|---|---:|---|
| 已完成运行证据分母 | 47/47 | 46 条 G6 记录 + 1 条 `px4ctrl` 基线 |
| `Results/` 中源结果窗口覆盖 | 47/47 控制器 | 46 条 G6 结果截图 + `px4ctrl` 记录下的 2 张结果窗口截图均可追溯 |
| 报告目录当前源绑定的结构图 | 0/47 | 目录中的 46 张 PNG 是用户审核历史结构候选，不是当前 G5 绑定图；`px4ctrl` 有独立图形证据 |
| 报告目录当前源绑定的逐控制器结果图 | 0/47 | 尚未完成逐条复制、哈希、运行记录和证据类别绑定 |
...
现在不能写“47 张当前整机仿真截图已经全部归位”。准确表述是：**47
条运行证据可追溯，报告目录尚未完成 47 条当前源绑定截图材料化，而且
其中 G6 的多数结果截图属于固定输入探针，不能统一当作整机图。**

报告插图前必须对每条图片绑定：控制器 ID、FormalRunner、源哈希、运行记录、
结果文件、截图来源和证据类别；`pid_awff_linear_eso` 待终端记录完成后再
追加并把分母改为 48。
