# gain_scheduled_pid 失败诊断

## 控制器架构 (2026-08-19)

**GainScheduledPidGraphicalRunner.mo 配置**:

Line 24-25:
```modelica
MoSimQuadrotorModel.Control.PidFamily.GainScheduledPid.GainScheduledPidCore core
```
...
**使用 GraphicalScalarRotorPreview 适配器**:

1. **适配器架构**:
   - 输入: 单个标量 `command` (归一化推力 0~1)
   - 输出: 4个相同的转速 `rotor_command[1:4]`
   - 映射公式: `speed = min_speed + command × (max_speed - min_speed)`
   - min_speed = 59.78 rad/s, max_speed = 69.53 rad/s (15%裕度)

2. **控制器输出**:
   - GainScheduledPidCore 输出单个标量 `command`
...
Phase 5 报告误差 11.53m 符合预期。

## 结论

**gain_scheduled_pid 使用了错误的适配器架构**:

1. ✅ **控制器核心可能工作正常** (增益调度PID)
...
3. 进入下一个失败控制器: **fopid** (error=14.12m)
