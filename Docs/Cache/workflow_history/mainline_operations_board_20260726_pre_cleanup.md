...
2026-07-23 scoped controller-evidence and model-root-retirement work is active.
Its authority is `Docs/Workflows/controller_evidence_closeout.md` and its only
permitted order is:

```text
D1-D3  Freeze current 49/46/2/1 terminology, the per-route evidence chain,
       the selected-champion formal-harness promotion gate, and R1 retirement
       preconditions.
G5     Review internal graphical topology, bind a formal whole-aircraft test
       harness where it actually exists, then collect real MWORKS result and
       native whole-window evidence or a precise blocker.
G6-G7  Select one champion per nominal family only after G5. Before its
       seven-scenario A/B, build and validate that champion's formal-root
       whole-aircraft harness; existing fixed integrated chains cannot stand in
       for it. Then run the declared safety, fault, formation, and Syslab
       representative evidence.
R1     Audit all active dependencies, write a reversible archive manifest,
       archive the three compatibility roots only after the audit passes, and
       run canonical-root smoke checks while the roots are absent.
```

The historical `67`-route / `65/67` summaries below are provenance only. They
cannot select a current model, prove a graphical layout, substitute for a
formal test harness, or authorize archival.

### Historical 2026-07-18 non-frontend closeout snapshot

The 2026-07-18 closeout goal was to finish the competition engineering,
...
RViz-embedding frontend features. Its recorded execution order was:
...
Historical 2026-07-20 report-evidence priority update: the 67-route controller evidence
...
evidence. The historical snapshot counts are `accepted=1`, `executed_blocked=11`, and
...
the historical 67-row authority counts for that frozen snapshot and fails closed if the JSON count field
does not match the row statuses. Do not hard-code a stale status distribution
into downstream report sources.
...
`executed_blocked=33`, and `not_run=7`; report generators must read these row
states rather than retain the superseded `27/25/15` distribution. C3 is frozen with the result
...
full pass via `cmd/启动Factory单机同飞行覆盖建图.cmd`, accepting full coverage only
...
`cmd/启动Factory单机同飞行覆盖建图.cmd`, accepting completion only if the final
...
`cmd/启动Factory单机同飞行覆盖建图.cmd` now uses
...
that route. The Windows entry `cmd/启动Factory单机同飞行覆盖建图.cmd` is promoted to a
...
unknown autonomous exploration. Use `cmd/启动Factory多机已知覆盖建图审核.cmd` for a
fresh RViz review run. UE remains the separate S11 display entry
`cmd/启动Diff工厂UE实时审核.cmd` and does not replace this backend evidence.
```
