# Factory Three-UAV Self-Service Flow (Archived)

This is a historical no-QGC/no-UE fixed-formation workflow. Its Windows
wrappers are archived in `Scripts/cmd/Archive/legacy_unverified/` and must not
be treated as a current supported route. It does not establish autonomous
exploration, online multi-UAV avoidance, FAST-LIO localization, or support for
four to nine vehicles.
...
1. Historical entry: `Scripts/cmd/Archive/legacy_unverified/01_预检Factory三机环境.cmd`.
   ...
   - When it reports `busy`, do not start another backend. Run
     `Scripts/cmd/Archive/legacy_unverified/06_停止Factory三机编队.cmd`, wait for the runner to exit, then repeat the
     preflight.
2. Historical entry: `Scripts/cmd/Archive/legacy_unverified/03_启动Factory三机固定编队.cmd`.
...
3. Run `Scripts/cmd/Archive/legacy_unverified/05_检查Factory三机状态.cmd` until it reports `passed`.
...
4. Historical entry: `Scripts/cmd/Archive/legacy_unverified/04_打开Factory三机RViz审核.cmd`.
   ...
   - Run `Scripts/cmd/Archive/legacy_unverified/05_检查Factory三机状态.cmd` once more if a text check is wanted: its
...
6. End with `Scripts/cmd/Archive/legacy_unverified/06_停止Factory三机编队.cmd`.
...
| Stop script cannot find the runner | Do not start another swarm run. Check the backend terminal and `Results\sunray_ros1`; use the broad `Scripts/cmd/停止所有仿真.cmd` only when the user intends to stop every managed simulation. |
