...
Unreal is the high-quality visual layer of the MoSim simulator product. In the
current 2026-06-24 operating mode, this workflow is for explicit S11
display/frontend work, scene review, video material, and visualization
enhancement only. It is not the current ROS1/Sunray/Gazebo/PX4/MAVROS/px4ctrl
runtime lane and must not be used as controller, localization, planner, or
closed-loop success evidence.

MWORKS/Sysplorer/Syslab remain the formal controller/model evidence source.
Current runtime plant, sensor, PX4/MAVROS/px4ctrl, and RViz evidence is owned
by `Docs/Design/架构.md`,
...
`Docs/Design/架构/MoSim控制体系总览.md` and
`Docs/Design/架构/MoSim真机化收尾与C++化重构方案.md`.

## Current Policy

Read this workflow only when the task explicitly asks for UE/frontend/S11
display work or review media. Older sections that mention ROS2, RViz2,
UAV-stack-first routing, or UE-driven sensor work are historical design notes
unless the user explicitly reopens that route and the current architecture docs
are updated first.
...
UE operator action
  ...
  -> route-local MWORKS/ROS command adapter validates authority and the
     explicitly reopened gate
  -> runtime applies or rejects the request
  -> next MWORKS/ROS status frame echoes active mode, reason, and evidence level
...
The UE workflow's Factory vehicle gate default is first-frame-only. Use
...
Propeller placement, MID-360 placement, heading, and materials for the UE
vehicle visual must be traced to the user-reviewed DAE-derived assembly
...
20.0Hz, IMU held about 200.0Hz, and LiDAR held about 10.0Hz. This historical
UE/ROS2 slice proves only the ROS2 topic-rate bridge, not final co-simulation
and not the current ROS1/Sunray/RViz lane. The Factory LiDAR input in that
slice had only about 160 points per frame and was therefore below the density
...
The historical UE/ROS2 Factory headless real-stack gate was:
...
actor records the editable project path, `.uproject` path, truth artifact
list, acceptance gates, renderer-local content root, renderer map asset, and
renderer package name from the registry. For the UE workflow's Factory fallback it
...
| Local FAST-LIO | `References/Lab/localization_slam/FAST_LIO/launch/mapping_mid360.launch` starts `fastlio_mapping` and optionally starts `rviz -d loam_livox.rviz`; `References/Lab/localization_slam/FAST_LIO/rviz_cfg/loam_livox.rviz` subscribes to `/cloud_registered`, `/Odometry`, `/path` |
| Local EGO-Planner/Sunray | `References/Lab/planning_local/ego-planner/.../default.rviz` and `References/Sunray/.../launch_rviz/*.rviz` show planning markers, point clouds, occupancy/grid maps, robot state, and paths in a separate visualization window |
...
`References/Lab/localization_slam/FAST_LIO` package. A dry-run validates the contract without
creating files:
...
The historical Factory FAST-LIO route must pass a diagnosis gate before it is used as localization,
mapping, planner, or controller evidence. Topic existence is not sufficient:
the recorded Factory artifacts publish `/odometry`, `/path`, and
