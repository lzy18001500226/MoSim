...
| MOSIM-TOOLS-20260524-EPIC-LIBRARY | main agent; UnrealMCPAuditor(Laplace/Locke); EpicCacheAuditor(Kant) | Current phase: implement only `unreal_engine` and `mosim_epic_library` MCP abilities plus the Skills needed by those MCPs; Fab route is valid only if it can import editable content and support planning truth | `Scripts/UE5/epic_library_index.py`, `Scripts/UE5/epic_library_view.py`, `Scripts/UE5/audit_scene_source.py`, `Scripts/UE5/plan_scene_truth_export.py`, `Scripts/UE5/export_unreal_scene_truth.py`, `Scripts/UE5/mosim_epic_library_mcp.py`, `Scripts/UE5/mosim_epic_library_mcp_wsl_wrapper.sh`, `Scripts/UE5/check_epic_library_inventory.py`, `Scripts/tests/test_epic_library_index.py`, `Docs/Skills/unreal/mosim-epic-fab-library/SKILL.md`, `Docs/Skills/unreal/mosim-unreal-editor-mcp/SKILL.md`, `Docs/Workflows/debug_mcp.md`, `Docs/Workflows/unreal_renderer.md`, `Docs/Index/workflow_index.md`, `PROGRESS.md`, this ledger | running | User corrected scope: do not pursue a broad MoSim MCP/Skills system yet. Current phase is UE + Fab/Epic library MCP only. User further clarified that Fab must support full import, component use, map modification, and planning/map truth; rendering alone is insufficient. `epic_library_view.py` can merge account/Fab/Vault rows. `audit_scene_source.py` now audits local UE projects with bounded scans and reports all current `References/UnrealScenes` projects as visual candidates that still need explicit truth extraction/proxy artifacts. `export_unreal_scene_truth.py` defines the first explicit AABB collision-truth export route from UE Editor Python. `codex mcp list` now points `git`/`unreal_engine` to MoSim paths and includes `mosim_epic_library`; current `unreal_engine` listener probe fails because no editor listener is open. | Next: open one accepted editable scene in UE, export collision truth JSON, validate it, rerun scene audit, and only then claim scene is ready for mapping/planner integration. |
