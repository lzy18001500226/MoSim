...
> 同日已确认静态资产、可复用数据、运行证据和历史归档的目标边界；该确认不授权直接移动、删除
> 或切换任何现有路径。
...
对于资产包，必须额外冻结以下分类，不能只按文件扩展名或当前目录名判断：

```text
静态资产：场景、机体、地图、材质、坐标/转换清单 -> Assets/
可复用数据：跨任务传感器回放、标定输入、UE truth 数据 -> Data/
运行证据：run_id绑定的Result.msr、rosbag、ULog、指标、截图和日志 -> Results/
本机生成物：build、devel、install、工作区编译输出、下载工具和临时转换 -> 本机工作区
历史代码：退役实现与追溯说明 -> Docs/Cache/agent_legacy/或经批准的历史归档
```

`Result.msr`、rosbag、ULog和可回放传感器数据不能因“缓存清理”被删除。若一个场景或数据集已被
Config、launch、脚本或QGC资源消费，它是活动资产包，必须整体迁移并改写消费者路径，不能从
`Results/` 中局部抽走。
...
5. 对资产包，以完整版本目录而非单文件为迁移单位；必须包含资产MANIFEST、坐标/时间语义、
   消费者清单、源/目标哈希和回退说明。QGC、Gazebo和UE各自的派生副本必须指向同一资产真源。
...
本工作流只规定迁移方法；控制器证据门禁由对应专题工作流定义，而是否执行
任何门禁只由当前用户的直接请求决定。
...
#### 5.0.3 `image/AGENTS/` 历史截图归档（2026-07-28）

根目录 `image/AGENTS/` 的 12 张受版本控制 PNG 是 2026-06-03 的历史截图集，不是当前运行、
构建或交付入口。经活动引用审计后，全部文件以无内容改动的 `R100` Git 重命名归档到
