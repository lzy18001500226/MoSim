...
### 1.1 Codex CLI 使用与源码兜底构建（可选，用于 AI 辅助功能）

Model Studio 第四栏的 AI 助手功能需要 Codex CLI。该功能为**可选**，跳过不影响核心仿真。
正常路径是使用已安装且已登录的 Codex CLI。助手依次检查显式配置的二进制、Windows
安装目录或 `PATH` 中的 CLI，最后才回退到项目 `src/Agent` 的源码构建产物。
只有前两者不可用，或维护者需要重建项目内兜底产物时，才执行下表的源码构建。
...
**首次启用 GPT：** 先确认本机已安装 CLI 能执行 `codex --version` 与
`codex login status`。助手未发现可用 CLI 时，再执行本节对应平台的源码兜底构建。
将 `src/Agent/codex.config.example.toml` 的非密钥配置合并到用户自己的
`CODEX_HOME/config.toml`；认证状态由 Codex 的用户配置保存，不写入
`MOSIM_ROOT`。AI 助手无法使用不影响 MWORKS 核心仿真、结果复核或 px4ctrl
代码生成。
...
2. 可选启用 Studio AI 助手时，先确认安装版 CLI 的 `codex --version` 与
   `codex login status`。只有助手未发现可用 CLI 时，才在 `src/Agent`
   运行本清单第 1.1 节对应平台的源码兜底构建脚本。凭据和 `CODEX_HOME` 必须
   位于用户目录，不能置于项目根。
3. 在授权的 MWORKS 中仅加载
...
10. Windows 使用已安装的 Visual Studio Build Tools 或 LLVM/Clang，在能够发现
    C 编译器的终端中重新构建并测试；不要复用 Linux `.so` 作为 Windows DLL。
