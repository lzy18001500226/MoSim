...
> 状态：当前 G1-G7 的控制器证据、模型重构和实验收敛工作流。2026-07-16 的旧
> G1-G7 收尾编号已在接口规范中改标为历史 H1-H7，不能与本工作流的当前 goal 混用。
...
- 权威路线矩阵：`Results/control_platform/classic_controller_closeout_20260717/CLASSIC_CONTROLLER_FINAL_MATRIX.json`
...
当前矩阵有 67 条历史分层证据路线；`65/67` 仅表示相应路线通过了各自记录的
...
5. G5：49 条方案都保留入口或状态记录；当前实际进入 MWORKS 图审和最小闭环队列的是 46 条路线，即 41 条名义图形控制器核与 5 条固定集成整机链。mu_synthesis、neural_smc 保持实现 blocker，px4ctrl 保持 ROS1/PX4 运行时基线，不伪造 MWORKS 图。
6. G6：先对 46 条当前路线做最小筛选；再由 PID、经典鲁棒、滑模、优化、几何平坦、学习六个名义控制族的冠军完成七场景 A/B。固定集成链只在自身最小闭环合格后保留为整机对照，不与六族冠军重复计数。
...
1. 先用当前 activation/window 证据或一次有界 sentinel/probe 确认 MWORKS 可用；
...
完成 E1 的路线逐条运行 `check_model -> 最小 MWORKS simulate -> result variables ->
metrics`。每条至少保存：模型路径/哈希、场景配置、原生结果或可定位结果、时间序列、
指标、结果图和 MCP/GUI 观察。失败、发散和超阈值保留为失败证据，不重复改写成通过。
...
1. 46 条当前路线先按 G5 的最小闭环门完成筛选：41 条名义图形控制器核与 5 条固定集成链均各自保留首个 blocker 或最小闭环证据，不借用邻近路线结果。
2. 从 PID、经典鲁棒、滑模、优化、几何平坦、学习六个名义控制族各选择一条合格冠军；每条与 Official PID 基线完成同参数、同指标的七场景 A/B。PID 族冠军若为 Official PID，不重复计数。
...
- 六族冠军与 Official PID 的核心对比均有真实模型、结果、指标和同场景绑定。
