...
flowchart LR
  ...
  D --> E["传感器、定位、建图、规划、编队"]
  E --> C
  K["Isaac Sim / Isaac Lab训练"] --> L["Policy Artifact Registry"]
  L --> E
...
| 学习规划层 | Isaac训练、策略导出、策略注册、YOPO局部轨迹提议和Gazebo验证 |
...
Flight Console同时提供面向普通用户的“快速实验”和面向算法开发者的“算法与训练”工作区。
用户无需进入终端即可选择任务算法、传感器路线、训练后端、场景、速度范围、随机种子和导出
格式，并通过按钮完成配置生成、Isaac训练、策略导出、策略注册和Gazebo验证。尚未满足运行
条件的YOPO、深度相机或训练后端仍保留在界面中，直接显示所缺依赖和解锁条件，使平台能力
边界清晰且后续接入不需要重做界面。
...
所有按钮和自然语言操作最终生成相同的结构化Action并进入同一个Orchestrator：

```text
Flight Console按钮 ─┐
                    ├─> Action API -> Orchestrator -> MWORKS/Gazebo/Isaac/分析后端
MoSim Agent自然语言 ─┘
```

因此，Agent不是另一套隐藏脚本，GUI也不是命令行的静态外壳。用户既可以逐项点击完成实验，
也可以让Agent自动填写同一组表单、生成同一份Profile并提交同一种Action，二者共享校验、
权限、ACK、日志和结果。
...
“使用Isaac Lab在Factory和随机障碍场景训练YOPO，速度范围1至4 m/s，加入深度噪声和随机风，
导出TensorRT策略，然后在Gazebo运行20个随机种子并与经典规划器比较。”
```
...
5. 对学习规划任务生成训练配置、启动训练、导出并注册策略产物；
6. 持续汇总运行状态、遥测异常、安全介入和任务进度；
7. 自动读取结果，比较基线与改进算法并定位失败原因；
8. 生成下一轮参数建议、消融实验和报告摘要。
...
同一算法可以通过独立的传感器Profile保留不同复现路线。FUEL既可使用当前MID360与
FAST-LIO形成的工程点云路线，也可使用前向深度相机复现上游默认输入；两条路线具有独立
的传感器、参数、证据和运行Profile，不允许把激光点云静默伪装成深度图。

YOPO作为`learned_local_planner`接入局部规划层。Isaac Sim/Isaac Lab根据场景几何和ESDF
梯度完成Guidance Learning，导出的PyTorch、ONNX或TensorRT策略读取深度图、状态和目标方向，
输出轨迹基元终端状态及评分。MoSim运行适配器将其转换为候选轨迹，经过Trajectory Validator
和Trajectory Server后再交给px4ctrl或生成控制器执行。学习策略不直接输出电机命令，也不
绕过现有安全、控制和证据链。
...
统一Action API覆盖`prepare_experiment`、`train_policy`、`export_policy`、`register_policy`、
`run_mil`、`generate_code`、`start_gazebo`、`start_mission`、`apply_injection`、`stop_run`和
`analyze_result`。每个Action携带`request_id`、Profile hash、期望运行状态和确认策略，保证
按钮操作、Agent批处理和自动回放得到同样的校验与审计结果。
...
Diff-Planner、FUEL、RACER、覆盖规划和YOPO通过统一任务注册、轨迹合同和评价流程接入。
Isaac负责训练，Gazebo/PX4负责工程验证，策略产物带版本、hash、观测定义、延迟预算和回退
规划器，能够与经典规划算法在同一场景和指标下比较。
