...
- 点击启动、代码生成或训练后缺少阶段反馈，用户容易重复点击并启动冲突进程；
- 参数、实时曲线和链路延迟不可见，难以判断问题来自控制、通信还是显示；
- 手工调参需要反复修改、运行、筛选和整理结果，无法稳定复现最优参数。
...
- 创建自动调参任务，配置搜索空间、目标、预算和验证集并查看候选排行榜；
...
- 通用参数与信号查看器，以及Gazebo、Bridge、UE和MWORKS镜像延迟状态；
- 所有长操作的真实阶段进度、已用时间、超时预算、取消、恢复和失败说明。
...
启动Gazebo、代码生成、策略训练、自动调参和结果分析等耗时操作都进入统一Operation Center。
界面依据真实子系统ACK显示当前阶段和进度，重复点击附着到同一任务，不会启动第二份runtime。
用户可以离开或重启GUI，回来后继续查看原操作、日志和结果。
...
同一Signal Registry同时供Flight Console低延迟查看和MWORKS低频实时分析镜像使用。用户可
勾选PX4、定位、规划、控制、安全、故障和多机变量；平台同时显示源频率、数据年龄、UE链路
P50/P95延迟、丢包、插值缓冲和MWORKS镜像延迟。显示链始终与实时控制解耦。
