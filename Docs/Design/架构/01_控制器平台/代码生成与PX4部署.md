...
Generated-C equivalence:
  Results/g9/controller_family_attitude_thrust_v1/g9_family_generated_c_gate_20260630_195728/RUN_MANIFEST.json
Static ROS/Sunray adapter:
  Results/g9/controller_family_attitude_thrust_v1/g9_family_ros_sunray_adapter_gate_20260630_200721/RUN_MANIFEST.json
```

这些证据只覆盖生成源码、数值一致性和px4ctrl/MAVROS姿态加归一化推力命令
形状，本身不等于Gazebo回灌闭环。当前看板已进一步记录 official-PID
generated-family 的可逆 Sunray/ROS wrapper build、单机Gazebo、Diff单机和
Diff三机回归证据。后续使用本文时，只对新选中的控制器或增强组合重复这条
证据链，不要把已关闭的G9生成族回灌路线当作下一步重新执行。
...
不得只对误差最小的控制器做代码生成后跳过其他implemented/accepted候选。
增强层必须和名义控制器组合后单独记录profile、参数、开关状态和消融证据。
```

## 0.0.1 MWORKS live gate与掉激活处理

px4ctrl Golden Slice 的前半段必须尽量离线完成。进入 Sysplorer/MWORKS
live 模型检查、代码生成或GUI布局审核前，先执行 no-start probe 或引用最新
用户授权的激活/窗口巡检证据：
...
工程任务不得自行点击登录、激活、授权、保存、关闭、重启或错误报告控件。
若出现以下任一状态，立即停止 live MWORKS/model/GUI 工作，保留当前源码和
日志，形成 blocker：
...
背景截图不能证明激活/许可成功；若需要证明恢复成功，必须由用户或PMO明确
授权后，使用目标 MWORKS/Sysplorer/Syslab 主窗口前台或最大化截图证据。没有
该证据时，只能说“本次check_model/codegen局部运行足够”，不能说永久激活
状态已经恢复。

## 0.1 代码生成生命周期矩阵

每一次MWORKS模型、生成代码或C++ wrapper进入闭环前，必须形成可追溯的
生命周期记录：

| 阶段 | 必填记录 | 门禁 |
| --- | --- | --- |
| Model Freeze | `model_id`、模型文件路径、模型hash、数据字典hash、solver/step配置 | 模型与参数profile可复现 |
| Algorithm Profile | controller类型、接口层级、所需参考阶次、参数profile/hash、状态变量定义 | 与统一控制接口匹配 |
| Codegen | 生成工具版本、生成配置、源码hash、ABI版本、编译器与目标平台 | 生成代码不依赖ROS/Gazebo/PX4类型 |
| Wrapper | `IController` wrapper版本、单位转换、四元数约定、推力语义 | 输入输出与core ABI一致 |
| Offline Equivalence | 原始实现、抽取core、MWORKS模型、生成C代码同输入序列对齐 | 容差profile通过 |
| Gazebo Reinjection | 同一Sunray/ROS包装层、同一轨迹、同一状态源、同一参数profile | 闭环A/B指标可比 |
| Freeze / Release | release manifest、profile hash、测试报告、已知限制 | 允许进入控制器候选队列 |

任何“代码生成成功”只代表生成阶段完成；只有离线一致性和Gazebo回灌门禁
同时通过，才允许称为可用于系统闭环的生成控制器。

## 0.2 PX4 uORB部署门禁矩阵

MWORKS生成代码进入PX4不是一次性动作，而是从平台无关控制核心逐步进入PX4
Module、uORB、参数系统和板级配置。门禁命名固定如下：

| Gate | 范围 | 必须证明 | 失败时允许做什么 | 禁止声明 |
| --- | --- | --- | --- | --- |
| G-CODEGEN-0 | 生成源码 | 生成C/C++可被主机工具链独立编译 | 修改模型或wrapper后重跑 | PX4可部署 |
| G-CODEGEN-1 | 离线一致性 | 生成代码与参考core同输入序列对齐 | 修正dt、reset、饱和、类型和参数语义 | Gazebo闭环成功 |
| G-ROS-0 | ROS/Sunray回灌 | 生成core经IController和MAVROS闭环A/B通过 | 保留原core或C++ core继续作为基线 | PX4内部Module成功 |
| G-PX4-0 | 空PX4 Module | `px4_add_module`、Kconfig、module.yaml、目标SITL编译通过 | 只修构建和目录结构 | 控制器接入 |
| G-PX4-1 | uORB影子订阅 | 订阅状态、参数更新、ulog诊断正常，不发布控制 | 继续影子运行和日志检查 | 控制闭环 |
| G-PX4-2 | uORB影子计算 | Module调用控制core，输出candidate/status但不争抢setpoint | 对齐数值和实时性 | 可以解锁飞行 |
| G-PX4-3 | SITL控制发布 | 在明确模式和控制权条件下发布标准setpoint，SITL稳定 | 触发Safety回退并保留ulog | 板载可用 |
| G-PX4-4 | 板级编译与参数 | 目标飞控固件可编译，参数在QGC/参数系统可见且持久 | 修board、module.yaml、airframe和默认参数 | HITL/真机可飞 |
| G-PX4-5 | HITL候选 | 真实调度、链路、日志、回退和实时性通过 | 限定为HITL问题排查 | 真机完成 |

G-PX4系列不能反向阻塞第一阶段`ATTITUDE_THRUST`的ROS/MAVROS最小闭环。
第一阶段只需要G-CODEGEN-0、G-CODEGEN-1和G-ROS-0；PX4内部Module是后续
部署路线。

## 0.3 PX4 Module产物清单

进入G-PX4-0前，必须明确以下产物，不允许只拿生成C代码去改PX4：

```text
src/modules/mosim_controller/
  CMakeLists.txt              # px4_add_module入口
  Kconfig                     # 是否编译模块和候选算法
  module.yaml                 # 参数元数据和QGC显示基础
  mosim_controller_main.cpp   # PX4 Module生命周期和命令入口
  mosim_controller_params.*   # PX4参数读取和更新
  mosim_uorb_adapter.*        # uORB到MoSim标准Frame的映射
  mosim_command_publisher.*   # 标准setpoint发布和发布权检查
  generated/                  # MWORKS生成代码，只读导入
  wrapper/                    # IController/ABI包装
  tests/                      # 主机侧和SITL侧最小测试
boards/<vendor>/<board>/default.px4board 或等价配置
airframes/或启动脚本默认参数片段
ulog topic清单和参数hash记录
```

生成目录必须视为只读导入物。若需要改控制律，回到MWORKS模型或平台无关
wrapper；不得直接修改生成C代码后宣称模型到代码链路通过。

## 0.4 代码生成和PX4部署失败处理

| 失败模式 | 典型原因 | 必须保留的证据 | 处理策略 |
| --- | --- | --- | --- |
| 生成代码主机编译失败 | 动态内存、可变尺寸、非嵌入式API、头文件路径 | 编译命令、错误日志、生成配置hash | 修改模型代码生成约束或wrapper，不进入ROS/PX4 |
| 离线输出不一致 | float/double差异、dt、四元数符号、积分reset、限幅顺序 | 输入序列、输出diff、容差profile | 先修离线一致性，再谈闭环 |
| ROS回灌不一致 | 推力映射、坐标转换、状态时间戳、参数默认值不同 | A/B run_id、rosbag、profile hash | 固定除wrapper外所有变量做二分排查 |
| PX4构建失败 | Kconfig未引用、CMake路径错、module.yaml格式错、消息未生成 | PX4 target、完整build log、变更diff | 先用空Module通过G-PX4-0 |
| 参数不可见 | module.yaml缺失、前缀过长、参数未注册、board未启用 | QGC/param dump、参数元数据 | 修参数元数据和board配置，不改控制律 |
| uORB schema不匹配 | topic字段版本差异、时间戳单位错、坐标系错 | topic echo/ulog、PX4版本hash | 写适配层，不把uORB类型传入core |
| 控制发布冲突 | 原生控制器和MoSim Module同时发布setpoint | uORB发布者记录、飞行模式日志 | 进入影子模式，修控制权仲裁 |
| deadline miss | 算法耗时、内存分配、work queue选择错误 | perf counter、CPU/stack/heap记录 | 裁剪算法或降级为外置ROS/伴飞计算 |
| SITL不稳定 | 输出NaN、饱和、坐标反向、参数错误 | ulog、Safety事件、控制输出序列 | 自动回退基线控制器，禁止继续放大实验 |

每个失败都必须形成`blocker`或`run packet`。没有日志和profile的失败不能作为
架构结论，只能作为待复现实验。
...
```text
/mosim/uav1/reference/trajectory
/mosim/uav1/state/vehicle
```
