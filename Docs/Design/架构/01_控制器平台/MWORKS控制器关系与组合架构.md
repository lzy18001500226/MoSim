...
任务 / 轨迹 / 场景
  ...
  -> 参考约束 (可选，如 Reference Governor)                           |
                                                                          v
状态 / 传感器 -> [位置 / 平动外环: 恰好一个] -> 期望姿态 + 总推力 -> [增强: 0..N]
                                                                          |
                                                                          v
...
| 增强/扰动补偿 | 架构 0..N | 状态、参考、候选命令 -> 修正项/命令 | 只能挂到模块声明的作用点 | AWFF、PID-INDI、L1 等不替代外环；当前 APP 单个下拉框最多选一个增强候选 |
...
一致性和后端合同决定能否运行**；不是任意笛卡尔积都可直接开始 MWORKS 或 PX4 仿真。
