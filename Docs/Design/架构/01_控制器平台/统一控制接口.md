...
> 状态：当前统一控制接口专题规范，2026-06-24。
> 当前实际输出层级以 `ATTITUDE_THRUST` 为限；下文的其他输出层级是接口预留，
> 不是当前可运行 Adapter 或控制器验收结论。当前 G1-G7 的执行边界以
...
### 0.1 离线 MWORKS 姿态轴与混控符号合同

`Vehicle.Sensors.AbsoluteAngles` 使用 `{1,2,3}` 旋转序列，`AngleMea[1..3]`
按当前离线模型分别传递 roll、pitch、yaw。离线 `ROTOR_COMMAND` 链路的
`OfflineAttitudeRateAllocator` 不是通用 PX4 分配器：它保留官方 MWORKS PID 内核的
既有符号约定。官方内核把 roll 测量先经过 `gain13(k=-1)` 再送入 `Feedback`，因此其
roll 通道等价于 `roll_ref + roll_mea`；pitch、yaw 通道为 `ref - mea`。共享分配器必须
保持这一对应关系，不能因为表面上“一个加号、两个减号”而单独翻转 roll。

任何符号调整必须同时通过以下三个约束：

1. `AllocatorOfficialPidParityProbe` 的固定输入混控奇偶一致性；
2. `AllocatorRollAxisSignPlantSmoke` 与 `AllocatorPitchAxisSignPlantSmoke` 的正参考轴响应；
3. 以 `Sunray150VirtualPx4Classic.mworks_rotor_center_m`、
   `mworks_spin_command_sign` 和 `mworks_yaw_direction` 为唯一的离线转子顺序来源。

上述约束只说明当前 MWORKS 离线植物与官方 PID 的内部符号一致，不等同于 PX4/Gazebo
实时姿态环、机体系或飞行性能已经验收。
...
MoSim 的统一接口定义以下可扩展接管层级；当前只实际启用 `ATTITUDE_THRUST`，
其他层级必须先有 Adapter、下游 owner 和相应证据门，不能由枚举存在推断可运行：
...
/mosim/uav1/reference/horizon

/mosim/uav1/state/vehicle
/mosim/uav1/state/actuator
/mosim/uav1/state/battery
/mosim/uav1/state/estimator

/mosim/uav1/control/command
/mosim/uav1/control/status

/mosim/uav1/fault/actuator_health
/mosim/uav1/disturbance/estimate

/mosim/uav1/controller/select
/mosim/uav1/controller/parameter_update
```
