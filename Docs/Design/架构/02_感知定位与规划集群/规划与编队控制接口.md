...
>   任务算法Registry、场景地图Registry、算法/地图选择和QGC二维地图的跨域权威接口
>
> 后续关联文档：
>
...
## 0.1.2 多机通信与联合仿真基础设施边界

近期调研的外部资料包括：

| 资料 | 可吸收内容 | MoSim归属 | 当前结论 |
| --- | --- | --- | --- |
| [ROS多机集群组网通信（一）](https://blog.csdn.net/m0_73745340/article/details/142556982) | Ad-Hoc、BATMAN、OLSR、ZeroMQ桥接的分层思路 | 网络/通信基础设施 | 作为多机网络设计参考，不作为QGC飞行控制实现 |
| [swarm_ros_bridge](https://blog.csdn.net/benchuspx/article/details/128576723) | ROS话题/服务选择性转发、ZeroMQ、断线重连 | `multi_vehicle_bridge`适配层 | 可作为ROS1多机通信候选，不替代MAVLink/MAVROS |
| [Multibotnet](https://blog.csdn.net/m0_73745340/article/details/146445055) | 多机ROS话题和服务共享、任意消息类型扩展 | 多机传输适配层参考 | 需经过MoSim消息白名单和版本合同，不直接把任意消息暴露给QGC |
| [Ubuntu Ad-Hoc组网](https://blog.csdn.net/m0_73745340/article/details/146459854) | 网卡、IP、BATMAN/OLSR配置与检查 | 网络预检/启动脚本 | 不在QGC中复刻网卡和路由配置界面 |
| [Ubuntu 20.04图形化Ad-Hoc配置](https://blog.csdn.net/m0_73745340/article/details/147921824) | 通过图形工具建立基础网络连接 | 网络预检体验参考 | QGC只显示连通性和链路质量，不拥有操作系统网络管理权 |
| [MATLAB/Simulink与WSL2下ROS2/PX4联合仿真](https://blog.csdn.net/m0_73745340/article/details/155386514) | 固定步长、Simulation Pacing、实时调度、ROS消息发布订阅 | 未来联合仿真适配器 | 当前ROS1/MAVROS主线不直接采用ROS2/XRCE-DDS链路 |

这些资料必须按职责分层吸收，不能全部转换成QGC的配置项。MoSim的最小通信架构为：

```text
网络基础设施
  -> 链路、IP、路由、时间同步、重连
multi_vehicle_bridge
  -> 白名单状态、心跳、任务分配、编队状态和轨迹摘要
ROS1/MAVROS/PX4
  -> 每架飞机的飞行控制、状态反馈和安全状态
Orchestrator
  -> Profile、任务、运行生命周期和唯一裁决
QGC/Flight Console
  -> 飞行操作、状态审核、告警和可视化入口
```
...
`multi_vehicle_bridge`可以转发下列低带宽、可审计数据：

```text
vehicle_state_summary
vehicle_heartbeat
vehicle_pose_velocity_attitude
formation_reference
formation_status
mission_assignment
trajectory_summary
link_quality
fault_and_degradation_state
```

第一版默认禁止通过该桥接层转发原始MID360点云、全量地图、原始IMU和其他高频大数据。
这些数据应在各自ROS/Gazebo实例本地处理；跨机只交换规划或协同所需的摘要。所有转发
消息必须带`vehicle_id`、`frame_contract_id`、源时间戳、序列号和schema版本。

### 0.1.2.2 QGC与网络层的边界

QGC/Flight Console允许显示：
...
QGC不直接修改网卡模式、IP、信道、BATMAN/OLSR路由、防火墙或NetworkManager配置。
这些操作由受控启动器/基础设施脚本完成；QGC只能通过Orchestrator请求“预检”或“重新
连接”，不能绕过Orchestrator执行任意shell、roslaunch或控制话题。

### 0.1.2.3 联合仿真边界

外部Simulink资料验证了实时联合仿真需要固定步长和节拍控制，但其ROS2/PX4/XRCE-DDS
实现不能直接替换当前ROS1 Noetic/MAVROS/PX4/px4ctrl主线。当前设计保留两条路线：

```text
A: MWORKS在线控制器 -> ROS1/MAVROS/PX4 -> Gazebo
B: MWORKS生成代码 -> ROS1/Sunray/PX4运行时 -> Gazebo
```

QGC两条路线均只负责连接、飞行状态、任务和安全操作；MWORKS不通过GUI节拍驱动高频
控制。任何`mworks_live`开放前必须通过RT0频率、deadline、失联回退和悬停/降落门禁。
...
## 0.3 MWORKS大地图三机可重构编队自主避障边界

### 0.3.1 产品目标

MWORKS“在线建模验证”中的多机目标不是让固定三角编队沿预设轨迹播放动画，
而是在现有复杂STL大地图中完成三机可重构编队自主避障：宽阔区域保持三角形，
狭窄通道自动切换为纵队，通过后恢复三角形；三架飞机必须使用各自的真实动力学
闭环执行轨迹，并同时满足机间和机障安全距离。

复杂地图存在墙体、立柱和窄通道。固定三角形作为刚体整体规划容易无解，因此：

```text
碰撞安全 > 单机飞行稳定 > 轨迹连续性 > 队形保持
```

队形不能只做统一比例缩放。至少应支持：

```text
TRIANGLE -> TRANSITION_TO_COLUMN -> COLUMN -> TRANSITION_TO_TRIANGLE -> TRIANGLE
```
...
```text
Models/MoSimQuadrotorModel/Guidance/Formation/Scenarios/
  FormationTriangleFigure8LinearMPCSysblockClosedLoop.mo
```

该模型具备三套`QuadChassis`、十二个执行器、三套传感器和三套控制器。领导机
跟踪平面8字参考，两架跟随机使用领导机理想参考叠加固定偏移，形成固定三角形
共同运动。当前认证记录为：

```text
Results/mworks_generated_profiles/
  cert-three-uav-linear-mpc-figure8-20260719-v2/CERTIFICATION.json
```

该认证只证明命名模型的仿真结果和原生动画入口可用，不证明复杂地图避障、在线
规划、真实分布式通信或队形重构。当前跟随机没有通过通信链路读取领导机实时状态，
而是使用集中式共享理想参考；没有通信延迟、丢包、断链或拓扑切换模型。

现有复杂地图显示资产由下列模型使用：

```text
Models/MoSimQuadrotorModel/Guidance/Planning/Scenarios/PlanningNavigationDisplay.mo
```

当前三机8字模型没有接入该地图显示、碰撞查询、规划器或局部地图。现有单机大地图
路线主要使用预先生成航点和`PlannedQuinticReference`插值，再由MWORKS动力学闭环
跟踪；它不能作为运行时在线自主探索证据。

注册表中的九种编队模式及其C核心、MIL、官方代码生成和SIL证据属于算法模块底座。
`implemented/selectable=true`不能解释为三机大地图闭环已经完成。特别是当前
`formation_reconfiguration`只在重构标志生效时将队形偏移统一乘以默认比例`0.65`，
不能自动识别通道，也不能完成三角形与纵队之间的拓扑切换。当前`formation_cbf`
只处理机间距离，不处理无人机与墙体/障碍物距离。

### 0.3.3 当前完成度口径

按“复杂STL大地图三机可重构编队自主避障完整MWORKS闭环”计算，当前完成度约为
`20%~25%`。该比例是范围估算，只用于范围和优先级判断，不是验收指标。
...
```text
STL或占据地图到规划可查询碰撞模型的转换
编队质心路线或三机时空轨迹规划
三角形、纵队等队形模板及合法间距参数
地图/通道宽度驱动的队形选择状态机
连续、动力学可行的队形变换轨迹
无人机-墙体与无人机-无人机联合安全约束
编队参考到三机独立动力学控制链的统一Adapter
三机复杂地图统一MWORKS图形模型
Result.msr、动画、碰撞距离、编队误差和任务完成指标
Model Studio合法Profile、状态显示和执行门禁
多场景回归、失败回退和最终认证
```

### 0.3.4 比赛视频优先的第一版冻结

比赛视频优先阶段冻结为“已知地图预规划 + 运行时自动队形切换”，不做未知环境
实时自主探索：

```text
输入：现有完整STL地图及其可查询碰撞/通道模型
规划：仿真前生成编队质心可行路线和通道宽度信息
重构：运行时按路线事件自动执行三角形 -> 纵队 -> 三角形
执行：三架飞机在MWORKS内使用真实动力学闭环跟踪各自连续轨迹
显示：原生结果查看器显示同一复杂地图、三机姿态、轨迹和队形变化
指标：任务完成、零碰撞、最小机间距、最小障碍距离、编队误差、重构耗时
```

按这一收缩目标估算，当前可复用基础约为`35%`。第一版不得宣称未知环境探索、
运行时局部地图发现或完整在线多机重规划。真正的局部感知、在线地图更新、三机
实时规划和动态重构属于后续阶段，必须另行通过规划时限、通信和失败回退验收。

### 0.3.5 第一版完成门槛

只有同时满足以下条件，才能将第一版标记为完成：

1. 同一个MWORKS整机模型加载复杂地图和三架独立动力学无人机；
2. 至少一次自动完成`三角形 -> 纵队 -> 三角形`，不能靠人工切换结果文件；
3. 三机轨迹连续且没有翻滚、炸机、穿墙或机间碰撞；
4. 结果中可查询每机位姿、队形状态、机间距离、障碍距离和任务状态；
5. 生成当前运行的`Result.msr`、指标文件和原生动画入口；
6. Model Studio只能开放已具备合法地图、队形、控制器和证据组合的运行按钮；
7. 任何静态源码门禁、旧`raw/metrics`、代码生成或其他仿真器结果均不得替代上述
   MWORKS整机运行证据。
