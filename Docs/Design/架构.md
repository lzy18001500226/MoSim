...
必须释放指针。Factory路线先做L2静态导入Gazebo和UE Global Overview姿态轨迹视图；
...
每个需要 QGC 二维态势的已发布 ExperimentProfile 显式声明 `operator_map_id`。`prepare_run`
从已启用的 Map Registry 深拷贝完整条目到 `operator_map_snapshot`，并写入确定性的
`operator_map_snapshot_hash`；准备后的 QGC、live sidecar 和 rosbag 回放只消费该快照。QGC
仅在尚未准备运行时可显示目录默认图，已有 `run_id` 但没有合法快照时必须显示阻断状态，不能
退回到可变目录默认值。sidecar 的 `--map-id` 和 `--coordinate-contract-id` 仅作一致性断言，
不匹配即拒绝启动；`--coordinate-contract-status=verified` 不得作为运行时开关。坐标校验可以
通过绑定同一地图快照的证据文件把状态更新为 `verified` 或 `rejected`，但不能改变快照身份或其哈希。

进入 QGC Bridge 前，Orchestrator 必须再次校验 `map_state` 的 run/profile/map 快照身份、
传输时间语义、车辆编号、几何边界和回放字段；当状态为 `verified` 时，车辆位置和轨迹还必须
显式声明同一 `world_frame`。校验失败只拒绝该遥测帧，不改变飞控或控制器状态。

#### Rosbag 派生回放入口

`Scripts/ui/replay_rosbag_operator_map.py` 是 Factory 二维态势的离线回放入口。它只读取
ROS1 `.bag` 中每架飞机的 Odom 记录，或读取仅供测试的标准化 JSONL 导出；不发布 ROS 消息、
不解锁飞机、不启动 Gazebo/PX4，也不向控制器或规划器回写任何数据。它以已准备运行目录的
`RUN_MANIFEST.json` 为唯一身份来源，并产生：

```text
OPERATOR_MAP_REPLAY_MANIFEST.json  # 源文件路径/sha256、bag_id、话题映射、坐标证据、帧数和时长
...
第一版不连续流式发送滑块值；
QGC命令面板必须显示、复制或启动已验证的完整终端命令，用户始终可见日志、报错和停止状态；
...
`profile_id`、显示名称、启用状态、操作模式和受控任务关键词；
`src/orchestration/core.py:list_operator_profiles` 必须从对应 ExperimentProfile、控制器
注册表和地图注册表派生 `controller_id`、`vehicle_count`、`operator_map_id` 与 Profile 哈希。
因此 QGC 只能选择一个已发布 Profile，控制器、机数和地图在界面中只读显示；禁用项仍可见，
但必须显示禁用原因。准备成功后，`RunManifest` 冻结同一 Profile 哈希，起飞前后均不能改选。

第一版故障请求使用同一逻辑合同的控制面路径：

```text
QGC 暂存风扰或单电机效率
  -> stage_injection(command, run_id, profile_hash)
  -> RunManifest.pending_injection
  -> 人工点击“应用待应用故障”
  -> apply_staged_injection(run_id)
  -> 后端 ACK + injections 记录

QGC 点击“恢复正常”
  -> restore_normal(run_id, vehicle_id)
  -> 风扰 0.0 + 电机 1..4 效率 1.0 的五条离散命令
  -> 逐条 ACK；任一失败返回 restore_normal_partial_failure 并保留逐条结果
```

`apply_injection` 和 `restore_injection` 仅保留为旧调用方兼容接口，QGC 操作面不得直接使用。
上述契约已经有源码和测试覆盖，但不证明 Gazebo、PX4、MAVROS 或真实飞行端已接受任一命令；
运行时结论仍需同一 `run_id` 的后端 ACK、遥测和日志。
