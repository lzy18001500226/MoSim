...
Ubuntu-20.04 / ROS1 Noetic
  -> 项目本地 `src/` Sunray、FAST-LIO、px4ctrl 与 Livox 插件工作区
  -> Sunray150 + MID360
  -> Gazebo Classic
  -> PX4 + MAVROS
...
`PX4CTRL_Original_OuterLoop_Graphical_Sysblock` 的 C99 产物已通过
`graphical_px4ctrl_c99` 后端接入项目本地
`src/control/runtime_adapters/px4ctrl`、MAVROS、PX4 和 Sunray/Gazebo。
以下三条记录均保留 `RUN_MANIFEST`、原始状态/轨迹数据和注入确认：

```text
名义起飞-悬停-降落：
  Results/sunray_ros1/sunray_ros1_graphical_c99_takeoff_hover_land_20260731_002/
风扰注入：
  Results/sunray_ros1/sunray_ros1_graphical_c99_wind_hover_20260801_002/
转子1效率0.85与恢复：
  Results/sunray_ros1/sunray_ros1_graphical_c99_motor_fault_recovery_20260731_002/
```

这证明 px4ctrl 的这一条图形 C99 部署链可以在源本地 Gazebo/PX4 环境中运行，
且风扰与电机效率注入可到达物理 Gazebo 执行器。它不证明所有控制器均已部署、
严格同 FAST-LIO 参数下的性能 A/B、完整故障容错或 QGC/UE 的显示验收。
名义记录使用 FAST-LIO `0.02 m` 体素参数，风扰和电机效率记录使用 `0.5 m`；
该差异必须随各自 `RUN_MANIFEST` 保留。

当前唯一正式的 Windows 复现入口和结果检查顺序见
`Scripts/cmd/README.md` 与 `Docs/Workflows/sunray_ros1_execution_checklist.md`。
QGC、UE 和 RViz 仍是独立显示/审核面，未包含在这三条 C99 运行证据的通过结论内。
...
vehicles[], task_paths{expected?, future?}, actual_tracks{uavN?}, task_boundary?, formation_target?,
...
`actual_tracks` 是可选的持久化只读轨迹字典，键为 `uav1` 至 `uav9`。每条轨迹必须绑定
同一 `run_id`、`vehicle_id`、`frame_id=map.world_frame`、`updated_at` 与
`semantics=actual_vehicle_track`，其点只能来自已投影的真实车辆遥测；不得由预期路径、
插值缺帧或 UI 推测生成。live sidecar 和 rosbag 回放器均以 5 cm 二维位移阈值有界采样，
最多保留 1200 点，因此任务结束、地面站重启或回放完成后仍可显示最后有效实际航迹，
但不把最后地图帧误称为飞行端当前状态。
...
#### 8.0.1 Config/Results 轻量运行结果契约

本阶段不移动、不删除或回写任何历史结果。`Result.msr`、rosbag、ULog、点云回放、日志和指标都属于
证据或可复现实验输入，不能被当成缓存清理。现有的 P0 `mosim.run_manifest.v1`、QGC
`mosim.operator_run_manifest.v1` 和历史包裹式 `{"run_manifest": ...}` 文件继续保留原样；其既有检查器
和证据含义不因新契约改变。

新创建的 QGC 操作运行采用 `Config/schemas/mosim_run_manifest_v2.schema.json` 的
`mosim.run_manifest.v2`。它冻结以下内容，同时保留 QGC/ROS1 回放消费者需要的顶层
`experiment_profile_id`、`experiment_profile_hash`、`runtime_profile_id`、`vehicle_count`、
`operator_map_snapshot`、`operator_map_snapshot_hash`、`scenario_snapshot`、`controller_backend` 和
`state` 字段：

```text
run_id / run_kind / created_at / status
  -> profile(id + hash) / map snapshot + hash / scenario snapshot + hash
  -> source_state
  -> artifacts:
     mworks_model, native_result_msr, raw_csv, metrics_json,
     rosbag, px4_ulog, operator_map_replay, telemetry, logs_directory
  -> open_actions:
     open_model, open_native_result, replay_rviz,
     replay_operator_map, open_result_directory
  -> claim_boundary
```

每个 artifact 只记录 `not_requested`、`pending`、`declared`、`available`、`missing` 或 `not_applicable`
及可选的相对路径/哈希。历史清单归一化出的 `declared` 仅表示原清单声明了该位置；`available` 也不等同于
控制器、规划器、Gazebo、PX4、MAVROS 或飞行任务已经通过验收。`open_actions` 是后续 MoSim Studio 用于打开模型、结果查看器、RViz
回放入口或结果目录的元数据，不会替用户启动 MWORKS、导出代码或在后台运行仿真。

`Scripts/results/build_run_index.py` 只读取 `Results/runs/**/RUN_MANIFEST.json`，兼容归一化上述三类
历史形态并写出派生的 `Results/RUN_INDEX.json`。它不扫描或移动 `Results/mworks_generated_profiles/`、
`Results/control_platform/` 等其他证据根，也不修改任何 run bundle；不支持或重复的历史条目必须保留在
索引的 `issues` 中，而不是被静默丢弃。该索引仅解决“按 run_id 发现声明的文件和入口”，不是新的运行
权威、证据升级器或自动回放器。
...
Control/
  ...
  Baselines/        正式 Runner 的离线默认控制律和通用核心
  LegacyExamples/   同元 Example1/2/3 PID 变体的直接审查入口
...
进入 `Control/Baselines`，同元 `Example*PID` 变体进入
`Control/LegacyExamples/PidVariants`。`GraphicalMIL` 是历史评审阶段名，迁移后统一为
...
当前Model Studio            -> apps/model_studio/
```
