...
`operator_map_id` 与冻结的运行身份。因此 QGC 只能选择一个已发布 Profile，控制器、机数和
地图在界面中只读显示；禁用项仍可见，但必须显示禁用原因。`RunManifest` 形成后冻结同一
Profile 身份和哈希，起飞前后均不能改选或再次复制启动命令。复制的前台命令首先执行
...
当前 UI 以已发布 Profile 为主选择器，不提供脱离 Profile 的独立控制器拼装。后续控制器下拉框
只能列出能解析到已发布兼容 Profile 的条目；未发布或尚无运行证据的控制器可见但禁用。增强模块、
故障注入、安全过滤和不同输出层级后续按兼容性矩阵逐步释放。因此“自由选择”应理解为受约束的
实验配置选择，不是控制模块任意堆叠。
