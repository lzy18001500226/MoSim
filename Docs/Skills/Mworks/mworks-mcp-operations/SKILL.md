...
16. If the preflight sees demo edition, missing activation, login/activation prompts, authorization errors, an error-report dialog, mixed education/demo state, visible unknown MWORKS/Sysplorer/Syslab window, unavailable tooling, or an unknown state caused by sentinel failure, stop live work and return a `status=blocked` packet to PMO. Hidden Qt/browser-proxy/helper windows with no license/error text are risk evidence, not standalone blockers. Do not tune solvers, patch models, open a new MWORKS window, or click login/activation/error-report controls from a specialist department.
