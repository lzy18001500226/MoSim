...
15. If the patrol or current task evidence shows demo edition, login/activation prompt, authorization failure, GUI-error, mixed relevant windows, visible unknown blocking state, or unknown blocking evidence, engineering departments stop and block the live task. Do not close the suspect window and continue by choosing a clean one.
...
17. Background screenshot evidence can miss the login/license pane. PMO or CoAgentOps may, under the user's current authorization, maximize/focus the existing window and use only the official foreground login/license UI; login/license screenshots must use maximized target-window evidence and must visually show that target window, not Codex/another app/helper windows. If the official login action does not return or cannot complete on the existing window, PMO/CoAgentOps may reopen MWORKS and log in through the official UI as a bounded recovery. Use only the approved secure credential source and never write credentials to docs, logs, packets, screenshot manifests, email, or terminal output. Stop on MFA/captcha, account/password error, abnormal authorization, unknown modal/window, crash/error-report, save/overwrite prompt, or any non-MWORKS credential surface. After success, close only the login/license dialog when possible and keep the reusable main window open.
...
| demo edition / activation lost / login prompt | engineering department stops live MCP work and returns `license_or_login` blocker; PMO/CoAgentOps may perform bounded official login recovery under user authorization and credential-redaction rules |
