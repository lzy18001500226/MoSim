...
9. `MCP Transport closed`, a stale Sysplorer endpoint, or an ordinary recovery-after-abnormal-exit dialog is a recoverable tool incident, not a project blocker. End the clearly identifiable `mworks` and `sysplorer-acp-server` processes, restart MWORKS, maximize the target main window, handle the normal recovery dialog under the Failure Handling rule below, rediscover the live port through the official API, and retry the same smallest operation. If the native MCP channel remains stale but official `ModelingPy` can connect to the recovered window, continue through that official API and preserve the recovery log. A model/compiler/connection error exposed after recovery is a route-level engineering result to repair or record while the next route proceeds.
9a. When the current task is authorized to open or operate a new or reopened MWORKS/Sysplorer/Syslab main window, visually classify any immediately visible modal, then maximize the main window as soon as it is operable and before the first model or MCP operation. Inspect the full window for automatic recovery, demo edition, activation/login/authorization, error-report or crash-dump dialogs, an unexpected active window, and any unknown modal. A clean view records only the absence of an obvious UI blocker; it does not prove activation or replace `check_model`/simulation evidence. After a clean inspection, minimize the window unless the next explicitly authorized step needs foreground GUI interaction. If a problem is visible, follow rules 15, 17, 22, or the Failure Handling table before continuing; do not minimize the incident window. Do not repeat this gate when reusing an unchanged window in the same batch.
...
| Sysplorer `自动恢复` dialog after abnormal exit | classify it as recoverable `gui_blocked`, not a license/login incident. With current-task desktop-action authorization, choose `丢弃所有` when the task does not explicitly require unsaved model recovery; choose `恢复已选` only when the task explicitly requires that recovery. Verify that the dialog is gone and the existing Sysplorer main window remains usable. Stop instead on save/overwrite, license/login/authorization, crash-report, or other unknown dialog text. |
