...
1. **Activation/window gate**: reuse current MWORKS/Sysplorer/Syslab activation/window evidence when available. Static file-only work records `live_mworks_touched=false`; live MCP/model/GUI work records `live_mworks_touched=true`. If no current activation/window evidence exists and live work is needed, run at most one bounded sentinel/API check or return a blocker. If current-turn evidence is collected, inspect the JSON/capture/window-title evidence and classify it in the same turn; path-only evidence is not enough. Stop with `status=blocked` when current evidence shows demo, unactivated/login, authorization, mixed, visible unknown blocking, unknown blocking state, or a GUI error whose license/login meaning is unclear. Bounded official login recovery is allowed only when explicitly user-authorized, with foreground/maximized evidence and credential redaction. Hidden Qt/browser-proxy/helper windows with no license/error text are risk evidence but do not alone block live work. If the screenshot shows a non-license internal/compiler/dmp issue, capture the evidence, restart/reopen MWORKS once, and retry the same smallest command. If it repeats, continue as model/codegen/topology debugging. If the screenshot says missing wires, unconnected ports, no connection, `未连线`, or equivalent topology wording, this gate passes to graphical repair rather than activation recovery.
...
| internal compiler error or `.dmp` prompt with no license/login marker | capture screenshot/error/dmp path, restart or reopen MWORKS once, retry the same smallest command, then debug model/codegen/topology if repeated |
| screenshot says missing wires / unconnected ports / `未连线` | classify as graphical topology and repair ports/wiring; do not route to activation recovery |
| demo edition / activation lost / login prompt / mixed or unknown license state | stop live diagram work and return a `status=blocked` `license_or_login` blocker with current evidence; bounded official login recovery is allowed only under explicit user authorization |

## Output

State the model file, graphical model name, gates run, logs produced, behavior-equivalence status, and remaining risks. For live MWORKS work, include `mworks_activation_reference` when known, `will_not_click_activation_login=true`, and `live_mworks_touched`. If current-turn sentinel/capture evidence is collected, include `activation_sentinel_before`, `gui_sentinel_before`, `background_screenshot_before`, `activation_state_observation`, `license_state`, and `mworks_window_evidence_touched=true`. For live graphical/layout review, also include `mworks_phase_screenshots`, `mworks_phase_observations`, and `screenshot_manifest` describing the visible wiring/layout state.
