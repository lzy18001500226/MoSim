...
本目录用于存放从 MWORKS 官方资料包中筛选、索引和转换出来的项目相关资料，供 Codex/Agent 后续查询。当前项目运行和 Agent 查询以本目录的 Markdown 索引与转换结果为准，不再依赖原始资料包。
...
```text
1. Docs/MworksDocs/scan/scan_summary.md
2. Docs/MworksDocs/scan/relevant_index.md
3. Docs/MworksDocs/scan/categories/*.md
4. Docs/MworksDocs/converted/转换索引.md
5. Docs/MworksDocs/converted/**/*.md
6. 如转换结果不足，再临时补充对应官方原始 PDF 或 MCP 官方文档
```

## PDF 说明

已将第一批 P0/P1/P2 高价值 PDF 转换到 `Docs/MworksDocs/converted/`。当前 17 份目标 PDF 均已通过 MinerU 精准解析生成 Markdown，并同步保存各自的 `_images/` 图片目录；另有 1 份 MWORKS/MATLAB 在线链接索引已整理到 `converted/matlab_compat/`。
...
默认不再生成 `Docs/MworksDocs/extracted/`，因为该目录主要是自动扫描碎片，和 `converted/` 重复且噪声较多。如确实需要临时摘录文本文件，可显式运行：
...
脚本会把 URL 写入 `Docs/MworksDocs/tmp/mineru/pending_downloads.md`，后续可手动下载后用 `--import-mineru-result` 导入。

`Docs/MworksDocs/tmp/` 仅用于转换缓存和中间结果，不作为知识库入口；确认 `converted/` 已生成后可以安全清理。
