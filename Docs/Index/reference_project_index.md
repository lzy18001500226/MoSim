...
| PX4 source family | `References/PX4/` | Flight-stack and control/runtime reference family; actual PX4-Autopilot source tree is `References/PX4/PX4/` |
| RflySim materials | `References/RflySim/` | RflySim packages and scene reference |
| ROS2 source snapshots | historical/not currently present | ROS2 organization source snapshots for client libraries, RMW/DDS layers, interfaces, launch, CLI, RViz, rosbag2, bridge, tracing, docs, and selected integration references; update workflow: `Docs/Workflows/reference_snapshot_update.md` |
...
These rows keep the machine-checkable index aligned with the current
`References/` tree after the legacy agent-OS documentation migration.

| Project | Path | Primary Use |
|---|---|---|
| CarlaAir | `References/AirSim/CarlaAir` | simulator/runtime reference |
| Control family root | `References/Control` | control algorithm and controller design references |
| Gazebo family root | `References/Gazebo` | simulator source snapshots and runtime architecture references |
| Lab experiment platforms | `References/Lab/experiment_platforms` | experiment-platform and simulator integration references |
| Lab exploration and coverage | `References/Lab/exploration_coverage` | exploration, coverage, and next-best-view references |
| Lab localization and SLAM | `References/Lab/localization_slam` | localization, mapping, odometry, and sensor-driver references |
| Lab local planning | `References/Lab/planning_local` | local planning and trajectory-generation references |
| Lab swarm coordination | `References/Lab/swarm_coordination` | multi-UAV coordination and formation references |
| Lab visualization | `References/Lab/visualization` | trajectory, map, and robotics visualization references |
| Diff-Planner | `References/Lab/planning_local/Diff-Planner` | planning/reference algorithm source |
| FASTLIO2_ROS2 | `References/Lab/localization_slam/FASTLIO2_ROS2` | localization/reference source |
| Fast-Drone-250 | `References/Lab/planning_local/Fast-Drone-250` | UAV planning/control reference |
| Fast-Planner | `References/Lab/planning_local/Fast-Planner` | planning/reference algorithm source |
| far_planner-melodic-noetic | `References/Lab/planning_local/far_planner-melodic-noetic` | ROS1 planning reference |
| faster | `References/Lab/planning_local/faster` | planning/reference algorithm source |
| livox_ros_driver2 | `References/Lab/localization_slam/livox_ros_driver2` | Livox ROS driver reference |
| livox_ros_driver_compat | `References/Lab/localization_slam/livox_ros_driver_compat` | Livox compatibility driver reference |
| CopterSim | `References/RflySim/CopterSim` | RflySim runtime/reference package |
| Simulation family root | `References/Simulation` | simulation reference materials |
...
| ChatGPT-Exporter-main | `References/Agent/ReviewLater/ChatGPT-Exporter-main` | chat export utility; low-priority and not first-line agent-runtime/MCP infrastructure |
...
| 3dmr | `References/Lab/exploration_coverage/3dmr` | 3D multi-robot/exploration reference |
| CERLAB-UAV-Autonomy | `References/Lab/exploration_coverage/CERLAB-UAV-Autonomy` | ROS Noetic UAV autonomy framework covering simulator, mapping, planning, control, navigation, exploration, and inspection |
| Diff-Planner | `References/Lab/planning_local/Diff-Planner` | fixed-goal single/multi-UAV planning runtime baseline |
| EGO-Planner-v2 | `References/Lab/planning_local/EGO-Planner-v2` | local replanning reference |
| ExplorationRRT | `References/Lab/exploration_coverage/ExplorationRRT` | RRT next-best-trajectory UAV exploration reference |
| FALCON-ros1-noetic | `References/Lab/exploration_coverage/FALCON-ros1-noetic` | autonomous aerial exploration with coverage-path guidance candidate |
| FC-Planner | `References/Lab/exploration_coverage/FC-Planner` | known-scene aerial coverage/reconstruction planning candidate |
| FAST-LIVO2 | `References/Lab/localization_slam/FAST-LIVO2` | lidar-visual inertial odometry reference |
| FAST_LIO | `References/Lab/localization_slam/FAST_LIO` | lidar odometry/mapping reference |
| FASTLIO2_ROS2 | `References/Lab/localization_slam/FASTLIO2_ROS2` | ROS2 FAST-LIO2 reference |
| Fast-Drone-250 | `References/Lab/planning_local/Fast-Drone-250` | px4ctrl and quadrotor message reference |
| Fast-Planner | `References/Lab/planning_local/Fast-Planner` | local planning and map environment reference |
| Fast-Racing | `References/Lab/planning_local/Fast-Racing` | high-speed planning/control reference |
| FUEL | `References/Lab/exploration_coverage/FUEL` | frontier-based single-UAV exploration reference |
| GCOPTER | `References/Lab/planning_local/GCOPTER` | trajectory optimization reference |
| HighStar | `References/Lab/exploration_coverage/HighStar` | high-speed online UAV exploration candidate with ROS Noetic/catkin source |
| MGGPlanner | `References/Lab/exploration_coverage/MGGPlanner` | multi-goal/global planning reference |
| Point-LIO-point-lio-with-grid-map | `References/Lab/localization_slam/Point-LIO-point-lio-with-grid-map` | point-lio and grid map reference |
| RACER | `References/Lab/exploration_coverage/RACER` | multi-UAV exploration and racing-style planning reference |
| SOAR | `References/Lab/exploration_coverage/SOAR` | heterogeneous UAV exploration/photographing/reconstruction reference |
| SUPER | `References/Lab/planning_local/SUPER` | swarm/perception/planning reference |
| Swarm-Formation | `References/Lab/swarm_coordination/Swarm-Formation` | multi-UAV formation reference |
| aerostack2 | `References/Lab/experiment_platforms/aerostack2` | UAV autonomy framework reference |
| crazychoir | `References/Lab/experiment_platforms/crazychoir` | swarm/choreography reference |
| crazyswarm2 | `References/Lab/experiment_platforms/crazyswarm2` | swarm experiment stack reference |
| ego-planner | `References/Lab/planning_local/ego-planner` | local replanning reference |
| ego-planner-swarm | `References/Lab/swarm_coordination/ego-planner-swarm` | multi-UAV planning reference |
| exploration-algorithms | `References/Lab/exploration_coverage/exploration-algorithms` | exploration algorithm survey/build reference |
| far_planner-melodic-noetic | `References/Lab/planning_local/far_planner-melodic-noetic` | ROS1 planning reference |
| fast_multi_robot_exploration | `References/Lab/exploration_coverage/fast_multi_robot_exploration` | multi-robot exploration reference |
| faster | `References/Lab/planning_local/faster` | fast local replanning reference |
| gbplanner_ros-gbplanner2 | `References/Lab/exploration_coverage/gbplanner_ros-gbplanner2` | graph-based exploration planner reference |
| livox_ros_driver2 | `References/Lab/localization_slam/livox_ros_driver2` | Livox ROS2 driver reference |
| livox_ros_driver_compat | `References/Lab/localization_slam/livox_ros_driver_compat` | Livox ROS1 compatibility driver reference |
| mader | `References/Lab/swarm_coordination/mader` | multi-agent trajectory deconfliction reference |
| nbvplanner | `References/Lab/exploration_coverage/nbvplanner` | receding-horizon next-best-view exploration reference |
| rmader | `References/Lab/swarm_coordination/rmader` | robust multi-agent trajectory deconfliction reference |
| skybrush-server | `References/Lab/experiment_platforms/skybrush-server` | swarm ground server/reference |
| tare_planner-melodic-noetic | `References/Lab/exploration_coverage/tare_planner-melodic-noetic` | ROS1 exploration planner reference |
| uav_frontier_exploration_3d | `References/Lab/exploration_coverage/uav_frontier_exploration_3d` | multi-resolution 3D frontier exploration reference |
| visualize_uav_trajectory | `References/Lab/visualization/visualize_uav_trajectory` | UAV trajectory visualization/reference |
...
| Project | Path | Primary Use |
|---|---|---|
| PX4 family root | `References/PX4` | flight-stack reference family; use `References/PX4/PX4` for PX4-Autopilot source files |
...
| Research family root | `References/research` | cross-family research notes and source collections |
...
1. `Docs/Index/agent_learning_strategy.md`
...
1. `References/Lab/localization_slam/FAST_LIO`
2. `References/Lab/planning_local/Diff-Planner`
3. `References/Lab/planning_local/Fast-Drone-250`
4. `References/Lab/exploration_coverage/FUEL`
5. `References/Lab/exploration_coverage/RACER`
6. `References/Lab/exploration_coverage/FALCON-ros1-noetic`
7. `References/Lab/exploration_coverage/FC-Planner`
8. `References/Lab/exploration_coverage/gbplanner_ros-gbplanner2`
9. `References/Lab/exploration_coverage/uav_frontier_exploration_3d`
10. `References/Lab/exploration_coverage/nbvplanner`
11. `References/Lab/exploration_coverage/ExplorationRRT`
12. `References/Lab/exploration_coverage/SOAR`
13. `References/Lab/exploration_coverage/exploration-algorithms`
14. `References/Lab/planning_local/ego-planner`
15. `References/Lab/planning_local/EGO-Planner-v2`
16. `References/Lab/planning_local/Fast-Planner`
17. `References/Lab/planning_local/GCOPTER`
18. `References/Lab/planning_local/SUPER`
19. `References/Lab/swarm_coordination/Swarm-Formation`

For the full Lab inventory and move-risk classification, read
`References/Lab/README.md` first.
