...
| Codex App / VSCode / WSL session and config repair | Infrastructure history is documented; do not touch external `.codex` state unless user explicitly requests infrastructure repair | `Docs/Cache/research/codex_app_session_research_20260526.md`, `Docs/Workflows/debug_mcp.md` | `Docs/Cache/session_memory_migration/02_round2_review/round2_infrastructure_memory_20260604.md` | assuming App/VSCode live sync is durable state; normalizing SQLite/JSONL injection |
...
| Safety, fault, and system-level mission closure | Safety/fault/system scenarios are part of the competition-control story and must remain tied to event logs and MWORKS evidence | `Docs/Design/架构/01_控制器平台/控制增强与容错.md`, `Docs/Design/架构/03_测试调参与证据/测试与评价.md` | `Config/scenarios/system/`, `Config/scenarios/robustness/`, `Models/MoSimQuadrotorModel/Experiment/Scenarios/Robustness/` | treating failure-mode demos as complete without event/metric evidence |
| Multi-UAV formation | Formation is a later extension after single-UAV control/scene evidence is stable | `Docs/Design/架构/02_感知定位与规划集群/规划与编队控制接口.md` | `Models/MoSimQuadrotorModel/Guidance/Formation/Scenarios/FormationTriangleFigure8LinearMPCSysblockClosedLoop.mo`, `Config/scenarios/formation/` | starting formation before single-UAV model/evidence gates are stable |
...
| CoSim future platform blueprint | Future platform design is now vehicle-family-first: multirotor, fixed-wing, VTOL, and benign ducted model-aircraft lines share a common core and select backend adapters from reviewed research decisions; raw research is preserved separately | `Docs/Cache/cosim/blueprint_20260614/README.md`, `Docs/Cache/cosim/blueprint_20260614/00_platform/00_CoSim总体蓝图.md`, `Docs/Cache/cosim/blueprint_20260614/10_shared_core/01_共享内核与数据契约.md`, `Docs/Cache/cosim/blueprint_20260614/20_vehicle_families/README.md`, `Docs/Cache/cosim/blueprint_20260614/30_backend_adapters/README.md`, `Docs/Cache/cosim/blueprint_20260614/research/README.md` | `Docs/Cache/cosim/cosim_rebuild_plan_20260614.md`, `Docs/Cache/cosim/source_migration_manifest_20260614.md`, `Docs/Cache/cosim/architecture_draft_audit_20260614.md`, raw preserved under `Docs/Cache/cosim/blueprint_20260614/research/raw/` | treating simulator names as the product tree; deleting raw research before reviewed conclusions are accepted; presenting CoSim architecture draft as runtime evidence |
...
| MoSimQuadrotorModel formal package | Project-owned formal quadrotor package is the only Modelica source root: `Models/MoSimQuadrotorModel`; its embedded `Plant`, Controllers, scenarios, resources, runners, and LiveIntegration sources are loaded together from one `package.mo`. Former root names are historical provenance only. | `Docs/Design/架构/03_测试调参与证据/真机化与C++化.md`, `Docs/Design/架构/01_控制器平台/控制体系总览.md`, `Docs/Index/simulation_model_structure_index.md`, cached migration plan `Docs/Cache/design/historical_snapshots/absorbed_or_superseded_20260614/12_MoSimQuadrotorModel模型归档与迁移计划.md`, `Docs/Workflows/new_conversation_context.md#5-current-mworks-dynamics-state`, `Docs/Workflows/identify_quadrotor_parameters.md#102-2026-06-07-mosimquadrotormodel-formal-package-migration-rule` | `Models/MoSimQuadrotorModel/package.mo`, `Models/MoSimQuadrotorModel/Vehicle/package.mo`, `Config/control_platform/model_namespace_migration.json`, `PROGRESS.md` latest entries | adding a second model root or restoring legacy aliases to make an old command work |
