...
| Start and stop the Gazebo flight runtime and QGC/UE ground station independently | `Docs/Workflows/qgc_ue_operator_startup.md`; use the `cmd/` launchers and keep Gazebo/PX4/MAVROS as runtime authority |
...
| Expand Model Studio offline controller composition and defer model-directory migration until regression freeze | `Docs/Workflows/model_studio_offline_expansion_goal.md`, `Config/control_platform/offline_expansion_inventory.json`, `Config/control_platform/offline_runner_interface_contract_v1.json`, `Scripts/quality/check_offline_expansion_inventory.py`, `Scripts/quality/check_offline_runner_interface_contract.py` |
| Close out the current 49-scheme controller evidence, model mapping, and MWORKS experiment sequence | `Docs/Workflows/controller_evidence_closeout.md`, `Docs/Index/simulation_model_structure_index.md`, `Scripts/quality/check_controller_evidence_closeout_contract.py`; finish G3 static audit before changing model files in G4 |
...
| Deferred long-term project structure refactor | `Docs/Workflows/project_structure_refactor.md`; reference only until the user explicitly reopens it after experiments |
...
| Generate MWORKS/Sysblock C/C++ controller runtime | `Docs/Workflows/mworks_codegen_controller_runtime.md`, historical G9 closeout at `Docs/Workflows/g9_mworks_generated_runtime_closeout.md`, `Docs/Design/架构/03_测试调参与证据/真机化与C++化.md`, `Scripts/mworks/check_codegen_runtime.py`, `Scripts/tests/test_mworks_codegen_runtime.py` |
