...
| Execute current Sunray ROS1/Gazebo/RViz lane and source boundaries | `Docs/Workflows/sunray_ros1_current_runtime_lane.md`, `Docs/Index/sunray_migration_index.md`, `References/Sunray`, `References/Lab/FAST_LIO` |
| Close out Sunray/PX4/Gazebo baseline toward flight-like deployment | `Docs/Design/MoSim真机化收尾与C++化重构方案.md`, `Docs/Design/MoSim_FASTLIO定位闭环与规划复现基础方案.md`, `Docs/Workflows/sunray_ros1_current_runtime_lane.md`; classify runtime code as T0/T1/T2/T3 before promoting Python nodes as deployable |
| Optimize current single-UAV baseline controller before MWORKS generated-code deployment | `Docs/Design/MoSim控制器调参与参数优化规范.md`, `Docs/Design/MoSim统一控制接口规范.md`, `Docs/Workflows/sunray_ros1_current_runtime_lane.md`; order is freeze frequencies -> compare control interfaces -> algorithm fixes -> parameter tuning |
...
| Validate MoSim RflySim-style four-layer architecture and UAV gates | `Docs/Design/MoSim控制体系总览.md`, `Docs/Design/架构.md`, `Docs/Design/MoSim真机化收尾与C++化重构方案.md`, `Docs/Design/MoSim控制系统测试与评价规范.md`, `Docs/Design/MoSim_FASTLIO定位闭环与规划复现基础方案.md`, cached absorbed inputs `Docs/Design/cache/absorbed_or_superseded_20260614/`, historical replan cache `Docs/Design/cache/pre_rebuild_20260610/09_UE_ROS_MWORKS无人机仿真架构重构.md`, `Results/unreal_scene_mapping/factoryenvironmentcollect/REALSTACK_MINILOOP_GATE.md`, `Results/unreal_scene_mapping/factoryenvironmentcollect/FASTLIO_FACTORY_FAILURE_DIAGNOSIS.md`, `Docs/Workflows/mworks_codegen_controller_runtime.md` |
| Design formal generated-controller deployment and future Simulink backend replacement | `Docs/Design/MoSim控制器代码生成与PX4部署规范.md`, `Docs/Design/MoSim统一控制接口规范.md`, `Docs/Design/MoSim控制系统测试与评价规范.md`, `Docs/Workflows/mworks_codegen_controller_runtime.md`; PX4/ROS2 route is disabled until explicitly reopened |
| Design multi-UAV formation architecture, identity, metrics, and database boundary | `Docs/Design/MoSim规划与编队控制接口规范.md`, `Docs/Design/MoSim统一控制接口规范.md`, `Docs/Design/MoSim控制系统测试与评价规范.md`, `Docs/Index/sunray_migration_index.md#多机编队与避障索引` |
...
| Generate MWORKS/Sysblock C/C++ controller runtime | `Docs/Workflows/mworks_codegen_controller_runtime.md`, `Docs/Design/MoSim真机化收尾与C++化重构方案.md`, `Scripts/mworks/check_codegen_runtime.py`, `Scripts/tests/test_mworks_codegen_runtime.py` |
...
| CoAgentOps patrol/recovery and thread registry hygiene | `CoAgent/docs/operating/coagent_ops_patrol_workflow.md`, `CoAgent/docs/operating/coagent_meta_maintenance.md`, `CoAgent/dispatch/department_threads.json`, `Docs/Index/codex_app_session_research.md#department-thread-layout` |
...
CoAgent is a support layer for packet schemas, registry helpers,
runtime/recovery state, result import, doctor checks, and evidence manifests.
Current ordinary MoSim work uses PMO-led direct dispatch to visible Codex
department threads, with project-local result/blocker packets as the durable
return channel. Before changing CoAgent runtime, transport, automation,
task-state schema, permanent department design, or tool expansion, check
`CoAgent/STATUS.md` and
...
surface, `CoAgent/docs/operating/agent_orchestration.md` for the full delegation
