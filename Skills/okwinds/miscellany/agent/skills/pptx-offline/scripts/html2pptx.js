...
let playwright;
let isPlaywrightCore = false;
try {
  playwright = require('playwright');
  } catch (e) {
  try {
    playwright = require('playwright-core');
    isPlaywrightCore = true;
  } catch (e2) {
    throw new Error(
      "Missing dependency: install 'playwright' or 'playwright-core' (see this skill's README.md / SKILL.md)."
    );
  }
}
...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  try {
    // Use Chrome on macOS, default Chromium on Unix. If running on playwright-core,
    // prefer a system browser channel (no bundled browser download).
    const launchOptions = { env: { TMPDIR: tmpDir } };
    if (process.platform === 'darwin') {
      launchOptions.channel = 'chrome';
    }
    if (isPlaywrightCore && !launchOptions.channel) {
