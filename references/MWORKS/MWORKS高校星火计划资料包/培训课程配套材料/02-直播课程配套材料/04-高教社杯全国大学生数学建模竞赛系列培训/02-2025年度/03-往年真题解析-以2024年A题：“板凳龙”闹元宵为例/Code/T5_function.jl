...
function iteration_vel(v_last, flag_last, flag, theta_last, theta, x_last, y_last, x, y, v0)
    x1 = -0.7600091
    y1 = -1.3057264
    x2 = 1.7359325
    y2 = 2.4484020
...
function fun_solve_T5(v0, dt)
    ...
    for t_i in 1:length_t
        for dz_i in 1:223
            ...
            v = iteration_vel(v_last, flag_last, flag, theta_last, theta, x_last, y_last, x, y, v0)
            v_dragon[dz_i+1, t_i] = v
        end
    end

    v_max_t = zeros(Int((length_t - 1) / 2 + 1))
    i = 1
    for t_i in Int((length_t - 1) / 2 + 1):length_t
        v_max_t[i] = maximum(v_dragon[:, t_i])
        i = i + 1
    end
    plot(v_max_t)
    println(maximum(v_max_t))
    return maximum(v_max_t)
end
