model DFIG_GSC_Q "双馈发电机并网系统-功率控制"
  annotation(Documentation(link = "modelica://TYWindPower/Resources/HTML/DFIG_GSC_Q.html"), Diagram(coordinateSystem(extent = {{-160, -100}, {160, 100}}, 
    grid = {2, 2})), 
    ...
    CreatePlot(id = 6, position = [0, 28, 656, 551], y = ["DFIG.wMechanical"], x_display_unit = "s", y_display_units = ["rad/s"], y_axis = [1], legends = ["转子相对于定子的机械角角度 [rad/s]"], legend_layout = 1, legend_frame = True, left_title_type = 2, left_title = "转速/rad/s", bottom_title_type = 2, bottom_title = "时间/s", right_title_type = 2, fix_time_range_value = 1.4595e-311), 
    CreatePlot(id = 7, position = [0, 28, 660, 551], y = ["Vdref.y", "voltageSensor.v"], x_display_unit = "s", y_display_units = ["V", "V"], y_axis = [1, 1], legends = ["直流侧参考电压 [V]", "直流侧实际电压 [V]"], legend_layout = 1, legend_frame = True, left_title_type = 2, left_title = "电压/V", bottom_title_type = 2, bottom_title = "时间/s", right_title_type = 2, fix_time_range_value = 1.4595e-311), 
    CreatePlot(id = 8, position = [0, 28, 658, 552], y = ["GSCtrl.power_Cal.Q"], x_display_unit = "s", y_display_units = ["var"], y_axis = [1], legends = ["无功功率 [var]"], legend_layout = 1, legend_frame = True, left_title_type = 2, left_title = "功率/var", bottom_title_type = 2, bottom_title = "时间/s", right_title_type = 2, fix_time_range_value = 1.4595e-311), 
    CreatePlot(id = 9, position = [0, 28, 656, 552], y = ["GSCtrl.power_Cal.P", "Pref.y"], x_display_unit = "s", y_display_units = ["W", ""], y_axis = [1, 1], legends = ["有功功率实际值 [W]", "有功功率参考值 [W]"], legend_layout = 1, legend_frame = True, left_title_type = 2, left_title = "功率/W", bottom_title_type = 2, bottom_title = "时间/s", right_title_type = 2, fix_time_range_value = 1.4595e-311), 
    CreatePlot(id = 10, position = [0, 28, 660, 552], y = ["resistor.v[1]", "resistor.v[2]", "resistor.v[3]"], x_display_unit = "s", y_display_units = ["V", "V", "V"], y_axis = [1, 1, 1], legends = ["A相电压 [V]", "B相电压 [V]", "C相电压 [V]"], legend_layout = 1, legend_frame = True, left_title_type = 2, left_title = "电压/V", bottom_title_type = 2, bottom_title = "时间/s", right_title_type = 2, fix_time_range_value = 1.4595e-311)})
  ...
  Modelica.Electrical.Polyphase.Basic.Inductor inductor(L = fill(Lg, 3)) 
    ...
    extent = {{-10, -10}, {10, 10}}, 
    rotation = 90)));
  TYWindPower.Controllers.DFIG.MachineSideControllerP MSCtrl(j = 77.96, Rr = 6.21e-3, Rs = 5.5e-3, p = 3, Lm = 11.01e-3, Ls = 1.71e-4 + 11.01e-3, Lr = 11.01e-3 + 2.26e-4) 
  ...
  TYWindPower.Controllers.DFIG.GridSideController GSCtrl(Id_pi(k = 5e-5, T = 0.08), Iq_PI(k = 5e-3, T = 0.01), Vdc_PI(T = 10 / 60)) 
    ...
    extent = {{10, -10}, {-10, 10}})));
  Modelica.Blocks.Sources.RealExpression Pref(y = 1e6) 
    "参考有功" annotation(Placement(transformation(origin = {-29, -62.2}, 
    extent = {{-3, -3}, {3, 3}})));
  ...
  TYUtility.SignalRouting.Goto theta_s(redeclare Modelica.Blocks.Interfaces.RealInput u) 
    annotation(Placement(transformation(origin = {-83, 76.2411}, 
    extent = {{3, -3}, {-3, 3}})), HideResult = true);
  ...
  Modelica.Mechanics.Rotational.Sources.Torque torque 
    ...
    extent = {{-10, -10}, {10, 10}})));
  TYWindPower.Controllers.Basics.PLL3ph pLL1(pI(Kp = 0.5)) 
    annotation(Placement(transformation(origin = {-48.75, 78}, 
    extent = {{4.25, -4}, {-4.25, 4}})));
  Modelica.Electrical.Polyphase.Sensors.PotentialSensor potentialSensor2 
    annotation(Placement(transformation(origin = {-33, 78}, 
    extent = {{-5, -5}, {5, 5}}, 
    rotation = -180)));
  Modelica.Blocks.Math.Gain gain(k = 2 * Modelica.Constants.pi) 
    annotation(Placement(transformation(origin = {-62.5, 76.32}, 
    extent = {{3, -3}, {-3, 3}})));
  Modelica.Blocks.Continuous.Integrator integrator 
    annotation(Placement(transformation(origin = {-72.75, 76.32}, 
    extent = {{3, -3}, {-3, 3}})));
  Modelica.Blocks.Sources.RealExpression Vdref(y = 1200) 
    annotation(Placement(transformation(origin = {86, -37.94537}, 
...
equation
  ...
  connect(GSCtrl.Us_abc, potentialSensor1.phi) 
    ...
    points = {{-15.1093, -32.7875}, {-10, -32.7875}, {-10, -29}, {16, -29}, {16, 30.5}}, 
    color = {0, 0, 127}));
  ...
  connect(GSCtrl.fire_p, DCAC.fire_p) 
    ...
    points = {{-0.10925, -18.13333}, {-5, -18.13333}, {-5, 4}, {4.89075, 4}, {4.89075, 11.8907}}, 
    color = {255, 0, 255}));
  ...
  connect(torque.flange, absoluteSensor1.flange) 
    ...
    color = {0, 0, 0}));
  connect(potentialSensor2.phi, pLL1.Uabc) 
    annotation(Line(origin = {-38, 78}, 
    points = {{-0.5, 0}, {-5.65, 0}}, 
    color = {0, 0, 127}));
  connect(potentialSensor2.plug_p, currentSensor1.plug_p) 
    annotation(Line(origin = {-22, 69}, 
    points = {{-6, 9}, {2, 9}, {2, -9}, {5, -9}}, 
    color = {0, 0, 255}));
  connect(pLL1.f, gain.u) 
    annotation(Line(origin = {-58, 76}, 
    points = {{4.575, 0.32}, {-0.9, 0.32}}, 
    color = {0, 0, 127}));
  connect(gain.y, integrator.u) 
    annotation(Line(origin = {-67, 76}, 
    points = {{1.2, 0.32}, {-2.15, 0.32}}, 
    color = {0, 0, 127}));
  connect(integrator.y, theta_s.u) 
    annotation(Line(origin = {-78, 76}, 
    points = {{1.95, 0.32}, {-1.4, 0.32}}, 
    color = {0, 0, 127}));
  connect(from4.y, MSCtrl.Ws) 
    annotation(Line(origin = {-18, -52}, 
    points = {{-11.52, 0.119959}, {11, 0.119959}, {11, 0.12}}, 
    color = {0, 0, 127}));
  connect(Pref.y, MSCtrl.Ps_ref) 
    annotation(Line(origin = {-16, -62}, 
    points = {{-9.7, -0.2}, {9, -0.2}}, 
    color = {0, 0, 127}));
  connect(GSCtrl.P, MSCtrl.Ps) 
    annotation(Line(origin = {19, -58}, 
    points = {{33.89075, 9.6}, {17, 9.6}, {17, -14}, {-33, -14}, {-33, -0.76}, {-26, -0.76}}, 
    color = {0, 0, 127}));
  connect(GSCtrl.Q, MSCtrl.Qs) 
    annotation(Line(origin = {25, -60}, 
    points = {{27.8907, 7.73333}, {11, 7.73333}, {11, -12}, {-28.6, -12}, {-28.6, -5}}, 
    color = {0, 0, 127}));
  connect(GSCtrl.Vdcref, Vdref.y) 
    annotation(Line(origin = {78, -42}, 
    points = {{-3.10925, -3.6}, {2.5, -3.6}, {2.5, 4.05463}}, 
    color = {0, 0, 127}));
end DFIG_GSC_Q;