...
#gets the static meshes in the unreal scene
    ...
    def simGetObjectFloatProperty(self, object_name, property_name):
        """
        Read a float property from an Actor in the world.

        Useful for exposing Blueprint runtime values (for example custom Yaw variables)
        to Python scripts.

        Args:
            object_name (str): Actor name
            property_name (str): Float property variable name in Blueprint/C++

        Returns:
            float: Property value, NaN if not found or not float-compatible
        """
        return self.client.call('simGetObjectFloatProperty', object_name, property_name)

    def simSpawnHumanByTag(self, human_tag, x, y, z):
        """
        Spawn a human through BP_HumanPlace::SpawnHumanByTagApi.

        Args:
            human_tag (str): Human tag to assign
            x (float): Spawn X (world/NED consistent with your blueprint usage)
            y (float): Spawn Y
            z (float): Spawn Z

        Returns:
            bool: True if blueprint reported Success=True
        """
        return self.client.call('simSpawnHumanByTag', human_tag, float(x), float(y), float(z))

    def simSetTailTrackTag(self, target_tag):
        """
        Set BP_TailCameraComponent.TargetTag in world.

        Args:
            target_tag (str): Desired target tag for tail tracking

        Returns:
            bool: True if at least one tail component was updated
        """
        return self.client.call('simSetTailTrackTag', str(target_tag))
