...
equation
  pitch_ref_raw = 0.1 * (kp_x * x_error + kd_x * x_error);
  roll_ref_raw = 0.1 * (kp_y * y_error + kd_y * y_error);
  thrust_ref_raw = kp_z * z_error + kd_z * z_error + kff_z * z_ref_rate;
  ...
  roll_cmd_raw = kp_roll * roll_error + kd_roll * roll_error;
  pitch_cmd_raw = kp_pitch * pitch_error + kd_pitch * pitch_error;
