...
scenarios/planning/sunray150_planning_open_blocks_linear_mpc_sysblock.yaml  Sunray150 A* 单障碍避让 LinearMPC-style 闭环
...
单无人机控制收尾状态：截至 2026-05-14，active 场景矩阵 71 项均有结果，54 项为 `pass`，17 项为保留的边界/负样本；单机主控制、鲁棒、故障分配、安全返航/降落闭环可以进入报告和录屏素材整理。后续再考虑瞬态故障切换、多旋翼同时故障或编队扩展。
单机规划闭环状态：`planners/astar_min_snap/map_open_blocks.yaml` 和 `planners/astar_min_snap/map_corridor_gate.yaml` 已生成 `results/planning/*/raw/reference.csv`、`metrics/trackability_report.json` 和 `figures/map_preview.svg`。两组规划预检查均 `accepted=true`、`dynamic_violation_count=0`、`trackability_score=1.0`，最小障碍距离分别为 `0.355 m` 和 `0.291 m`。接入 Sunray150 MWORKS 闭环后，LinearMPC-style 外环在 `planning_open_blocks` 中 RMSE `0.1667 m`、最大误差 `0.3511 m`、最大倾角 `0.1297 rad`，在 `planning_corridor_gate` 中 RMSE `0.1257 m`、最大误差 `0.2996 m`、最大倾角 `0.1003 rad`，两者均为 `quality_status=pass`。AWFF 规划闭环出现横向发散，保留为控制器适用性负样本。
```
