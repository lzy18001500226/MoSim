...
| UE-S0S1-20260523-LISTENER-ROUTE-AUDIT | main agent | Align listener diagnostics with the WSL wrapper's actual `UNREAL_HOST` route | `scripts/probe_unreal_mcp_listener.py`, `workflows/debug_mcp.md`, `workflows/unreal_renderer.md`, `PROGRESS.md` | verifying | Wrapper defaults `UNREAL_HOST` to WSL default gateway, while the first probe only checked `127.0.0.1`. Probe now checks explicit `--host`, `UNREAL_HOST`, WSL gateway, and `127.0.0.1`. Current result: WSL gateway timeout, localhost refused. | Commit/push. Next viewport attempt must first pass the multi-host listener probe, then use one read-only UE MCP actor/scene probe. |
