...
| AIRSIM-20260522-COMPLETE-MIGRATION | AirSimMigrationCoordinator | Finish migrating `C:\Users\HP\Desktop\AirSim` into `references/AirSim` after ignore hardening, with main-agent routed sub-agent/reviewer handoffs | `.gitignore`, `references/AirSim/`, workflow docs, Git index/branches | running | Current baseline is clean: `main` equals `origin/main` at `f7dd12c922`; no `.git/index.lock`; no `>100MB` files found under current `references/AirSim` scan. Already pushed low-risk families include `AirSim`, `AirSim360`, `PEDRA`, `PegasusSimulator`, `ProjectAirSim`, `UESVONavigation-develop`, `unrealcv-5.2`, and a Cosys-AirSim source subset. Still local/ignored by policy: `carla-ue5-dev`, `IsaacSim`, `spear`, and Cosys-AirSim large tutorial/content assets. | Continue only in small path-scoped batches from clean `main`. Do not retry old aggregate CARLA/SPEAR/IsaacSim commits. Candidate next batches: Cosys 50-100 MB assets one file per commit, SPEAR root/docs/python/examples source subsets, CARLA root/docs source subsets, IsaacSim text/source subsets excluding LFS-managed assets. |
