...
| AIRSIM-20260522-COMPLETE-MIGRATION | AirSimMigrationCoordinator | Finish migrating `C:\Users\HP\Desktop\AirSim` into `references/AirSim` after ignore hardening, with main-agent routed sub-agent/reviewer handoffs | `.gitignore`, `references/AirSim/`, workflow docs, Git index/branches | running | User confirmed sub-agents cannot communicate directly; main agent must route instructions, reviews, and grandchild follow-ups. Initial missing source dirs are `AirSim360`, `Cosys-AirSim`, `IsaacSim`, `carla-ue5-dev`, and `spear`; known >100 MB generated artifacts are in Cosys/SPEAR build outputs and AirLib libs. | Harden ignore rules first, copy missing dirs with generated artifacts excluded, verify no >100 MB tracked candidates, then hand Git commit/push to `GitIntegrator`. |
