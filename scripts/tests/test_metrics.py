...
RAW = ROOT / "scripts" / "tests" / "fixtures" / "sample_tracking.csv"
...
def load_metrics_module():
    spec = importlib.util.spec_from_file_location("calc_metrics", ROOT / "scripts" / "results" / "calc_metrics.py")
...
def test_metrics_regression() -> None:
    ...
    try:
        ...
        subprocess.run(
            [
                sys.executable,
                str(ROOT / "scripts" / "results" / "calc_metrics.py"),
                str(RAW),
                str(output),
                "sample_tracking",
                "fixture",
