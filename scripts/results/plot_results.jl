...
#
# Usage:
#   julia scripts/results/plot_results.jl results/{group}/{scene}/{experiment}/raw/figure8.csv results/{group}/{scene}/{experiment}/figures

using Dates

function main()
    if length(ARGS) < 2
        println(stderr, "Usage: julia scripts/results/plot_results.jl <raw_csv> <figure_dir>")
        exit(2)
    end
    raw_csv = ARGS[1]
    figure_dir = ARGS[2]
