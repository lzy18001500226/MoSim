...
创新图形化控制器：2026-05-11 新增 `models/QuadrotorControllerBlocks/AWFF_InnovationGraphicalControllers.mo`，包含 `AWFF_L1ResidualControllerGraphical_Sysblock`、`AWFF_INDIControllerGraphical_Sysblock`、`AWFF_L1FaultAllocationControllerGraphical_Sysblock` 和 `AWFF_L1MultiFaultIsolationControllerGraphical_Sysblock` 四个可人工审查的 Sysblock 图形控制器。该包不使用 `redeclare` 图形继承，避免单文件打开时出现类型丢失或红框；包内状态更新按 Sysblock 编译器限制使用可检查的离散/代数近似，不使用 `der()`、`when` 或 `reinit()`。真实 MCP `load_file/check_model` 复测通过，日志见 `results/model_checks/awff_sysblock/logs/sysplorer_innovation_graphical_sysblock_check_20260511.jsonl` 与 `results/model_checks/awff_sysblock/logs/sysplorer_innovation_graphical_sysblock_check_20260511_summary.json`。正式整机性能结论仍引用 Equation 控制器与 QuadrotorExperiments 场景结果；图形包用于结构截图、接口审核和控制链路说明。
