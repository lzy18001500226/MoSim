...
规划避障正式证据：`scenarios/planning/sunray150_planning_open_blocks_linear_mpc_sysblock.yaml` 已升级为局部感知避障 + 实时重规划场景，证据等级为 `real_sysplorer_mcp_sysblock_local_replanning_linear_mpc`。地图由 `planners/astar_min_snap/map_open_blocks.yaml` 固定随机种子生成 `36` 个真值障碍，Mid360 抽象局部窗口半径为 `2.5 m`；规划器只把当前窗口内障碍加入已知集合，未观测障碍只参与环境显示和事后碰撞评价。`trackability_report.json` 显示 `local_planning_enabled=true`、`local_replan_count=96`、最终已知障碍 `29/36`、`ego_optimizer_accepted=true`、最小障碍距离 `0.5998 m`、碰撞计数 `0`。对应 MWORKS/Sysplorer 闭环模型 `QuadrotorExperiments.Sunray150PlanningOpenBlocksLinearMPCSysblockClosedLoop` 全时长 `80.213 s` 通过质量门：RMSE `0.1429 m`、最大位置误差 `0.3107 m`、健康分 `94.0968`，并已进入 `results/人工审核清单.csv` 的 `planning` 高优先级人工审核项。
