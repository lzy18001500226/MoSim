...
创新图形化控制器：2026-05-11 新增 `models/QuadrotorControllerBlocks/AWFF_InnovationGraphicalControllers.mo`，文件根节点本身是可打开的总览模型，双击打开即可看到 4 个创新控制器入口；内部包含 `AWFF_L1ResidualControllerGraphical_Sysblock`、`AWFF_INDIControllerGraphical_Sysblock`、`AWFF_L1FaultAllocationControllerGraphical_Sysblock` 和 `AWFF_L1MultiFaultIsolationControllerGraphical_Sysblock` 四个可人工审查的 Sysblock 图形控制器。该包不使用 `redeclare` 图形继承，避免单文件打开时出现类型丢失或红框；包内状态更新按 Sysblock 编译器限制使用可检查的离散/代数近似，不使用 `der()`、`when` 或 `reinit()`。2026-05-12 进一步为底层方程子块 `L1ResidualOuterLoopBlock`、`PIDAttitudeInnerLoopBlock`、`INDIAttitudeInnerLoopBlock`、`MotorMixerBlock`、`KnownRotorFaultMixerBlock`、`RotorFaultIsolationBlock` 和 `AdaptiveFaultMixerBlock` 补齐 `Diagram.graphics` 内部信号流线，解决双击进入子文件后看不到连线的问题；该修正只增加审核用图形元素，不改变任何控制方程。真实 MCP `load_file/check_model` 复测通过，日志见 `results/model_checks/awff_sysblock/logs/sysplorer_innovation_graphical_overview_check_20260511.jsonl`、`results/model_checks/awff_sysblock/logs/sysplorer_innovation_graphical_overview_check_20260511_summary.json`、`results/model_checks/awff_sysblock/logs/sysplorer_innovation_graphical_line_visibility_20260512.jsonl` 和 `results/model_checks/awff_sysblock/logs/sysplorer_innovation_graphical_line_visibility_20260512_summary.json`。正式整机性能结论仍引用 Equation 控制器与 QuadrotorExperiments 场景结果；图形包用于结构截图、接口审核和控制链路说明。
